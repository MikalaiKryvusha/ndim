...
| Правила безопасности | [`firestore.rules`](firestore.rules) и [`storage.rules`](storage.rules) — единственный сторож между клиентом и чужими данными | **94** теста на эмуляторах, проверяют **отказы**; мутации ловятся |
...
| Экран «Профиль» | [`src/routes/profile/`](src/routes/profile/+page.svelte) — аудитории по кругам у каждого свойства, окно **«Как меня видят»** (предпросмотр глазами Я/Все/Друзья), **статистика NDim ID и связей виджетами** (канон «Дома» 1.x) | живой стенд + e2e + QA-прогон живым Chrome; смена аудитории физически перекладывает бакеты |
| Экран «Связи» | [`src/routes/relations/`](src/routes/relations/+page.svelte) — топ похожих: три метрики сразу, «яркость связи»; раскрытие — **досье знакомства**: человек по его же настройкам видимости и параметры пространств с коэффициентами сравнения | живой стенд + e2e + QA-прогон живым Chrome |
| Вычислитель связей | [`calculator/`](calculator/index.mjs) — Docker-служба, очередь «грязных» точек, топ-250 в `relations/`; только исходящие соединения. Экономия запросов в духе 1.x: кэш точек в памяти, запись только изменившихся топов, тихий период с окном новичка, суточный полный проход (`researches/13`) | прогон на эмуляторе, из контейнера и в бою |
...
| Аккаунт без пароля | [`src/lib/data/account.ts`](src/lib/data/account.ts) — Google и ссылка на почту. Два намерения различаются словами человека, а не угадываются по сессии: **«сохранить мои результаты»** → `linkWithCredential`, UID тот же и труд гостя остаётся; **«у меня уже есть аккаунт»** → обычный вход, с честным предупреждением до письма | стенд: оба пути сквозным прогоном (58 проверок), мутация ловится |
...
| Память вида экранов | [`src/lib/ui/view-memory.ts`](src/lib/ui/view-memory.ts) — страница открывается там, где её оставили: прокрутка, вкладка, докуда долистано | 60 проверок живым браузером, 3 мутации |
| Приложение на телефон | [`static/manifest.webmanifest`](static/manifest.webmanifest) — манифест и иконки, растущие из утверждённого знака | 24 проверки по собранному сайту, 3 мутации |
...
| Паритет с 1.x | инвентарь поведения 1.x снят целиком ([`researches/12`](researches/12_1x_behavior_inventory.md)); боли переезда разложены по тирам ([`plans/07`](plans/07_owner_pain_tiers.md)); **волны 10–13 в бою**: поиск по всему Пространству, **руководство V1 «Как было»** — иллюстрации 1.x, таблица шкалы со звёздами и цветными смайликами, плавающий пагинатор глав; **карточка ждёт фото** (лица без мерцания, канон 1.x); виджет «Версии» с номерами сборок и датами приложения И сервера; **навигация без загрузок с нуля** — данные экранов живут в памяти вкладки весь сеанс ([`ideas/18`](ideas/18_DONE_session_cache_for_screen_data.md), в бою с 2026-07-29: повторный круг по пяти экранам стоил 49 чтений, стал 3) | 🔧 |
...
npm test           # 192 юнит-теста: ядро, модель данных, статистика NDim ID, кэш сессии, витрина
npm run test:rules # 94 теста правил Firestore и Storage на эмуляторах (проверяют ОТКАЗЫ)
npm run test:calc  # 37 тестов сервера синхронизации на эмуляторе
npm run e2e        # 72 браузерные e2e-проверки (Playwright, продакшен-сборка; гаси стенд!)
...
| Security rules | [`firestore.rules`](firestore.rules) — the only guard between a client and other people's data | **87** emulator tests asserting **denials**; mutations get caught |
...
| Profile screen | [`src/routes/profile/`](src/routes/profile/+page.svelte) — per-property audiences by circles, the **"How others see me"** window (a preview through Me/Everyone/Friends eyes), **NDim ID and relations statistics as widgets** (the 1.x "Home" canon) | live stand + e2e + a live-Chrome QA pass; audience change physically redistributes buckets |
| Relations screen | [`src/routes/relations/`](src/routes/relations/+page.svelte) — top similar people: all three metrics at once, "connection brightness"; expanding a card gives a **dossier**: the person as their own visibility settings allow, plus space parameters with comparison ratios | live stand + e2e + a live-Chrome QA pass |
| Relation calculator | [`calculator/`](calculator/index.mjs) — Docker service, dirty-points queue, top-250 into `relations/`; outgoing connections only. Request economy in the 1.x spirit: in-process points cache, changed-tops-only writes, quiet period with a newcomer window, daily full pass (`researches/13`) | verified on the emulator, from the container, and in production |
...
| Parity with 1.x | the 1.x behaviour inventory is fully mapped ([`researches/12`](researches/12_1x_behavior_inventory.md)); migration pains are sorted into tiers ([`plans/07`](plans/07_owner_pain_tiers.md)); **waves 10–13 are live**: search across the whole Space, the **V1 "As it was" User Manual** — 1.x illustrations, the grade table with stars and coloured emojis, a floating chapter paginator; **cards wait for photos** (faces without flicker, the 1.x canon); the "Versions" widget with build numbers and dates for both the app AND the sync server; **navigation without loading from scratch** — screen data lives in the tab's memory for the whole session ([`ideas/18`](ideas/18_DONE_session_cache_for_screen_data.md), live since 2026-07-29: a repeat lap across five screens used to cost 49 document reads, now 3) | 🔧 |
...
npm test           # 173 unit tests: core, data model, NDim ID statistics
npm run test:rules # 87 Firestore rules tests on the emulator (asserting DENIALS)
npm run test:calc  # 30 sync-server tests on the emulator
npm run e2e        # 70 browser e2e checks (Playwright, production build)
