...
> **✅ STATUS: DONE (2026-07-17).** Владелец проверил руками: «Смог зайти через гугл» —
> вход через Google на проде работает. По шагу 7 cutover (`plans/04`) проверены живьём
> Google и почтовая ссылка; гость с лендинга — остаётся на слове владельца.
>
> Хроника закрытия:
> Владелец создал новый клиент по пути Б (`…78klr724…`, креды в `google/` — папка в
> `.gitignore`, в git не попадает; origins и redirect URI заполнены верно). Агент прописал
> клиент в провайдер Google через Identity Toolkit API (`PATCH defaultSupportedIdpConfigs/
> google.com`, нужен заголовок `x-goog-user-project`) и проверил наблюдением:
> конфиг `enabled=true` с новым clientId; живой эндпоинт `accounts.google.com/o/oauth2/v2/auth`
> с этим клиентом и нашим redirect URI отдаёт **страницу входа** — ошибок `deleted_client` /
> `redirect_uri_mismatch` больше нет. [TESTED: 2026-07-17 · API-конфиг + живой OAuth-эндпоинт]
> **Осталось:** владельцу один раз нажать «Войти через Google» на
> https://ndimspace.app/profile (попап-флоу автотестом не закрыть) — после его слова файл
> помечается DONE.
