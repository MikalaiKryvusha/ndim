...
Владелец сгенерировал три иллюстрации (единый стиль, акцентные цвета совпали) и положил в
`static/img/personas/`. Агент выкропал лица «общим планом» (правка владельца: причёски должны
влезать целиком; кроп — System.Drawing, без новых зависимостей) → `*_face.png` 320×320 и
встроил в демо все три подхода владельца: кроп в кружках-аватарах и узлах карты,
hover-увеличение (×2.2 карточка, ×2.1 узел карты), тап → оверлей с полным портретом
(Esc/клик закрывает), мягкий пульс кольца (уважает `prefers-reduced-motion`).
e2e: 30 проверок, включая «тап открывает полный портрет».
