...
> **Статус:** ✅ **ЗАКРЫТ 2026-08-16 (сессия 18)** — код в `SimilarityDemo.svelte`, страж
> `tools/verify-tier2-prefetch.mjs` **7/0**, доказан 2 мутациями (1 и 3 адресных падения),
> Lighthouse без просадки (80 → 80 на одном и том же preview). Исполнила СИЛЬНАЯ модель.
> **Исходящее:** строка стража → реестр стенда `AGENT_GUIDE.md` ✅ и `STATUS.md` ✅
>
...
## ✅ ИСПОЛНЕНО 2026-08-16 (сессия 18)

**Код** — `src/lib/ui/SimilarityDemo.svelte`: `warmFullPersonas` + `idle` в новом `onMount`
(`onMount` в файле не было вовсе — план это предусматривал). Больше не тронуто ничего.
`npm run check` 490 файлов / 0 ошибок · `npm test` **284/0** (план ожидал 234 — число устарело).

**Страж** — `tools/verify-tier2-prefetch.mjs`, **7 проверок, 0 провалов**. Стережёт С ОБЕИХ
сторон: «нет в сыром HTML» и «есть в живом `head` после `load`». Проверка одной стороны была бы
зелёной на удалённом коде.

**Мутации — адресаты названы ДО прогонов:**

| Мутация | Ожидание | Факт |
|---|---|---|
| м1: снят guard `saveData` | ровно 1 падение (экономия трафика) | **ровно 1**, адресное |
| м2: ссылки в разметку (`svelte:head`) | 2 падения (сырой HTML + `saveData`) | **3** — плюс «в живом head ровно 3» → их 6 |

⚠️ **Честно о м2: я предсказал 2, вышло 3.** Причина не в страже, а в записи мутации: ссылки в
разметку я ДОБАВИЛ, не заменив ими скрипт, поэтому в `head` их стало шесть. Страж поймал и
дублирование — сигнал сильнее ожидаемого, но предсказание было неточным. Расхождение меньше чем
вдвое, то есть STOP-ASK плана не сработал (и правильно).

**Замер Lighthouse (шаг 4), preview против preview на одной машине — правило `bugs/119`:**

| | Perf | FCP | LCP | TBT | Speed Index |
|---|---|---|---|---|---|
| ДО (правка в stash) | **80** | 2,6 с | 4,5 с | 0 мс | 2,6 с |
| ПОСЛЕ | **80** | 2,6 с | 4,5 с | 0 мс | 2,6 с |

Просадки нет — критерий «не ниже ДО − 2» исполнен. ⚠️ Число 80, а не боевые 100, потому что
машина была загружена (стенд, эмуляторы, браузеры замеров); сравнивать боевой Perf с preview
запрещено тем же `bugs/119`, поэтому «до» снят тем же способом на той же машине.
