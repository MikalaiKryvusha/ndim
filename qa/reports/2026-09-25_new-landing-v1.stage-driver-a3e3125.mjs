/**
 * ДРАЙВЕР РУЧНОГО ПРОГОНА НА СТЕЙДЖЕ — новая V1 главной (набор `qa/suites/new-landing-v1.md`, кейсы НЛ-12, НЛ-14 (повтор на a3e3125),
...
const OUT = 'test-results/new-landing-v1-stage-a3e3125';
...
export async function run() {
  ...
  try {
    const q = await hogql(
      `SELECT event, properties.$pathname, properties.$browser, properties.entry, properties.env, timestamp FROM events WHERE timestamp >= toDateTime({since}, 'UTC') AND properties.env = 'stage' AND event IN ('landing_view', 'demo_touch', 'guest_start') ORDER BY timestamp`,
