...
test('мост в гостя: кнопка приходит после звёзд, ведёт в гостя и не оставляет записи истории', async ({ page }) => {
	...
	// из 10, с паузой перед кликом 20 из 20), поэтому клик повторяется, пока мост не придёт. Кейс стережёт МОСТ; ранний
	// тап человека до гидратации — отдельный риск фазы 2 (`plans/104`, риски) и отдельный кейс её набора.
	await expect(async () => {
		await page.getByRole('button', { name: 'Спорт: 10' }).click();
		await expect(bridge).toBeVisible({ timeout: 1000 });
