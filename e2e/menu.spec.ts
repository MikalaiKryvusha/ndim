...
test('меню: манифест пререндерен в сыром HTML — это лицо проекта, а не всплывающий текст', async ({ request }) => {
  ...
  // Заголовок переименован владельцем 2026-07-27 («заменить просто на Манифест Ndim Space»).
  // Страж стережёт ровно то же, что и раньше: манифест лежит в СЫРОМ пререндеренном HTML,
  // а не подгружается скриптом. Изменилась строка канона, а не то, что проверяется.
  expect(html).toContain('Манифест NDim Space');
