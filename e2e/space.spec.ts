 ...
 *
 * ⚠️ Прогон требует, чтобы эмуляторы были ПОГАШЕНЫ: проверка «стенд не поднят» ждёт их
...
test('пространство: без эмуляторов — честное состояние «стенд не поднят»', async ({ page }) => {
