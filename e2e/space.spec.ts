...
test('пространство: без данных — честная ошибка, и ни одной выдуманной цифры', async ({ page }) => {
  ...
  // `:not(.brand)` — знак продукта в рельсе тоже ведёт на «Профиль» (владелец 2026-07-27:
  // клик по логотипу не должен мелькать лендингом). Проверяется ровно прежнее: ПУНКТ
  // навигации ровно один и он видим, а не «сколько всего ссылок на /profile на экране».
  await expect(page.locator('nav a:not(.brand)[href="/profile"]:visible')).toHaveCount(1);
});
