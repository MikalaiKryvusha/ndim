...
**Последнее обновление:** 2026-08-14 (языковые адреса: дерево `src/routes/` — `plans/39` шаг 2)
...
├── AUTHOR_STYLOMETRY.md                       # ★ портрет голоса владельца: КАКИМ ЯЗЫКОМ писать текст,
│                                        #   который человек читает как свой (снят 2026-08-02)
...
│   ├── +page.svelte             # ★ КОРЕНЬ-РАСПОЗНАВАТЕЛЬ языка (plans/39): noindex, письмо→/profile,
│   │                            #   память→браузер→location.replace на /ru | /en. НЕ лендинг!
│   ├── [lang=lang]/             # ★ ПУБЛИЧНЫЙ САЙТ на языковых адресах /ru/… и /en/… (plans/39 шаг 2)
│   │   ├── +layout.svelte       #   мост «адрес → память» (adoptLangFromPath) — одно место на все
│   │   ├── +page.svelte         #   ЛЕНДИНГ — макет «Колонна», тексты владельца; /ru и /en
...
│   ├── menu/+page.svelte        # экран «Меню» — ЛИЧНЫЙ; ссылки на документы несут /{lang}
│   ├── admin/+page.svelte       # дом админ-панели (plans/33): noindex, вне sitemap
│   └── sitemap.xml/+server.ts   # пререндеренный sitemap: 22 языковых + каталог = 10 244
...
├── src/lib/ui/ScreenHead.svelte # шапка экрана: заголовок + плашка-подсказка, одна на пять экранов (2026-09-19)
...
│  ── ИНСТРУМЕНТЫ ──
├── tools/
│   ├── stamp-build.mjs      # ★ кладёт `build/build-stamp.json` шагом `postbuild` — публичный
│   │                        #   адрес продукта, которого в дереве НЕТ (врезка под деревом)
...
> ## 📌 `/build-stamp.json` — публичный адрес, которого в дереве нет
>
> Продукт отдаёт `/build-stamp.json`: `sha` · `dirty` · `builtAt` · `buildNumber` — из какого
> дерева собран артефакт. **В дереве репозитория этого файла нет вовсе:** его пишет
> `tools/stamp-build.mjs` шагом `postbuild`, то есть он рождается сборкой.
>
> 🔑 **Строка стоит здесь ровно ради последнего свойства.** Прочие служебные файлы, которые
> продукт отдаёт, лежат в `static/` и находятся чтением дерева; этот — нет. Сессия, которая
> ищет «что мы отдаём наружу», найдёт его отсюда, а не случайно.
>
> ⚠️ **Это ИЛЛЮСТРАЦИЯ, а не опись, и врезка не объявляет список отдаваемого полным.** Карта —
> ориентир «где что лежит» (см. её собственную шапку), и правило «обновляй состав при добавлении
> файла» здесь НЕ заводится намеренно: опись, ведомая руками, — это `bugs/226`, «список,
> пополняемый руками, молчит о своём неполном составе».
> 📌 Настоящее лечение названо и отдано смене 15 постановкой: авторитет о том, что продукт
> отдаёт, — сам `build/`, и он ПЕРЕЧИСЛИМ; страж сравнивает нестраничные файлы артефакта с
> объявленным составом. Машинный признак здесь есть — потому страж и уместен.
>
> 📌 Разбор содержимого файла — вердикт QA №43: прецедент живой (`robots.txt`, `sitemap.xml`,
> `manifest.webmanifest` уже публичны), репозиторий проекта публичен, `dirty: true` в бой не
> уедет — `tools/deploy.mjs` отказывает жёстко.
...
| `tools/stamp-build.mjs` | Кладёт метку `build/build-stamp.json` — из какого дерева собран артефакт. Публичный адрес продукта, которого в дереве нет (врезка выше). | шаг `postbuild`; читает `tools/lib/build-freshness.mjs` |
| `tools/lib/build-freshness.mjs` | Судья артефакта: свежесть по времени и происхождение по метке. Пять ответов, а не два — «неизвестно» отделено от «неверно». | `tools/verify-catalog-hubs.mjs`, `tools/shoot-hub-snippets.mjs` |
