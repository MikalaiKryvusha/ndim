...
- **Форма — семь полей, все обязательны, каждое линтуется** (`node tools/verify-test-reports.mjs`):
  **Работа** (что проверяли) · **Контур** (стенд · стейдж · бой) · **Прогонов** (число и метки
  времени) · **Проверок** (пройдено / провалено, и провал назван) · **Найдено** (дефекты с
  адресами заведённых документов либо честное «ноль») · **Следы** (что прогон оставил в контуре:
  учётки, записи, кадры) · **Вердикт** (годится к следующему шагу или нет, и почему).
