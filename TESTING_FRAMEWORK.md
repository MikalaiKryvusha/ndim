...
**Повод, названный честно:** Менеджер принёс владельцу вопрос «катите?» по фиче аналитики, у
которой не было НИ ОДНОГО из перечисленного — ни плана, ни кейсов, ни чек-листа, ни зелёного
прогона. Владелец — профессиональный QA-инженер, и он это увидел первым же вопросом.
🔑 Хуже того: сам предложенный прогон стейджа не проверил бы фичу **по построению** — аналитика
включается белым списком боевых хостов, стейдж в него не входит и не отправляет ни одного события.
То есть агент собирался предъявить владельцу зелёное, которое не могло покраснеть. Это ровно тот
род ложного зелёного, за которым проект охотится весь свой канон, — и он дошёл до владельца.

**Ворота, и они СТОЯТ ПЕРЕД словом, а не перед выкатом:**

Прежде чем произнести владельцу «катить?», «готово к выкату», «выкатываем?» — по КАЖДОЙ фиче
существуют, лежат в файлах и названы адресами:
...
3. **Чек-листы** — упорядоченный набор, по которому прогон воспроизводится другим человеком или
   другой сессией.
4. **Тестовая документация в доме этого класса** — здесь это `qa/JOURNEYS.md` и `qa/suites/`,
   а не голова сессии и не сообщение в чат.
5. 🔴 **ЗЕЛЁНЫЕ ПРОГОНЫ ПО ЭТИМ КЕЙСАМ.** Не «ворота зелёные», не «юниты 447» — зелёные прогоны
   ПО ЭТОЙ ФИЧЕ, с числами и адресом кадров.

⛔ **Чего эти ворота НЕ принимают:**
· «тесты соседней фичи зелёные» — они о соседней фиче;
· «ворота проекта 13/13» — они стерегут стоящее в стволе, а не то, что делает новая фича;
· прогон на контуре, где фича **физически не работает** (см. повод выше) — проверка, неспособная
  покраснеть, зелёным не считается;
· «проверю после выката» — тестирование неотъемлемая часть разработки, а не следующий за ней шаг.

### 🔑 Контур, где фича молчит по построению, — это КОНТРОЛЬ, а не проверка

Выведено QA-инженером проекта в тот же час, и это точный диагноз промаха выше, а не его пересказ.

**Молчание стейджа не было провалившейся проверкой — это готовый ОТРИЦАТЕЛЬНЫЙ КОНТРОЛЬ:** «на
хосте не из белого списка не уходит ни одного события» стоит там бесплатно и по построению. Не
хватало ровно положительной половины, и без неё контроль был контролем ни от чего.

**Правило: пара «контроль + положительный опыт» ставится ЦЕЛИКОМ или не ставится.** Промах агента
был не в том, что он предложил стейдж, а в том, что **половина пары была принята за пару**.

Положительная половина нашлась и не потребовала ни выката, ни второго проекта, ни слова владельца:
собранный боевой артефакт поднимается ПОД БОЕВЫМ ИМЕНЕМ (`chromium --host-resolver-rules="MAP
<боевой хост> 127.0.0.1:<порт>"`), исходящее к чужому сервису перехватывается и наружу не
выпускается. Замер: обращений к сервису **3**, ушло наружу **0**, контроль прибора под `localhost`
— **0**. Белый список открывается по-настоящему, продукт не тронут ни на строку.
🔑 **Подменяется РАЗРЕШЕНИЕ ИМЕНИ, а не список хостов** — и в этом вся разница с соблазнительным
«временно расширить список на стенде». Правка списка делает предметом проверки МУТИРОВАННЫЙ
продукт: зелёное получено на том, что не поедет. Список обязан остаться байт-в-байт тем, который
уезжает.

⚠️ И граница, названная тем же замером: доказано, что ворота хоста открываются и SDK стартует.
Утверждение о ТЕЛЕ события (имя из белого списка, свойства внутри союза, предмет оценки не уходит)
— следующий слой, достижимый тем же приёмом через перехват отправки, и он в число доказанного
НЕ входит.
📌 Проверка ПРИЁМА чужим сервисом («он принял и показал») этим приёмом недостижима в принципе и
живёт в двери выката рядом со смоуком под сессией — как приёмка ПОСЛЕ выката, а не как ворота
перед словом «катить».

🔑 **И правило для агента, вытекающее из формы обязательства** (`AGENT_GUIDE.md` → «Форма
обязательства»): вопрос владельцу о выкате несёт ССЫЛКИ на все пять пунктов, и это его пропуск. <!-- ВОПРОС-ОК: канон описывает ФОРМУ вопроса о выкате, сам вопросом не является -->
🔑 И первым же прогоном этот раздел покрасил ствол СОБСТВЕННОЙ строкой выше: страж места вопросов
прочитал её как вопрос владельцу вне `interviews/`. Красное нашла роль, а не автор канона. Это
не курьёз, а подтверждение самого правила — **обязательство, записанное прозой, ловится машиной
ровно там, где машина умеет ловить**, и объявление исключения обязано нести причину, иначе маркер
станет способом заткнуть стража.
Нет ссылок — вопрос не задаётся вовсе, а сессия идёт писать тестовую документацию.
...
**Повод, названный честно.** Работу приоритета 0 «первый экран гостя показывает оценённый объект»
проверяли **двумя прогонами одного прибора** на стейдже. Прибор напечатал восемь шагов и положил
рядом файл вердикта. Записка следующей смене сказала «стейдж принят, всё зелёное». Ни тест-плана,
ни кейсов, ни чек-листа, ни отчёта не было, а работа при этом подавалась владельцу как готовая к
бою. Числа, снятые приборами уже по его вопросу: прогонов 2 · тест-кейсов проекта 0 · багов
заведено 0 · анонимных учёток заведено на контуре 9.

🔑 **Почему предыдущих ворот не хватило, и это главное.** Ворота выше стоят ПЕРЕД словом «катить»
и требуют документов, написанных ДО прогона. Между **исполненным прогоном** и документом не стояло
ничего: маркер `[TESTED: …]` — это пометка В СТРОКЕ, а не артефакт, вердикт прибора — это выход
инструмента, который читает только запустивший его агент. Сессия честно прогоняла, честно ставила
маркер и честно отчитывалась числом в чат — и работа считалась проверенной, не оставив ни одного
документа, который можно открыть и прочитать.
...
- **Адрес:** `qa/reports/<ISO-дата>_<короткое-имя-работы>.md`. Один документ на прогон или на
  серию прогонов одной работы за день.
- **Форма — семь полей, все обязательны; страж проверяет, что каждое есть и не пусто**
  (`node tools/verify-test-reports.mjs`). ⚠️ Граница стража, названная вслух (2026-09-18, судья обновления
  KAIF 2.7): точную команду каждого прогона, две строки `Гигиена:` / `Функциональный прогон:` и слово
  вердикта он НЕ проверяет — эта половина держится чтением; замер того дня: ни один из 13 отчётов
  `qa/reports/` двух строк не несёт. Поля:
...
- **Маркер `[TESTED: …]` о прогоне называет адрес своего отчёта** рядом с датой и уликой
  ...
  ловит как «протестировано без отчёта» (KAIF 2.7, issue #59 истока — это правило вернулось к нам
  поставкой: #59 родился из нашего случая 2026-09-09).
- **Числа снимаются приборами, а не памятью.** Число без команды, которой оно снято, — это
  утверждение о памяти сессии.
- **«Найдено: ноль» — законный вердикт**, и он ценен: он отличает «искали и не нашли» от
  «не искали».
...
📌 KAIF 2.7 поставляет свой линт той же формы — `node .kaif/tools/kaif-testrun-lint.mjs check`
(дом по умолчанию `testcases/`, настройка `.kaif/kaif.json` → `testdocs`). У нас он НЕ подключён:
наш дом — `qa/reports/`, а имена полей нашего канона («Прогонов», «Проверок») расходятся со словарём
поставки («Прогоны», «Проверки») — замер 2026-09-18: 16 находок в 14 отчётах, все о словаре, ни
одной о смысле. Наш страж остаётся единственным судьёй формы.

⚠️ **Названная граница, чтобы правило не выглядело шире, чем оно есть.** Страж судит ФОРМУ
существующих отчётов и полноту реестра. Он не умеет доказать, что прогон, о котором не написали,
вообще был, — ненаписанного отчёта не видит никто. Эту половину держит правило и вопрос владельца,
и это записано честно, а не закрыто ссылкой на стража.
...
**Повод, названный честно.** Серия починок красных багов 2026-09-12 доложила «закрыто 25, все
протестированы». Под «протестированы» сессия понимала: юнит зелёный, самотест зелёный, мутация
К4 красная адресно. По определению владельца ручной функциональный прогон был у **3 из 25**
(прибор руками запущен на живых данных, вывод прочитан), частичный у 2, и у 20 не было вовсе.
Число «25 протестированы» было правдой о стражах и неправдой о тестировании.
...
6. **Как это исполняет агент** (KAIF 2.7, решение владельца истока №116 — пересказ поставки, не
...
2. **Meeting `[NOT-TESTED]`** (yours or inherited) → do not build on it blindly: plan its verification,
   ...
   🔴 **Observation here means a MANUAL functional run** (the owner's word 2026-09-12, section above):
   a green unit test or a red mutant is developer hygiene and does NOT flip the marker.
...
10. 🔴 **УВЕРЕННЫЙ НОЛЬ — ЭТО ЗЕЛЁНЫЙ, И РАЗВОДИТ ЕГО ЛЕСТНИЦА ТРЁХ ВОПРОСОВ, А НЕ ОДНО ЛЕЧЕНИЕ.**
    ...
    ⚠️ **Ступени берутся ВСЕ ТРИ.** Канон лестницы говорит это буквально: «*порядок обязателен, и
    третий вопрос имеет смысл только после первых двух*», «*лестница ставится целиком или не
    ставится*» (`AGENT_GUIDE.md` → «ЛЕСТНИЦА ТРЁХ ВОПРОСОВ»). Первая редакция этого правила брала
    только ступени 2 и 3 — то есть была не достройкой одного инструмента, а **частичной копией**,
    и довод «не множу сущности» на ней не держался. *(Пробел найден Интегратором на вычитке.)*
    ...
    **Вопрос 2 — промах ВЫРАЖЕНИЯ: прибор смотрит не ТЕМ.** Алфавит, кодировка, форма имени.
    ...
    вопроса 2 знаменатель ЗДОРОВ и потому усыпляет: корпус прочитан целиком, `EXPERIENCE.md` — 267
    записей, дробь напечатала бы «0 из 267», и следующая сессия прочла бы её как честный ноль.
    ⚠️ **В первой редакции здесь стояло «232», и это число было ЛОЖНЫМ** — автор случая назвал его
    по памяти в переписке, я процитировал не замерив, и оно уехало в канон. Перемер:
    `git show main:EXPERIENCE.md | grep -c '^### EXP-'` → **267**. Вывод правила не меняется
    (знаменатель здоров при любом из двух), но случай стоит здесь как улика: **число, произнесённое
    без замера, дальше цитируется как замеренное** — и это тот же класс, против которого написано
    само правило, пойманный внутри него. 🔑 Отсюда строка, которой правило обязано ему же:
    **цитируя ЧУЖОЕ число, спроси, замерено ли оно** — автор мог сказать его по памяти.
    ...
    **знание класса не защищает — защищает форма.**
    ...
    · *Дробь (вопрос 3)* не нужна: на ПОСТРОЕННОЙ фикстуре юнита (материал создан тремя строками
      ...
      **Обязательна при двух условиях сразу:** прибор адресует материал, которого автор НЕ создавал,
      И ноль будет **ПРОИЗНЕСЁН ЛИБО ПОЛОЖЕН В ОСНОВАНИЕ СЛЕДУЮЩЕГО ШАГА**.
      🔑 **Вторая половина этого условия — ТИХАЯ, и она частая** (уточнение Интегратора, принято).
      Громкий случай виден сам: ноль едет в отчёт, вердикт или документ как утверждение. Тихий не
      виден вовсе: **ноль никуда не произносится, но на нём ДЕЙСТВУЮТ** — «апострофов ноль → чинить
      нечего → иду дальше». Решение невидимо, цена та же. Прежняя редакция требовала знать заранее,
      поедет ли число в отчёт, — а это знание ЗАДНИМ ЧИСЛОМ, и тихую половину она не покрывала
      вовсе. Проверка на живых случаях: мой прошёл по громкой (шёл опровергать документ владельца),
      случай Интегратора — по тихой (мерил для своей карты, «пусто» просто закрыло бы вопрос;
      произнёс он его только потому, что решил написать отчёт).
...
именно там, пока все приборы агента зелёные по построению (KAIF 2.6, issue #52 истока; слово <!-- KAIF-VERSION-OK: версия прихода правила -->
владельца истока в пересказе поставки: агент ОБЯЗАН проверять на реальном мире, чтобы не ломать
то, что уже в бою). На этом проекте класс оплачен дважды: смоук гостем при неработающем входе под
сессией (ворота 6 выше) и вечная «Загрузка» у каждого, кто хоть раз входил, при зелёной двери выката
(`AGENT_GUIDE.md` → «КОНТЕКСТ ВОЗВРАЩАЮЩЕГОСЯ ЧЕЛОВЕКА»).
...
- **Форма отчёта о прогоне как страж** — у проекта свой: `node tools/verify-test-reports.mjs`
  (`qa/reports/`). Поставляемый опциональный модуль `kaif-testrun-lint` (`.kaif/tools/`) судит те же
  семь полей и каталог по датам, `SKIPPED=3` без `reports/` в доме; у нас не подключён — причина в
  разделе «ИСПОЛНЕННЫЙ ПРОГОН РОЖДАЕТ ОТЧЁТ».
