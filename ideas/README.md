...
реализуй до одобрения. Документ идеи открывается болью, которую она решает, + тем, как проверим, что
сработало (`REQUIREMENTS_FRAMEWORK.md`), а сразу после H1 несёт линтуемую мета-шапку — **Создан:** ·
**Родитель:** · **Статус:** · **Исходящее:** (`AGENT_GUIDE.md` → «Мета-шапка документов»). После
реализации впиши статус и дату в файл и пометь `DONE` (`git mv NN_x.md NN_DONE_x.md`).
