...
<!-- ✅ ПРИНЯТО ВЛАДЕЛЬЦЕМ: интервью №063, В5 = А (2026-08-29 18:02) — запись 2.2 канонична.
Пять заходов вычитки: туториал-императив снят («это не что нового, а туториал») · «выверенные
вручную» снято («мы не оправдываемся») · вторая строка — его формулировкой из 1.4 · предмет
назван существительным («оценки чего?!») · тире убрано. Трассировка строк на факты: оценки в
каталоге/лендинге — plans/75 (выкат b81u35) + plans/67; пополнение — посевы партий и
возвращённых карточек. Дата в записи — день фактического выката, сверить при релизе. -->
...
<!-- Заход 2 (2026-08-29, №063 В1 = Г): «Первая оценка — это вход: поставьте…» снята — «это не
что нового, а туториал» (императив учит пользоваться вместо называния нового); «выверенные
вручную» снято — «это оправдание. Мы не оправдываемся» (канон лица продукта).
Заход 3 (№063 В2 = В): «открывает Пространство гостем» убрано его словом; вторая строка — его
же формулировкой из версии 1.4, релиз-нот пишется ЕГО прошлыми формулировками, не сочиняется.
Заход 4 (интервью №063, В3): «Оценки прямо в каталоге и на лендинге. — оценки чего? Разве это
по моей стилометрии писать без существительного?!» — предмет назван существительным, строка
стала «Оценки измерений — прямо в каталоге и на лендинге» по форме его же строки 2.1 («Сводная
оценка измерений — NDim Space Rating»). Это применение запрета 6 портрета: местоимение и
безобъектный термин стоят при своём существительном.
Заход 5 (интервью №063, В4): «Оценки измерений — зачем тире?» — тире убрано, строка приняла
окончательный вид «Оценки измерений прямо в каталоге и на лендинге». -->


<!-- ✅ ПРИНЯТО ВЛАДЕЛЬЦЕМ: интервью №027, В1 = А (2026-08-14 11:42) — запись 2.1 канонична.
Заказ и трассировка строк на факты — plans/43. -->
...
> 🔴 **Из версии 1.3 СНЯТА одна строка — слово владельца 2026-08-01 (интервью №010, Р15).**
> Была: RU «Добавлена аналитика рекламной атрибуции» / EN «Added advertising attribution analytics».
>
> **Почему:** волна «Ворота» открывает документы продукта поиску, и эта строка встала бы в индекс
> рядом с обещанием лендинга «у нас без рекламы» — продукт публично противоречил бы сам себе.
> Владелец пояснил, о чём шла речь: «Это я имел в виду Amplitude и то, что я ссылку на сайт
> продвигал в интернете, и там была utm метка — это реклама пространства NDim была, я рекламировал
> пространство и собирал аналитику по успешности этой рекламы». То есть рекламы **в** продукте не
> было и нет; речь о продвижении самого NDim.
>
> ⚠️ **Правится только здесь:** `src/lib/content/docs.ts` генерируется из этого файла
> (`tools/extract-docs.mjs`), и правка прямо в нём умерла бы при следующей перегенерации.
> ⚠️ И только ВНЕ списка: комментарий внутри списка разрывает его на два блока — проверено.
...
<!-- ✅ ПРИНЯТО ВЛАДЕЛЬЦЕМ: интервью №065, В4 = А (2026-08-29 20:59) — три захода вычитки.
Заход 1 (В1 = В): снято обещание тайны от разработчика системы и перечисление того, чего
аналитика не получает — «мы не обещаем тайны от разработчика системы»; чужую политику не
пересказываем, отправляем читателя к её документам.
Заход 2 (интервью №065, В2 = В): подлежащее — Система, а не автор («Не «автор системы использует», а Система
Пространство NDim использует»).
Заход 3 (В4 = А): последнее предложение о непередаче снято ЦЕЛИКОМ — его слово: «передавать не
передаём и не собираемся, НО МЫ НЕ МОЖЕМ ГАРАНТИРОВАТЬ этого 100 процентов». Обещание, которого
владелец не даёт, в документе не стоит; случай передачи покрыт пунктом 2.2 этой же Политики.
Заход 4 (интервью №065, В5 = А): по прямому слову владельца «за приватность данных и их в
постхог сохранность ндим спейс снимает с себя ответственность и передаёт её на сам постхог»
дописано четвёртое предложение — по форме его же пункта 2.5 про Firebase.
🔴 И его же поправка тем же вечером: «на самом сервисе» → «на самом сервисе PostHog».
Дословно: «всё никак ты не учишься писать полно». Это ВТОРОЙ за день возврат одного класса
(первый — «оценки чего?!» в записи 2.2): предмет назван существительным, отсылки к нему
местоимением или родовым словом в текстах лица продукта не бывает.
