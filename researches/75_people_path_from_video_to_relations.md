...
1. **Канал есть, путь рвётся после него.** Ролик 19–20.09 привёл ≈150 человек за два дня: гостем вошли 10 (≈6,7 %
   — медиана посадочных страниц индустрии 6,6 %), оценки поставили 6, аккаунт завёл 1, через неделю живых снова
...
5. **Зрение пути слепо в двух местах:** у 138 из 147 событий `landing_view` недели ролика браузер не записан;
...
| Неделя ролика 14–20.09 (PostHog, `env=prod`) | `landing_view` 162 · `guest_start` 10 · `rating_saved` 45 у 6 человек · `relations_view` 32 у 13 · `account_created` 1 | запрос по неделям, этот разбор |
...
| `landing_view` без браузера | 138 из 147 | тот же запрос: `$browser` пуст |
