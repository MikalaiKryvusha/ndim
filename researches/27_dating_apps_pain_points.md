...
- ✅ **Явление существует и живо сегодня.** Прямая ценовая дискриминация по полу в приложениях
  ...
  самих компаний.
- ✅ **Pure — подтвердился.** Ровно Ваша формулировка: женщины пользуются бесплатно, мужчины платят.
- ❌ **Badoo — не подтвердился.** Ни одного документа. Только российские блоги и таблоид.
...
— [ru.wikipedia.org](https://ru.wikipedia.org/wiki/Pure_(приложение))
...
При этом **опровергнуть Ваше «БЫЛО так» я тоже не могу**: архивы `web.archive.org` мне недоступны
(см. 5.4.1), а именно там лежали бы прайсы Badoo 2012–2018 годов. Это честная ничья, а не
опровержение. **Но на лендинге Badoo называть нельзя.**
...
|---|---|---|
| «В приложениях знакомств женщинам дают премиум даром, а мужчинам продают» — как утверждение об отрасли в целом | 🔴 **Нельзя** | Ложно для Tinder (проверено приборно, 5.4.8), не подтверждено для Badoo, Mamba и большинства европейских |
| «В Badoo премиум бесплатен женщинам» | 🔴 **Нельзя** | Ноль документов (5.4.5). Первый же читатель попросит ссылку, а её нет |
| «Есть приложения знакомств, где цена подписки прямо зависит от пола — это напечатано в их собственных прайс-листах» | ✅ **Можно** | Подтверждено первоисточниками пяти компаний (5.4.2). Но потребует сноски, а сноска на японскую страницу на лендинге выглядит странно |
...
**Рекомендуемая замена — та же, что в §5.3, и она выстояла проверку.** Говорить только о себе:

> «Здесь нечего купить. Ни женщине, ни мужчине.»

и Ваша собственная уже опубликованная строка из руководства («одинаково непредвзято… невзирая на их
пол…»).
...
1. **Архивные прайсы Badoo 2012–2018** — `web.archive.org` мне недоступен. Ваше «БЫЛО так» ни
   подтвердить, ни опровергнуть я не могу.
2. **Русскоязычные сервисы** (Mamba, Loveplanet, Teamo, Twinby, Fotostrana) — разведка запущена, но
   к моменту записи результат не вернулся. **Раздел требует дополнения.**
3. **Судебная практика ЕС и РФ по «женским скидкам»** (германский AGG, ФАС, Роспотребнадзор,
   Koire v. Metro Car Wash в Калифорнии) — разведка запущена, результат не вернулся.
