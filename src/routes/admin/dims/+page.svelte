<script lang="ts">
   ...
   * мимо нас*». До этого Менеджера измерений завести измерение в 2.0 было НЕЛЬЗЯ ВОВСЕ: админского экрана нет,
   ...
   * на главную, и только `admin` видит Менеджер измерений. Наивный редирект по отсутствию ответа выбросил бы
   ...
   * при открытии менеджера измерений*»). Менеджер измерений построен ради одного — чтобы он быстрее одобрял
   ...
   * и рассуждение было мимо цели Менеджера измерений: пустая очередь это честный ответ «разбирать нечего».
   */
  ...
  /** Подпись поля и подсказка. Тексты Менеджера измерений русские: админское лицо видит только владелец. */
  const t = {
    ...
    <main class="body">
      ...
      <div class="tabs">
        <!--
          ...
          чего он просил Менеджер измерений — «чтобы николай быстрее их одобрял». Прежняя редакция этого
...
{:else}
  <!-- `unknown`: ни Менеджера измерений, ни редиректа, пока прав не знаем. Это и уходит в пререндер. -->
  <div class="hold">
    <Loading {lang} />
  </div>
{/if}
