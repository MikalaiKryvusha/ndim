...
<div class="screen">
  ...
  <main class="body">
    ...
    <!-- Манифест — сниппет, потому что рисуется ВНУТРИ левой колонны (см. ниже), а не
         отдельным столбцом. На телефоне он скрыт (`display: none`), и вместо него работает
         ссылка `.manifest-link` выше — этого поведения правка не касается. -->
    {#snippet manifestCard()}
    ...
    <!-- ── Левая колонка: манифест, вид, поделиться ── -->
    ...
    <!--
      ⚠️ ЛЕВАЯ КОЛОННА — ЭТО ОДИН МАНИФЕСТ, без обёртки-контейнера (слово владельца
      2026-07-31: «Меню нужно привести к двум колонкам», затем «вид и поделиться в меню
      переместить в верх правой колонки»).

      Обёртки здесь нет намеренно: колонна из одной карточки — это и есть карточка. Держать
      вокруг неё пустой `.col` было бы вредно, а не просто лишне: на ТЕЛЕФОНЕ манифест скрыт
      (`display: none`, вместо него работает ссылка `.manifest-link` выше), контейнер остался
      бы нулевой высоты — и всё равно съедал бы зазор мобильной колонки, оставляя дыру
      неизвестного происхождения.
    -->
    {@render manifestCard()}

    <!-- ── Правая колонна: вид и поделиться СВЕРХУ, за ними документы, проект, версии ── -->
...
<style>
  ...
  /* ── Десктоп: макет V2 «Рабочий стол». Манифест — виджет РЯДОМ с кнопками меню
     ...
     ⚠️ ДВЕ КОЛОННЫ, А НЕ ТРИ (слово владельца 2026-07-31: «Меню нужно привести к двум
     колонкам»). Было `repeat(3, 1fr)`: манифест | вид-поделиться | документы-проект-версии —
     три узких столбца, и под коротким средним оставался пустырь. Стало то же, что на
     «Пространстве» и «Профиле»: сетка в ЧЕТЫРЕ ШАГА, колонна — два шага, манифест переехал
     ВНУТРЬ левой колонны (`ideas/24`, модель утверждена владельцем).

     Колонны здесь и раньше были самостоятельными контейнерами (`.col`), поэтому дыр МЕЖДУ
     карточками на этом экране не было; менялось только число столбцов и ширина карточек. */
  @media (min-width: 1024px) {
    ...
    /* Оба больших виджета — по два шага: манифест слева, колонна справа.
       ⚠️ Манифест лежит в сетке НАПРЯМУЮ (обёртки у него нет — см. разметку), поэтому ему
       нужна своя строка: правило для `.col` его не покрывает. Без неё он берёт один шаг и
       сжимается в 276px против 565 — поймано замером сразу после переноса. */
    .body > .col,
    .body > .manifest { grid-column: span 2; }
