<script lang="ts">
   ...
   * 2026-08-01). Переключение — руками владельца и ТОЛЬКО когда страница знает все режимы,
   * включая сброс пароля (фаза 7): переключение действует на письма ВСЕХ флоу разом.
  ...
  onMount(async () => {
    ...
    if (read.info.kind === 'reset-password') {
      // Приёмник сброса приедет вместе с самим паролем (фаза 7). Обещать то, чего нет,
      // нельзя — говорим честно и не оставляем человека в тупике.
      stage = 'not-yet';
      return;
    }
  ...
  const t = {
    ...
    notYetTitle: { ru: 'Этот путь ещё не открыт', en: 'This path is not open yet' },
    notYetBody: {
      ru: 'Сброс пароля в Пространстве NDim пока не работает. Войдите привычным способом — по ссылке на почту или через Google.',
      en: 'Password reset is not available in NDim Space yet. Sign in the usual way — by email link or with Google.',
    },
