...
<style>
   ...
   *
   * 🔴 Обернуть `.head` в кнопку НЕЛЬЗЯ: аватар САМ кнопка (`Avatar.svelte` — портрет во весь
   * экран, `bugs/14`), а кнопка внутри кнопки невалидна, и браузеры чинят такую разметку
   * по-своему. Поэтому путь (а) из документа: растянуть ЭТУ кнопку на всё, что не аватар.
   *
   * По горизонтали `flex: 1` это уже делал. Не хватало ВЫСОТЫ: при `padding: 0` кнопка была
   * ростом со строку текста (~19px) внутри ряда высотой с аватар (46px), и выше или ниже имени
   * палец попадал в пустоту. `align-self: stretch` отдаёт ей всю высоту ряда, а внутренний
   * флекс держит имя по центру — вид не меняется, меняется площадь цели.
   */
  .who {
