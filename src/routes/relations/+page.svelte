<script lang="ts">
  ...
  const t = {
    ...
    // ⚠️ ТЕКСТ ДЛЯ ЧЕЛОВЕКА, А НЕ ДЛЯ РАЗРАБОТЧИКА. Здесь стояло «сервер синхронизации ещё не считал.
    // На стенде: node sync-server/index.mjs --once» — и это увидели живые люди на боевом проде
    // (2026-07-12). Два нарушения канона разом: слово «сервер синхронизации» (в интерфейсе — только
    ...
    // последний раз менял свой NDim ID. Число кладёт сервер синхронизации в саму запись топа —
...
<div class="screen">
  ...
  <main class="body">
    <!-- Шапка экрана — общая, и стоит ВНЕ веток: видна и пока экран грузится (слово владельца
         2026-09-19, `$lib/ui/ScreenHead.svelte`). ПЕРВОЙ, выше плашки гостя: у «Профиля» этой
         плашки нет, и иначе заголовок у гостя прыгал бы между экранами (смоук стейджа). -->
    ...
    {:else}
      {#each data.cards.slice(0, revealed) as card, index (card.entry.guestUid)}
        ...
        <div class="card" in:fly={{ y: 12, duration: MOTION.base, delay: Math.min(index, 8) * 40, easing: cubicOut }}>
          <!-- ЦЕЛЬ РАСКРЫТИЯ — ВЕСЬ ВЕРХ, включая площадь фото (`bugs/104`, решение владельца
               2026-07-31: дом действий над СВОИМ фото — виджет «Профиля», здесь фото чужое).
               Разметка меняется ПО СОСТОЯНИЮ, чтобы кнопка не оказалась в кнопке:
               · свёрнутая — ОДНА кнопка на всю строку, лицо внутри БЕЗ своей кнопки
                 (`peek={false}`) — как `<summary>` 1.x, целью была вся площадь;
               · раскрытая — прежняя пара (bugs/14): лицо открывает портрет во весь экран,
                 имя сворачивает. Портрет чужого человека живёт В РАСКРЫТОЙ карточке —
                 предложение агента из bugs/104 (`PENDING` там же), отменяется одним словом. -->
          <div class="head">
          ...
          {#if expanded === entry.guestUid}
            ...
            <div class="deep" transition:slide={{ duration: MOTION.base }}>
              ...
              <!-- Строки нет вовсе, если сервер синхронизации её не положил (старые боевые relations):
...
<style>
  ...
  /* кружок с лицом живёт в Avatar.svelte (там же лайтбокс, bugs/14); кто здесь кнопка —
     зависит от состояния карточки, см. комментарий у .head в разметке (bugs/104) */
  /*
   * ЦЕЛЬ НАЖАТИЯ — ВЕСЬ ВЕРХ, а не поле имени (`bugs/104`, слово владельца: «чтобы карточку
   * можно было раскрывать тапом, кликом по всему верху, а не только целиться в поле имени»).
   * Это паритет с 1.x, а не улучшение: там карточка была нативной раскрывашкой и аватар жил
   * в `<summary>`, то есть целью была вся его площадь (`researches/12` §«Связи»).
   *
   * История в двух шагах, чтобы следующая сессия не перечитывала git:
   * · путь (а): `flex: 1` + `align-self: stretch` растянули кнопку имени на всё, что НЕ
   *   аватар (замер «до» финального шага: 81,7–89,1 % строки; мёртвой зоной остались фото
   *   и зазор — ~58px ширины);
   * · путь (б-гибрид, финал 2026-08-14): разметка меняется ПО СОСТОЯНИЮ — свёрнутая карточка
   *   несёт ОДНУ кнопку `.who.whole` на всю строку (лицо внутри, без своей кнопки:
   *   `peek={false}`), раскрытая возвращает пару кнопок (портрет · свернуть). Кнопка в
   *   кнопке не возникает НИ В ОДНОМ состоянии — вложенные интерактивы по-прежнему запрещены.
   */
  .who {
  ...
  /* Свёрнутое состояние: та же строка, но кнопкой стал ВЕСЬ верх — лицо внутри неё.
     Зазор 12px повторяет .head, чтобы геометрия не менялась при раскрытии. */
  .who.whole { gap: 12px; }
