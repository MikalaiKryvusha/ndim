<script lang="ts">
  ...
  /*
   * 🔴 ЩИТ ЗАГРУЗКИ НА КОРНЕ — И КТО ЕГО ОПУСКАЕТ (регрессия боя 2026-09-05,
   * `bugs/NEW_root_boot_shield_never_drops`).
   *
   * Инлайн-скрипт `app.html` поднимает щит (`html[data-booting]`, bugs/40) на корне и на лендинге у
   * всякого, у кого в браузере лежит маркер сессии `ndim-session` — то есть у каждого, кто хоть раз
   * входил, гостем тоже. Опускает щит РОВНО ОДНО место — `endBoot()` из `data/session.ts`, и до V5
   * оно звалось лендингом, куда корень уводил всех. V5 сделал корень страницей с содержанием, увод
   * убрал — и на корне щит опускать стало некому: владелец открыл ndimspace.app и увидел вечное
   * «Загрузка выполняется необычно долго». В свежем браузере (без маркера) страница исправна —
   * ровно поэтому ни смоуки, ни e2e этого не увидели.
   *
   * Страница живёт БЕЗ гидратации (`csr = false` в `+page.server.ts`): `onMount` здесь не
   * исполняется никогда, `hasSession()` позвать нечем. Поэтому решение принимает ИНЛАЙН-СКРИПТ
   * ниже, тем же признаком, что и `app.html`: маркер есть → человек уводится внутрь, в `/profile`
   * (bugs/08.1: вошедший и гость лендинг не разглядывают; щит держится до ухода, кадра главной он
   * не видит); маркера нет → щит и не поднимался, главная и есть нужный экран. Маркер, совравший
   * (вышел в другой вкладке, сессия истекла), приводит на экран входа — это та же дверь, куда
   * уводил и лендинг (`APP_URL`), а не вечный экран.
   */
...
(function () {
	try {
		...
		// Маркер сессии — тот же признак, по которому app.html поднял щит: вошедший и гость уходят
		// внутрь, щит держится до ухода (bugs/08.1; регрессия боя 2026-09-05 — разбор в шапке компонента).
...
<main class="door">
  ...
  <!-- БОЛЬШАЯ КНОПКА — дорога в дело ДО аккаунта. Адрес `/profile?guest=…` уже существует и
       ...
       Слово `root` — место входа для аналитики (`ANALYTICS_ENTRIES` в `funnel.ts`, `plans/105` Б2):
       сюда ведёт ссылка из роликов владельца, и число недели должно видеть эту дверь отдельно. -->
  <a class="cta" href="/profile?guest=root">
