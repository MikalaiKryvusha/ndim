<script lang="ts">
  ...
  /*
   * Мост из демо (plans/03 этап 2). На стенде CTA демо ведёт СРАЗУ в гостя — мгновенный анонимный
   * вход без единой формы. На публичном хосте — на /profile, то есть в стену входа.
   *
   * 🔴 ЭТО НЕ ЗАМЫСЕЛ, А ДОЛГ, и он стоит дороже, чем выглядит. Человек только что подвигал
   * звёзды в демо и на пике интереса нажал «Создать своё пространство» — а мы просим у него
   * почту. Это трение `ideas/09` в чистом виде, ради снятия которого гостевой режим и строился.
   * Гостевая дверь `/profile?guest=1` в бою РАБОТАЕТ (bugs/95 воспроизведён и починен в бою) —
   * сюда её не пускает только эта строка.
   *
   * ⚠️ Прежний комментарий здесь утверждал, что на проде кнопка ведёт «в живое 1.x, как весь
   * лендинг». Это перестало быть правдой 12.07.2026, когда 2.0 выкатили, а 1.x умер. Комментарий
   * прожил враньём три недели и успел ввести в заблуждение разведку (01.08.2026). Правится
   * вместе с ответом владельца на интервью №009, В3 — какой из путей станет главным.
   */
