...
import { doorEngineUrl } from '$lib/door/inline';
import type { DimView } from '$lib/content/dim-view';

const BY_SLUG = new Map(DIMS.map((d) => [d.slug, d]));

/*
 * ДВЕРЬ КАРТОЧКИ ВЕДЁТ В ГОСТЯ (`plans/75` Ш1, лечение `bugs/200`) — что здесь появилось и почему.
 *
 * Аудит 2026-08-28 нашёл один точный разрыв конверсии: десять из четырнадцати кликов поиска
 * приходят на карточки каталога, а дверь карточки вела в `/profile` — то есть в стену входа, на
 * пике интереса. Мост «интерактив вносит внутрь» был построен только на лендинге, куда органика
 * не ходит. Владелец выбрал лечение: №049 В1 = А (дверь ведёт сразу в гостя) и №050 = Д3
 * «Панель после касания» с его правками (эталон — `design/catalog-door-mockups.html`).
 *
 * 🔴 СТРАНИЦА ОСТАЛАСЬ БЕЗ ГИДРАТАЦИИ, И ЭТО ГЛАВНОЕ ОГРАНИЧЕНИЕ РЕШЕНИЯ. `csr = false` ниже не
 * тронут: включить его «чтобы заработали звёзды» значило бы вернуть БАНДЛ ФРЕЙМВОРКА на всю
 * SEO-поверхность. (Про `__data.json` здесь стояло второе слагаемое — оно снято как неверное:
 * эти файлы пишутся и при `csr = false`, см. ниже и `bugs/214`. Запрет от этого не ослаб:
 * бандла фреймворка достаточно.) Вместо этого дверь — «ванильный остров»:
 *   · разметка (звёзды и скрытая панель) приезжает ПРЕРЕНДЕРОМ, как остальная страница;
 *   · обработчики вешает инлайн-скрипт (`$lib/door/island.js`); его вес НЕ назван здесь числом
 *     намеренно — замечание З1 вердикта QA №5: копия числа в прозе устаревает молча, и три
 *     копии уже разошлись (проза говорила «~1,5 КБ» и «2 498», замер давал 2 563). Место у
 *     этого числа ОДНО — потолок `ISLAND_CEILING` в `island-core.test.ts`, и он же его стережёт;
 *   · слой данных грузится ОТДЕЛЬНЫМ чанком по первому касанию — `$lib/door/engine.ts`,
 *     собранный `vite.door.config.ts`, и это тот же `saveTestRating`, что в бою с 2026-08-14.
 * Человек, пришедший из поиска почитать про фильм, не платит за это ни байтом.
 *
 * ⛔ Инвариант владельца: «*и карточку не обеднять*». Ни одна секция страницы отсюда не ушла —
 * добавился только низ блока двери. Приёмка Ш4 сверяет секции «до/после» диффом.
 *
 * Адрес движка читается ОДИН раз на процесс сборки: манифест не меняется между страницами, а
 * 10 222 чтения файла — это работа, которой не должно быть.
 */
const ENGINE_URL = doorEngineUrl();
 ...
 *     быстрее, и сильнее всего это заметно на телефоне и слабой связи. 🔑 Это НАСТОЯЩАЯ и
 *     достаточная опора запрета `csr = true`, и она видна прямо в HTML страницы;
 *   · из самого HTML уходит сериализованная нагрузка гидратации.
 *
 * 🔴 ЧЕГО ЭТО НЕ ДАЁТ, вопреки прежней редакции этой врезки (`bugs/214`, решение Менеджера
 * 2026-08-29, путь 2): **`__data.json` НЕ ИСЧЕЗАЮТ.** Здесь стояло «исчезают 10 222 файла
 * `__data.json` — чистый мёртвый вес»; сборка это опровергает. Замер собственной сборкой
 * 2026-08-29 (SvelteKit 2.69.2 · Vite 8.1.4): под `dimension/` — **10 250 файлов, 51,6 МБ**,
 * и они уезжают в бой. SvelteKit пишет их всегда, когда у маршрута есть `load`, независимо
 * от `csr`.
 * И «мёртвым весом» они тоже не являются: это вероятная опора клиентских переходов ИЗ
 * приложения В каталог — человек на csr-экране кликает ссылку на карточку, и клиентский
 * роутер тянет ровно эти файлы. Человек из поиска их не качает.
 *
...
export function load({ params }: { params: { lang: string; slug: string } }): DimView {
  ...
  // 41 МБ в каждом релизе. Сегодняшний замер своей сборкой (2026-08-29): 10 250 файлов, 51,6 МБ —
  // урезание полей помогло, но корпус вырос. ⚠️ «Эти файлы никто не запрашивает» здесь стояло
  // и снято как недоказанное (`bugs/214`): человек из ПОИСКА их не качает, но клиентский переход
  // ИЗ приложения на карточку тянет ровно их. Почему их нельзя просто удалить — в `dim-view.ts`.
  return {
    lang,
    slug: dim.slug,
    // Идентификатор документа каталога — по нему дверь пишет оценку (`plans/75` Ш1). Из слага
    // он не выводится: слаг несёт лишь укороченный хвост идентификатора в нижнем регистре.
    id: dim.id,
    title,
    description,
    ...
    // Адрес движка двери — читается из манифеста второй сборки ОДИН раз на процесс (константа
    // ниже), а не на каждую из 10 222 страниц. Хеш в имени — класс `bugs/124`.
