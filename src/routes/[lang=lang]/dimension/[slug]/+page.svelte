  ...
  <section class="dossier">
    ...
    <dl>
      ...
      <!--
        ...
        но отрисовку сняли, увидев её на собранных страницах. Числа, которые это решили:

          · «Гарри Поттер» — 8,4 при 14 голосах, МАКСИМУМ по всему каталогу — получает
            534-е место из 981; «Дом Дракона» с ОДНИМ голосом стоит 26-м;
          · выше «Гарри Поттера» — 533 фильма, из них 245 (46 %) держатся на ОДНОМ голосе.

        Арифметика при этом исправна и утверждена владельцем (№030 В2 = Д): взвешенная
        (1·10 + 4·8,2405)/5 = 8,592 против (14·8,4 + 4·8,2405)/18 = 8,365. Порядок верен, а
        ЧИСЛО МЕСТА в середине каталога — шум: при потолке в 14 голосов оно говорит о разрежённости
        данных, а не об объекте. Строка «534-е среди 981» рядом с «8,4 · оценено 14 людьми»
        подрывала бы NDSR — то самое, что фаза 3 обязана укреплять.

        ⏭ Развилка владельцу (не блокирует фазу): показывать место ТОЛЬКО в голове вида, где оно
        уже не шум (в топ-50 фильмов средн. 5,14 голоса и НИ ОДНОГО объекта с одним голосом), —
        или не показывать вовсе. Показ «только когда лестно» — продуктовое решение, а не моё.
      -->
      {#if data.tags.length}
        <dt>{t.tags}</dt>
