...
<script lang="ts">
  ...
  const T = {
    ru: {
       ...
       *
       * 🔄 ЗДЕСЬ СТОЯЛО «состояний ДВА, а не три, и это осознанно» — ОТМЕНЕНО 2026-08-21, состояний
       * ТРИ. Прежний довод был такой: канон различает «не оценивал» и «оценил на ноль», но для
       * ДВЕРИ они одинаковы, а отдельная ветка ради двух объектов из 5111 — лишняя сущность.
       * Довод не выдержал кадра: при `rating = 0` человек видит «0,0» и десять ПУСТЫХ звёзд, то
       * есть картинку неоценённого объекта, и текст «есть оценка» ему этого не объясняет. Оккам
       * запрещает лишние сущности, а не необходимые различения; состояние, которое существует и
       * молчит о себе, читается как поломка. Третья ветка — `T.ru.doorAllZero` ниже.
       *
      ...
      /**
       * ТРЕТЬЕ СОСТОЯНИЕ — «ВСЕ ПОСТАВИЛИ НОЛЬ» (`plans/56` шаг 1).
       *
       * 🔴 ЗАЧЕМ ОНО ВООБЩЕ ЕСТЬ. Прежде дверь знала два состояния и на этих страницах говорила
       * текст «есть оценка». Человек видел «0,0», десять пустых звёзд и «оценено 1 человеком» —
       * и ничем не мог отличить «оценили на ноль» от «не оценивали вовсе». Ноль на шкале 0…10 —
       * законная оценка, а не пустота; страница, которая этого не говорит, читается как поломка.
       * В бою таких объектов два (`quadrobics-bdyaergq`, `serious-sam-vr-the-last-hope-otgjmpyx`).
       *
       * 🔑 ПОЧЕМУ ТЕКСТ ИМЕННО ТАКОЙ. Первые три предложения — ДОСЛОВНО текст владельца
       * (интервью №036, В3 = C), они общие у всех трёх состояний и не тронуты ни на знак. Своё
       * у каждого состояния ровно одно, ПОСЛЕДНЕЕ предложение — так устроил сам владелец:
       * «Ваша оценка будет первой!» у пустых, «Ваш голос отображается в рейтинге…» у оценённых.
       * Третье состояние встроено в его же форму, и агентом написано одно предложение, а не блок.
       *
       * ⚠️ УТВЕРЖДЕНИЕ ПРОВЕРЯЕМО, А НЕ ПРИДУМАНО: шкала неотрицательна, поэтому средняя РОВНО
       * ноль возможна только тогда, когда ноль поставили ВСЕ оценившие. Ни одного факта об
       * объекте, которого нет в данных каталога, здесь не появляется (STOP-ASK `plans/56`).
       *
       * ⏳ [AI] Предложение «Все, кто оценил этот объект…» написано агентом и ЖДЁТ ВЫЧИТКИ
       * ВЛАДЕЛЬЦА — как и английская половина этого файла (условие №023 В2 = А). Пометку
       * снимает только его слово; агент со своего текста её не снимает. [/AI]
       */
      doorAllZero: (t: string) =>
        `NDim Space Rating отображает симпатии пользователей Пространства NDim Space. ` +
        `Оцените «${t}» и ещё несколько вещей, которые Вы любите, ` +
        `и Пространство NDim Space найдёт Вам людей, которые думают так же, как и Вы. ` +
        `Все, кто оценил этот объект, поставили ноль баллов, и NDim Space Rating показывает ноль.`,
      /**
    ...
    en: {
      ...
      /**
       * Зеркало третьего состояния (разбор — в русской половине). Отличается тем же одним
       * последним предложением, и оно называет ФАКТ из данных, а не оценку этого факта.
       * ⏳ [AI] Написано агентом, ждёт вычитки владельца вместе со всей английской обвязкой. [/AI]
       */
      doorAllZero: (t: string) =>
        `NDim Space Rating reflects what the people of NDim Space like. ` +
        `Rate “${t}” and a few more things you love, ` +
        `and NDim Space will find you people who think the way you do. ` +
        `Everyone who rated this object gave it a zero, and NDim Space Rating shows zero.`,
      place: 'Rank',
  ...
  /**
   * Текст двери по состоянию величины — ОДНА таблица соответствия, а не цепочка тернарников
   * в разметке. Состояния три (`RatingState`), и каждое названо своим именем: пропущенное
   * состояние здесь видно глазом, а в тернарнике оно выглядит как «иначе».
   */
  const doorText = $derived((title: string) =>
    data.ratingState === 'none'
      ? t.doorNoVotes(title)
      : data.ratingState === 'all-zero'
        ? t.doorAllZero(title)
        : t.doorVoted(title),
  );
...
<article class="dim">
  ...
  <section class="door">
    <h2>{t.doorTitle}</h2>
    <!-- ТРИ СОСТОЯНИЯ, И НИ ОДНО НЕ ПРИТВОРЯЕТСЯ ДРУГИМ (`plans/56` шаг 1). Состояние приезжает
         готовым полем `ratingState` — считает его `dims-rating.ts`, то есть тот же модуль, что
         решает про звёзды строкой выше. Своё сравнение `rates` с порогом здесь запрещено:
         правило показа оценок одно на весь проект, и его вторую копию ловит
         `dims-rating.test.ts`. -->
    <p>{doorText(data.title)}</p>
