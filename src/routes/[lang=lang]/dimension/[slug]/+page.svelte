...
<script lang="ts">
  ...
  const T = {
    ru: {
       ...
       *
       * 🔑 Состояний ДВА, а не три, и это осознанно. Канон различает «не оценивал» и «оценил на
       * ноль», но для ДВЕРИ они одинаковы: NDSR существует в обоих случаях, когда голос есть.
       * Отдельная ветка ради двух объектов из 5111 — это лишняя сущность (Оккам), а текст ниже
       * честен при `rating = 0`: он говорит, из чего величина сложена, и нигде не утверждает,
       * что объект кому-то понравился.
       *
