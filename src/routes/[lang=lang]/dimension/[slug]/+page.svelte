...
<article class="dim">
  ...
  <section class="dossier">
    ...
    <dl>
      ...
      -->
      <!--
        🔴 МЕСТО В СВОЁМ ВИДЕ НЕ ПОКАЗЫВАЕТСЯ — снято ЗАМЕРОМ 2026-08-16, `plans/56` шаг 2.

        Механизм построен и покрыт тестами (`placesIn`, `placeOf`, поле `place` доезжает сюда),
        но отрисовку сняли, увидев её на собранных страницах.

        🔄 ЧИСЛА ПЕРЕСНЯТЫ 2026-08-16 ПОСЛЕ ГИПЕРБОЛЫ (интервью №036, В2 = B) — прежняя редакция
        этого комментария («534-е место», «выше 533 фильма, из них 245 на одном голосе») описывала
        каталог ДО решения владельца и сегодня неправда. Его правка сработала: одиноких голосов
        выше «Гарри Поттера» больше НЕТ ни одного.

        Что осталось и почему строки всё ещё нет (прибор `tools/measure-catalog-place.mjs`):

          · «Гарри Поттер» — 8,4 при 14 голосах, МАКСИМУМ по всему каталогу — стоит 151-м из 981;
            выше него 150 фильмов, и НИ ОДИН из них не держится на единственном голосе;
          · 96 % оценённых фильмов (937 из 981) стоят в группе объектов с ОДИНАКОВЫМИ числами:
            191 фильм «10,0 × 1» занимает места с 242-го по 432-е, и порядок внутри группы
            разводит слаг, то есть алфавит адреса;
          · один голос «5» уносит фильм со 100-го места на 508-е.

        То есть ПОРЯДОК верен (он утверждён владельцем, №030 В2 = Д), а ТОЧНОСТЬ ДО ЕДИНИЦЫ, на
        которую претендует слово «место», данными не обеспечена. Строка «151-е среди 981» рядом с
        «8,4 · оценено 14 людьми» подрывала бы NDSR — то самое, что фаза 3 обязана укреплять.

        ⏭ Развилка владельцу (не блокирует фазу): четыре варианта нарисованы страницей макетов
        `design/catalog-place-mockups.html` по его прямой просьбе «нарисуй макеты с визуализацией
        проблемы» — место всем · только голова списка · полка вместо номера · число голосов вместо
        места. Выбор за ним: что показывать на витрине — продуктовое решение, а не моё.
      -->
      {#if data.tags.length}
        <dt>{t.tags}</dt>
