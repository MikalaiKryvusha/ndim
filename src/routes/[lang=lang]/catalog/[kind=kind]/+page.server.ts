 ...
 * на страницах объектов, — **уходит бандл фреймворка**, и страница появляется быстрее на
 * телефоне и слабой связи. Тема при этом жива: её ставит инлайн-скрипт `app.html`.
 *
 * ⚠️ Про `__data.json` здесь стояло «исчезают лишние» — снято как неверное (`bugs/214`,
 * решение Менеджера 2026-08-29): `csr = false` их НЕ убирает, SvelteKit пишет их всегда, когда
 * у маршрута есть `load`. Замер своей сборкой 2026-08-29: под `catalog/` они тоже лежат
 * (10 438 файлов во всей сборке против 10 250 под `dimension/`). Запрет `csr = true` от этого
 * не ослаб — его настоящая опора, страница без бандла фреймворка, жива и главная.
 */
