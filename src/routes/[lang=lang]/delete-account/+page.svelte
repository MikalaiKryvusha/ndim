  ...
  onMount(async () => {
    try {
      ...
      if (isLoginLink() && !isReauthLink()) {
        ...
        /*
         * 🔴 БЛИЗНЕЦ `bugs/233`, НАЙДЕННЫЙ КОМПИЛЯТОРОМ, А НЕ ГЛАЗАМИ.
         *
         * Эта дверь наследовала ту же болезнь, что «Профиль»: человек, открывший письмо не в
         * том браузере, где начинал, получал «срок действия ссылки истёк» — при живой ссылке.
         * Здесь цена выше обычного: страница публичная, её адрес человек получает от Google
         * Play, и приложения у него может уже не быть — второго пути внутрь нет.
         *
         * `email-needed` тут молчит СОЗНАТЕЛЬНО: у двери на экране уже стоит поле почты и
         * кнопка «получить ссылку» (`signStep`), то есть человек и так видит, что делать.
         * Ложная красная строка про мёртвую ссылку — единственное, что ему мешало.
         * ⏳ Полный исход (завершить вход прямо здесь введённой почтой) приедет вместе с
         * экраном вопроса, который утверждает владелец, — `completeLoginLink` уже принимает
         * почту вторым доводом.
         */
        if (!entered.ok && entered.reason !== 'email-needed') {
          signError = accountErrorText(asAccountFailure(entered.reason), lang);
        }
      }
