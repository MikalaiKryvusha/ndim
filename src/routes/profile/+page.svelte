<script lang="ts">
  ...
  const t = {
    ...
    /*
     * ВРЕЗКА ПОДТВЕРЖДЕНИЯ ВЫХОДА — слово владельца дословно (2026-09-05): «Выйти? Вы
     * пользуетесь временным гостевым аккаунтом, в который будет невозможно войти после выхода.
     * Ваши оценки исчезнут навсегда.»
     *
     * Это ЕГО ЖЕ правка ЕГО ЖЕ прежней короткой фразы (интервью №007, В3=А: «Выйти? Ваши оценки
     * исчезнут»). Короткая называла ПОСЛЕДСТВИЕ, но не называла ПРИЧИНУ: человек не видел, что
     * дверь временного гостевого аккаунта закрывается за ним и обратно в неё не войти.
     *
     * ⚠️ СЛОВО «НАВСЕГДА» ЗДЕСЬ СТОИТ ПО ЕГО СЛОВУ. Общий запрет этого слова в текстах продукта
     * (правила тона, шапка `design/login-mockups.html`) НЕ отменён — владелец снял его ровно
     * на этой строке, и здесь оно правда: гостевую сессию вернуть нельзя. EN — рабочий перевод
     * агента до его вычитки.
     */
    acctLeaveConfirm: {
      ru: 'Выйти? Вы пользуетесь временным гостевым аккаунтом, в который будет невозможно войти после выхода. Ваши оценки исчезнут навсегда.',
      en: 'Sign out? You are using a temporary guest account that you will not be able to sign back into after signing out. Your ratings will be gone for good.',
    },
    ...
    expired: {
      ...
      signinSub: {
        // Имя на двери — целиком: так его пишет сам владелец в дверных строках («Войти в
        // Пространство NDim Space гостем», 2026-09-05). В прозе он же пишет «Пространство NDim»,
        // и там правило не действует.
        ru: 'Если Вы уже создавали аккаунт в Пространстве NDim Space',
    ...
    account: {
      ...
      // «Пароль не нужен.» СНЯТО 2026-09-05 — оправдание за то, чего продукт не требует
      // (запрет З1 портрета). Паролей в 2.0 нет вовсе, объявлять их отсутствие незачем: это
      // видно по форме. Нашёл страж текста лица в тот час, когда его наконец поставили в ворота.
      signinLead: {
        ru: 'Введите почту Вашего аккаунта — мы пришлём ссылку для входа.',
        en: 'Enter the email of your account — we will send a sign-in link.',
      },
      errors: {
        'already-in-use': {
