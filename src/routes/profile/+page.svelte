...
<style>
  ...
  @media (min-width: 1024px) {
    ...
    .body {
      ...
      display: grid;
      /*
       * ⚠️ СЕТКА С ШАГОМ (`ideas/24`) ЗДЕСЬ СОЗНАТЕЛЬНО НЕ ПРИМЕНЕНА — вывод ЗАМЕРА,
       * а не недоделка. Проверено 2026-07-31 (`tools/measure-ideas24.mjs`):
       *
       *   · колонны-контейнеры, как на «Пространстве», делают экран ВЫШЕ. Там один
       *     виджет-великан («Сервер») ломал строки, и колонны дали 26.8% → 22.3% пустоты.
       *     Здесь виджеты удачно спарены по высоте (346/303 и 247/192), дыра всего 98px, а
       *     лучшее разбиение с СОХРАНЕНИЕМ порядка (иначе изменится телефон) даёт 832px
       *     против нынешних 776;
       *   · равные шаги вместо `1.15fr / 1fr` при той же высоте экрана УХУДШИЛИ плотность:
       *     пустоты 16% → 17.4%, дыра 98 → 114px. Выигрыша для человека — ноль.
       *
       * Пропорции 1.15/1 сняты не с потолка и проверены боем, а менять их ради единого
       * словаря значило бы платить видом за внутреннюю стройность. Когда фаза хранения
       * персональных раскладок дойдёт до этого экрана, она объявит СВОЙ шаг — число шагов
       * это свойство экрана, а не проекта.
       */
