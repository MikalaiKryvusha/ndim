<script lang="ts">
  ...
  async function finishEmailLink(): Promise<string | null> {
     ...
     * экран объявлял человека гостем раньше, чем узнавал, что происходит. Владелец поймал это
     * в бою: он открыл письмо в браузере, где давно вошёл, и увидел «Сейчас Вы гость» и
     * «Ссылка больше не действует» ПОВЕРХ собственных данных. Сессия при этом оставалась его —
     * врал ровно этот кусок, а не Firebase.
     *
    ...
    /*
     * 🔴 «МЫ НЕ ЗНАЕМ, ЧЬЯ ЭТО ССЫЛКА» — НЕ ОТКАЗ, И ГОСТЯ ИЗ ЧЕЛОВЕКА ЗДЕСЬ НЕ ДЕЛАЮТ
     * (`bugs/233`, воспроизведено на стенде).
     *
     * Прежний хвост этой функции заводил `signInGuest()` всякому, у кого ссылка не прошла. Для
     * настоящей протухшей ссылки это спорно, а для случая «письмо открыто в другом браузере»
    ...
    if (result.reason === 'email-needed') {
      ...
      return null; // сессии нет — дверь входа (`stand = 'signedout'`), а не новый гость
    }
     ...
     */
    /*
     * 🔴 И ТО ЖЕ САМОЕ, КОГДА СЕССИИ НЕ БЫЛО ВОВСЕ (`bugs/NEW_dead_link_in_clean_browser_makes_guest.md`).
     * Здесь стоял `signInGuest()`: человек в чистом браузере с использованной ссылкой читал «Ссылка
     * больше не действует» и одновременно «Сейчас Вы гость». До №084 в чистом браузере сюда не
     * доходили (адреса не было, исход `email-needed`); адрес в ссылке сделал отказ настоящим и
     * разбудил этот хвост. Довод тот же, что строкой выше: он шёл ВХОДИТЬ (`bugs/233` К1).
     * [TESTED: 2026-09-13 · стенд, кейс СЛ-06 `verify-signin-link-any-browser`: до правки — гость, после — дверь входа]
     */
    return null;
  }
  ...
  const t = {
    ...
    guest: {
       ...
       * Подтверждено им повторно 2026-09-09, дословно: «*это фрод. Тысячу раз обсуждали*».
       *
       ...
       * 🔒 Стережёт `src/routes/profile/guest-facts.test.ts`.
       */
      fact2: {
       ...
       */
      /*
       * 🔴 ЛЕЧЕНИЕ ФРОДА, НАЙДЕННОГО ВЛАДЕЛЬЦЕМ 2026-09-09. Прежняя строка — «Если Вы не
       * вернётесь в течение 7 дней, мы удалим эти данные без следа» — врала ДВАЖДЫ:
       * возвращение на срок не влияет вовсе, и умалчивалось единственное действие, которое
       * спасает труд человека, — полноценный аккаунт.
       *
       * Правда по коду: `sync-server/index.mjs` — анонимный аккаунт живёт `GUEST_TTL_DAYS` ОТ
       * РОЖДЕНИЯ (№010 Р3); кандидат на уборку = анонимный провайдер И возраст больше срока,
       * неанонимная учётка не кандидат ни при каком возрасте.
       *
       * 🔑 ТЕКСТ НАПИСАН ВЛАДЕЛЬЦЕМ, а не агентом. Его слова: «*Мы смотрим не на факт
       * вернется-не вернётся, а просто на 7 дней от даты создания анонимного аккаунта*».
       * Формулировка взята дословно из утверждённого им набора макетов V5
       * (`design/guest-first-screen-mockups.html`) по его же указанию в интервью №080 В1 = Г:
       * «*мы уже сегодня этот текст писали в другом месте. Ищи*». Показывать сразу — В2 = А.
       * ⛔ Агент этот текст не переписывает и не сокращает.
       *
       * Число дней приходит из `GUEST_TTL_DAYS` — литерал здесь запрещён и стережётся
       * `guest-facts.test.ts` (класс `bugs/201`: текст обещал 30 дней, продукт стирал на 7).
       */
      fact3: {
        ru: `Гостевой аккаунт живёт ${GUEST_TTL_DAYS} дней от дня создания. Гостевой аккаунт, не ставший полноценным за эти ${GUEST_TTL_DAYS} дней, удаляется вместе со всеми оценками и связями. Превратите Ваш гостевой аккаунт в полноценный для сохранения Ваших данных на длительный срок.`,
        en: `A guest account lives ${GUEST_TTL_DAYS} days from the day it is created. A guest account that has not become a full account within these ${GUEST_TTL_DAYS} days is deleted together with every rating and relation. Turn your guest account into a full one to keep your data for the long run.`,
      },
      ...
      /*
       * 🔴 ОТСЮДА УБРАНА ВТОРАЯ КОПИЯ ОТМЕНЁННОГО ПРАВИЛА (2026-09-09). Строка гласила:
       * «Пока Вы гость, Вас не видит никто, поэтому открывать свойство некому. Настройки
       * аудитории появятся после создания аккаунта».
       *
       * 🔑 Найдена НЕ глазами, а стражем отменённого правила ниже по файлу
       * (`guest-facts.test.ts`) в первый же его прогон. Владелец нашёл первую копию сам,
       * в карточке гостя; эта пряталась в редакторе аудитории, куда заходят реже. Класс —
       * «копия класса пережила чистку класса» (`bugs/156`), тот же, что у срока гостя.
       *
       * Убрана только ПРИЧИННАЯ половина — та, что повторяет отменённое правило. Вторая
       * половина текста осталась дословно: она констатирует факт продукта (аудитория
       * появляется с аккаунтом) и от правила видимости не зависит вовсе.
       */
      audienceLocked: {
...
<div class="screen">
  ...
  <main class="body" class:appearing>
    ...
    <!--
      ШАПКА ЭКРАНА — общая с остальными экранами (слово владельца 2026-09-19: «На профиле нет
      заголовка "Профиль"… пока страницы грузятся… везде должна быть»). Стоит ВЫШЕ цепочки
      состояний — видна и пока экран грузится. Подсказка — канон «Дома» 1.x (bugs/43): про
      управление учётной записью в ней не обещаем (EXP-0036).
      Двух веток шапка не касается: предпросмотр «Как меня видят» — отдельный вид со своей
      строкой «Назад» (`bugs/105`), а карточка истёкшей гостевой сессии несёт свой заголовок, и
      подсказка «Это Ваша домашняя страница…» над ней спорила бы с ним.
    -->
    {#if !(seeMeOpen && data) && stand !== 'expired'}
      <ScreenHead title={t.title[lang]} help={t.homeIntro[lang]} />
    {/if}
    {#if seeMeOpen && data}
          ...
          {:else}
            ...
            {#if signupStep === 'facts'}
              <ul class="guest-facts">
                <!-- ⛔ Строки о видимости здесь НЕТ и не будет: `fact1` убран словом владельца
                     (№069 В2 = А). Разбор — в объявлении текстов выше и в `bugs/227`. -->
...
<style>
  ...
  @media (min-width: 1024px) {
    ...
    /* Во всю ширину — то, что не делится на колонки: шапка экрана (заголовок и вводная
       подсказка, общий компонент), гостевая карточка, состояния стенда. Карточка-шапка отсюда
       УШЛА: она наполнилась управлением аккаунтом и стала обычным виджетом колонны. */
    .body > .guest-card,
