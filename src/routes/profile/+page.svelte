<script lang="ts">
  ...
  /**
   * ВЫВЕСТИ УХОДЯЩИЙ ПРЕДПРОСМОТР ИЗ ПОТОКА (`bugs/117`).
   *
   * Слово владельца: «контент в профиле на мгновение сдвинут вниз, словно отступая место под
   * содержимое „Как меня видят“, а затем быстро перемещается вверх».
   *
   * Замер (`tools/measure-bug117.mjs`, 2026-08-15) назвал причину числами: при закрытии ветка
   * профиля монтируется В ТОТ ЖЕ КАДР, а угасающий блок предпросмотра ещё 240 мс (`MOTION.base`)
   * остаётся В ПОТОКЕ рабочей области — и все эти кадры толкает профиль вниз ровно на свою
   * высоту: **15 кадров подряд, сдвиг +453px на 390 и +439px на 1440, прокрутка окна 0**.
   * Прокрутка в дефекте не участвует вовсе — подозрение на память вида (`plans/08`) снято тем же
   * замером: `afterNavigate` отсекает возврат с ТОГО ЖЕ пути (`view-memory.ts:119`), и `scrollY`
   * все 60 кадров равен нулю.
   *
   * Лечение из ДВУХ частей, и обе обязательны — каждая убирает свой класс некрасивого:
   *
   *   1) **Место в потоке освобождается сразу.** Уходящий блок пришпиливается туда, где стоял
   *      (`position: fixed` в координатах вьюпорта), и профиль встаёт на своё место первым же
   *      кадром. Это убирает скачок.
   *   2) **Уходящий слой ПЛОТНЫЙ и уезжает вправо** (`slideOut` ниже) вместо угасания на месте.
   *      Кроссфейд скачок тоже убирал, но платил другим уродством, и прибор его снял: на
   *      середине перехода два полупрозрачных текста просвечивают друг сквозь друга. Плотную
   *      подложку кроссфейду не поставить в принципе — `fade` гасит `opacity` ВСЕГО элемента
   *      вместе с фоном. Поэтому прозрачность не трогается вовсе, а слой уходит движением —
   *      ровно так закрывается подэкран по жесту «назад» в мобильных ОС.
   *
   * Прокрутка в дефекте не участвует вовсе — подозрение `plans/49` на память вида (`plans/08`)
   * снято тем же замером: `afterNavigate` отсекает возврат с ТОГО ЖЕ пути (`view-memory.ts:119`),
   * и `scrollY` все 60 кадров равен нулю.
   *
   * Почему `fixed`, а не `absolute`: уезжающий вправо блок в потоке документа расширил бы
   * область прокрутки и родил горизонтальную полосу — то есть новый дефект вместо старого.
   * Фиксированные элементы в прокрутку страницы не входят. ⚠️ Это НЕ возврат лайтбоксового
   * набора, запрещённого `bugs/105`: там `fixed; inset: 0` был ПОСТОЯННЫМ состоянием открытого
   * предпросмотра и закрашивал оболочку; здесь он живёт 240 мс на уже закрытом блоке, а
   * открытый предпросмотр как был, так и остаётся секцией в потоке рабочей области.
   * Почему координаты СНИМАЮТСЯ, а не пишутся константой: на телефоне `.body` — колонна flex,
   * от 1024px — виджетная сетка (`ideas/24`), и место блока в них разное.
   */
  function pinLeaving(event: Event): void {
    ...
    // Плотная подложка: сквозь уезжающий слой не должно просвечивать содержимое «Профиля».
    // Здесь она работает — в отличие от кроссфейда, прозрачность мы не анимируем.
    el.style.background = 'var(--bg)';
    // Мягкая тень по левому краю: уезжающий слой читается как лист ПОВЕРХ «Профиля», а не как
    // обрыв фона. Без неё кадры середины перехода выглядят разрезом экрана по вертикали.
    el.style.boxShadow = '-10px 0 26px rgba(0, 0, 0, 0.14)';
    // Уходящий блок больше не принимает палец: 240 мс он лежит поверх живого профиля.
    el.style.pointerEvents = 'none';
  }

  /**
   * Уход предпросмотра — движением вправо, БЕЗ изменения прозрачности (`bugs/117`, часть 2).
   *
   * Своя функция, а не `out:fly={{ x: … }}` в разметке, ровно по одной причине: уехать блок
   * обязан за СВОЮ ширину, а она разная на телефоне и в виджетной сетке — и знать её можно
   * только у живого узла. `opacity: 1` в параметрах обязателен: у штатного `fly` конечная
   * прозрачность нулевая, и он вернул бы то самое просвечивание.
   */
  function slideOut(node: Element, params: { duration: number }) {
    const width = node.getBoundingClientRect().width;
    return fly(node, { x: width + 24, opacity: 1, duration: params.duration });
  }
...
<div class="screen">
  ...
  <main class="body">
    ...
    {#if seeMeOpen && data}
      <!-- Приход — угасанием, уход — движением вправо плотным слоем (`bugs/117`). `onoutrostart`
           та точка, где блок перестаёт занимать место в потоке: см. `pinLeaving` и `slideOut`. -->
      <div
        class="seeme"
        in:fade={{ duration: MOTION.base }}
        out:slideOut={{ duration: MOTION.base }}
        onoutrostart={pinLeaving}
      >
