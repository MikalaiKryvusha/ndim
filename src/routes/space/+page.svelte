<script lang="ts">
  ...
  //
  // Все цифры пишет сервер синхронизации (sync-server/); экран только читает и показывает.
  ...
  const t = {
    ...
    // ⚠️ Здесь тоже висел хвост для разработчика — «На стенде: node sync-server/index.mjs --once».
    // Команды на лице продукта не место (см. relations/+page.svelte).
    /*
     * ПУСТОЕ СОСТОЯНИЕ — слово владельца (интервью №061 В3 → В6 = А, 2026-09-05). Прежняя строка
     * («…ещё ни разу не считал — показывать пока нечего») нарушала его правило о лице продукта
     * дважды: оправдание «нечего» и рассказ о состоянии нашего механизма вместо объяснения
     * экрана (`bugs/211`). Он велел писать «как в хелп-карточке старого NDim»; в архиве 1.x
     * карточки про этот экран нет, поэтому текст собран из его же карточки `my_ndimid_is_empty`
     * (строй: что это · зачем · что делать) и принят им черновиком дословно.
     */
    empty: {
      ru: 'Здесь Пространство NDim показывает свои числа: сколько в нём людей, измерений и оценок. Числа появятся после ближайшего счёта Пространства. Чем больше измерений в Вашем NDim ID, тем точнее Пространство ищет похожих на Вас людей.',
      en: 'Here NDim Space shows its numbers: how many people, dimensions and ratings it holds. The numbers appear after the nearest count of the Space. The more dimensions your NDim ID has, the more precisely the Space searches for people similar to you.',
    },
...
<div class="screen">
  ...
  <GuestCard />
    ...
    {:else}
      ...
      <!--
        КОЛОННА 1 из двух (`ideas/24`, макет V4 утверждён владельцем 2026-07-31).
        Колонна — контейнер шириной в ДВА ШАГА сетки; виджеты внутри идут стопкой и никого
        не ждут. Ровно поэтому исчезают дыры: строк, чьи высоты пришлось бы синхронизировать,
        больше не существует (замер: пустоты 37.4% → 10.4% на 1440).
        ⚠️ На телефоне `.col` объявлена `display: contents` — обёртка исчезает, и порядок
        виджетов остаётся ТОТ ЖЕ, что был. Мобильный экран не меняется ни на пиксель.
      -->
      <div class="col">
      ...
      <!-- КОЛОННА 2: «Сервер синхронизации» — самый длинный виджет, держит колонну один. -->
      <div class="col">
...
<style>
  ...
  /* Колонны виджетной сетки (`ideas/24`) на телефоне не существуют: обёртка растворяется,
     и виджеты остаются прямыми детьми `.body`, как были до правки. */
  ...
  @media (min-width: 1024px) {
    ...
    .body {
       ...
       * Слова владельца: «есть шаг виджета. есть малые виджены на один шаг, и большие на
       * два шага» · «не нужно исскуственно их растягивать».
       *
       * ЧЕТЫРЕ ШАГА по горизонтали. Малый виджет (плитка метрики) — 1 шаг, большой
       * (содержательная карточка) — 2. Прежние `1.35fr / 1fr` заменены равными шагами:
       * доля колонки не является единицей, а шаг — является, и именно им будет записана
       * персональная раскладка в профиле человека, когда появится ручное перетаскивание.
       *
       * ШАГ ЗАДАЁТ ТОЛЬКО ГОРИЗОНТ. По вертикали пакуется сама колонна, карточка сохраняет
       * естественную высоту — `align-items: start` остаётся на месте. Растягивать карточку
       * под клетку сетки владелец запретил прямо; замер это подтверждает: растяжка не
       * убирает пустоту, а переносит её внутрь карточки.
       *
       * Почему это вообще убирает дыры: строк, чьи высоты надо синхронизировать, больше нет —
       * виджеты живут внутри колонн, а колонны независимы. Замер (tools/measure-ideas24.mjs):
       * 1440 — 966px и 37.4% пустоты → 710px и 10.4%; 1024 — 1000px и 38.1% → 744px и 15.6%.
       */
    ...
    /* Плашка гостя — во всю строку (`bugs/226`). `:global` — элемент чужой, из
       ...
       строка над панелью, и одинаковой она обязана быть на всех пяти экранах. */
    .body > :global(.gnote) { grid-column: 1 / -1; }
    /* Плитки перестают быть одним блоком и становятся ЧЕТЫРЬМЯ самостоятельными виджетами
       по одному шагу — это следствие модели шага, а не косметика. Обёртка растворяется,
       поэтому разметку менять не пришлось. */
    .tiles { display: contents; }
    .tiles > .card { grid-column: span 1; }
    /* Колонна — БОЛЬШОЙ размер, два шага. Внутри стопка виджетов своей высоты. */
    .col { grid-column: span 2; display: flex; flex-direction: column; gap: 12px; min-width: 0; }
  }
</style>
