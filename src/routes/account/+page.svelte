<script lang="ts">
   ...
   *
   * СОСТАВ ЭКРАНА ОКОНЧАТЕЛЕН: «Мой аккаунт» (фаза 4) · «Поменять Email» (фаза 5) ·
   * «Удаление аккаунта» (фаза 8). Виджета «Пароль» не будет никогда — пароли отменены
   * решением владельца 2026-08-01 («убираем пароли полностью»), `bugs/32` закрыт.
   *
  ...
  /** Кого удаляем. Держим отдельно: после `deleteUser` сессии не станет, а отчитаться надо. */
    ...
    try {
      ...
      if (isReauthLink()) {
        ...
        if (!confirmed.ok) {
          forgetPendingOp();
          fail(confirmed.reason);
        } else if (waiting?.op === 'change-email') {
          await askEmailChange(waiting.newEmail);
        } else if (waiting?.op === 'delete-account') {
          /*
           * `await tick()` — защита, а не починка наблюдавшегося дефекта (страж проходит и
           * без неё). Компонент удаления рисуется УСЛОВНО — только когда экран уже знает, кто
           * перед ним, — поэтому ссылка на него в принципе может быть пустой, а `?.` проглотит
           * вызов МОЛЧА: удаление просто не начнётся, и человек увидит обычный экран.
           * Цена ожидания — один кадр; цена молчаливого пропуска — необъяснимое бездействие
           * в самом ответственном месте продукта.
           */
          await tick();
...
<div class="screen">
  ...
  <GuestCard />
    ...
    {:else if facts !== null}
      ...
      <!-- ВИДЖЕТ «Удаление аккаунта» — ВО ВСЮ ПАНЕЛЬ (4 шага), последним. В сетке из двух
           колонн «низа страницы» не существует, а деструктив отделяется именно ПОЛОЖЕНИЕМ
...
<style>
  ...
  @media (min-width: 1024px) {
    ...
    /*
     * ПЛАШКА ГОСТЯ — ВО ВСЮ СТРОКУ (`bugs/226`, слово Дизайнера смены 14).
     *
     * `:global` обязателен: `.gnote` приходит из `$lib/ui/GuestCard.svelte`, и без него
     * Svelte добавил бы к селектору хеш ЭТОГО файла, которого у чужого элемента нет.
     *
     * 🔑 Класс, ради которого правка стоит на четырёх экранах сразу: собственной ширины у
     * плашки нет вовсе, поэтому её ширину назначает родитель — а `.body` здесь сетка, и
     * прямой ребёнок молча становится ячейкой. Замер до правки: одна плашка имела ЧЕТЫРЕ
     * ширины (на окне 1560 — 1228 · 411 · 298 · 296,5px, крайние вчетверо).
     */
