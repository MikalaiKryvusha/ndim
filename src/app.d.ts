...
declare global {
	namespace App {
		...
		interface PageState {
			...
			seeMe?: boolean;
			/**
			 * Развёрнут лайтбокс фотографии (`Avatar.svelte`) — здесь лежит id ИМЕННО ТОГО
			 * кружка, по которому нажали. Не `boolean` и не uid: на одной странице живёт много
			 * лиц, а одно и то же лицо может стоять дважды (свой аватар в «Профиле» и он же
			 * внутри окна «Как меня видят»), — от общего признака развернулись бы все сразу.
			 */
			photo?: string;
			/** Развёрнута панель глав документа (`ChapterNav.svelte`). */
			chapters?: boolean;
		}
		// interface Platform {}
	}
