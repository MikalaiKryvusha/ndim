...
test('боевой каталог: семь хабов покрывают 99,5 % записей', () => {
   ...
   */
  /*
   * 🔄 Перемерено 2026-09-08: **5125 → 5144**, покрытие хабами **5100 → 5119**. Растяжка сработала
   * по делу, и прирост объясним целиком — проверен, а не предположен:
   * · обе величины выросли на ОДНО И ТО ЖЕ число (+19);
   * · `withoutHub` остался **25**, `unknown` равен **НУЛЮ**, хабов по-прежнему семь;
   * · парный замер по хабам показал, куда разошлись все 19: фильмы +17, видеоигры +1, книги +1
   *   (см. тот же разбор в `catalog-hub.test.ts`, тест «числа замера»).
   * То есть все новые записи имеют известный вид и легли в СУЩЕСТВУЮЩИЕ хабы; словарь видов не
   * тронут. Прирост — одобренные владельцем кандидаты мастерской с 2026-08-22.
   */
  /*
   * 🔄 Перемерено 2026-09-25: **5144 → 5153**, покрытие хабами **5119 → 5128** — обе величины выросли на
   * ОДНО число (+9), `withoutHub` остался **25**, хабов семь; разошлись по хабам: фильмы +3, видеоигры +4,
   * сериалы +2 (тот же разбор в `catalog-hub.test.ts`, тест «числа замера»).
   */
  assert.equal(dims.length, 5153, 'размер каталога изменился — сверить числа плана 48');
  assert.equal(hubs.size, KIND_KEYS.length, 'не все хабы собрались');
  const covered = [...hubs.values()].reduce((a, b) => a + b, 0);
  assert.equal(covered, 5128, 'покрытие хабами разошлось с замером');
