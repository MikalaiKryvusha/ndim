...
test('оценка выше нуля — только ЗОЛОТЫЕ звёзды, числом округлённой оценки', () => {
...
test('🔴 средняя РОВНО ноль при живых голосах — ОДНА СЕРАЯ звезда, золотых нет', () => {
  assert.deepEqual(starRow(0), { gold: 0, grey: 1 });
});

test('положительная оценка НИКОГДА не рисуется как ноль', () => {
  // Средняя 0,3 (голоса 0, 0, 1) округляется в ноль — без нижней границы она притворилась бы
  // нулевой оценкой. В каталоге таких сегодня нет, но шкала их допускает.
  for (const r of [0.1, 0.3, 0.49]) {
    assert.deepEqual(starRow(r), { gold: 1, grey: 0 }, `сломалось на средней ${r}`);
  }
});
...
test('на РЕАЛЬНОМ срезе каталога ряд согласован с правилом показа оценок', () => {
  ...
  for (const d of dims) {
    ...
    if (d.rating > 0) assert.equal(row.grey, 0, `«${d.title.ru}»: серая звезда при ненулевой оценке`);
  }
