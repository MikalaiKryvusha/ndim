...
test('🔴 КЕЙС-12: ступени 89 / 81 / 8, и третью занимают ровно восемь НАЗВАННЫХ страниц', { skip: NO_BUILD }, () => {
  /*
   * 🔄 Перемерено 2026-09-08: ступени **91 / 82 / 5 → 92 / 78 / 8**. Сумма как была 178.
   * Прирост каталога +19 (из них 17 фильмов) сдвинул содержимое страниц, и три страницы съехали
   * на третью ступень. Причина проверена ПОИМЁННО, а не предположена — на каждой из трёх первые
   * имена длинны настолько, что при ориентире 155 не влезает даже ОДНО:
   * · `movie/12 ru` — «Толстый, больной и почти мёртвый 2», «Шрек 2»: два имени дали бы 168;
   * · `movie/38 en` — “Superman II: The Richard Donner Cut”, “The Basketball Diaries”: 183;
   * · `movie/42 en` — “The Loneliest Whale: The Search for 52”, “The Pokrovsky Gate”: 182.
   * 🔑 Это работа лестницы, а не поломка: КЕЙС-11 по-прежнему показывает переполнений НОЛЬ, а
   * `movie/1 ru` стоит на третьей ступени по ровно той же причине с самого замера («Пираты
   * Карибского моря: Сундук мертвеца» — 182 при двух именах). Прежние пять адресов целы все
   * до одного и остались в списке — новые ТРИ добавлены, а не заменили их.
   * ⚠️ Число третьей ступени растёт (5 → 8 из 178). Само по себе это не дефект, но описание без
  ...
  // дали бы тот же счёт. Список — из фикстуры §6 принятого документа, дополнен замером 2026-09-08.
  assert.deepEqual(
