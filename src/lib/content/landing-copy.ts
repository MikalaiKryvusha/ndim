 ...
 * «вычислитель» и «калькулятор» — сервер синхронизации называется сервером синхронизации.
 */
 ...
 * (якорь, вторая строка первого экрана, блок живых метрик, четыре отличия, текст доверия)
...
export const gateCopyPhase4 = {
	...
	/** Четыре отличия — заменяют три «фичи». Каждое подпёрто кодом, ни одно не о конкурентах. */
	differences: [
		{
			h: { ru: 'Ни одной рекламы и ни одного чужого счётчика', en: "No ads, no one else's trackers" },
			p: {
				ru: 'На этой странице и внутри Пространства нет ни одной рекламы и ни одного чужого счётчика. Мы считаем ровно четыре числа в сутки — сколько человек зашло, сколько подвигало звёзды здесь, сколько вошло гостем, сколько создало аккаунт, — и в этих числах нет ни имени, ни почты, ни адреса, ни устройства.',
				en: 'There is not a single advert and not a single third-party tracker on this page or inside the Space. We count four numbers a day — how many people came, how many moved the stars here, how many came in as guests, how many created an account — and those numbers hold no name, no email, no address and no device.',
			},
		},
		{
