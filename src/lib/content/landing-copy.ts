...
export const landingFeatures = [
	...
	},
	{
		...
		p: {
			/*
			 * 🔴 ЗДЕСЬ СТОЯЛО «Начните общение с теми, кто действительно Вам подходит» — приглашение
			 * к действию, КОТОРОГО В ПРОДУКТЕ НЕТ. Переписки не существует: единственная кнопка
			 * «Написать» (`routes/relations/+page.svelte:532`) стоит `disabled` с подписью «скоро»,
			 * маршрута общения нет ни одного.
			 *
			 * Класс тот же, что у `bugs/107` («каждый день приходят новые»), но дороже: та фраза
			 * врала о ТЕМПЕ, и человек этого не проверял, — эта зовёт его сделать то, что он
			 * попробует и упрётся в серую кнопку, ровно когда мы его наконец заинтересовали.
			 * Правило владельца 2026-08-28: на лице продукта констатируем факт и мягко поясняем;
			 * утверждаемый факт обязан быть правдой с опорой (`MASTER_PLAN.md`, принцип 8).
			 *
			 * Новая половина говорит ровно о том, что продукт делает СЕГОДНЯ, и держит слово
			 * «общее» из принятого якоря витрины (`gateCopyPhase4.anchor`).
			 * ⚠️ Требует вычитки владельца — текст на лице продукта идёт к нему, даже когда
			 * развилку уже решило правило.
			 */
			ru: 'Мы бережно анализируем Ваш профиль и находим самых похожих на Вас людей. Вы видите, с кем у Вас больше всего общего, и меру этого общего.',
			en: 'We carefully analyze your profile and find the people most similar to you. You see who you have the most in common with, and how much you share.',
		},
	},
	{
