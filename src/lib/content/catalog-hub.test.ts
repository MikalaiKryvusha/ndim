...
test('боевой каталог: опора взвешивания — числа замера 2026-08-14', { skip: NO_BUILD }, () => {
   ...
   */
  /*
   * 🔄 Перемерено 2026-09-08: **C 8,2401 → 8,2385**, `m` снова не сдвинулся (4).
   * Проверяемая часть объяснения: оценённых объектов в каталоге сейчас **1864**, и из закреплённых
   * величин выросла одна — оценённых ФИЛЬМОВ стало **983 → 984**, то есть ещё один объект получил
   * голоса живых людей. Сдвиг средней ВНИЗ означает, что пришедшие голоса ниже прежней средней.
   * Рост каталога на 19 записей на C не влияет вовсе: она считается по ОЦЕНЁННЫМ, а новые записи
   * приходят без единого голоса (то же наблюдение держит и перемер 2026-08-18).
   */
  /*
   * 🔄 Перемерено 2026-09-25 (каталог боя, `node tools/fetch-dims-slice.mjs --all` в работе `plans/106`):
   * **C 8,2385 → 8,2389**, `m` не сдвинулся (4). Оценённых объектов **1864 → 1882** (+18) — голоса живых
   * людей, из них фильмов +1 (984 → 985); сдвиг средней ВВЕРХ — пришедшие голоса выше прежней средней.
   * Рост каталога на 9 записей на C не влияет: новые записи приходят без голосов.
   */
  const prior = catalogPrior(catalog);
  assert.equal(prior.m, 4, 'm — 90-й процентиль числа голосов среди оценённых');
  assert.equal(prior.c.toFixed(4), '8.2389', 'C — средняя по оценённым объектам');
});
...
test('боевой каталог: знаменатель места совпадает со сводкой «с оценками»', { skip: NO_BUILD }, () => {
  ...
  // 🔄 2026-09-08: **984** — ещё один. То же событие, что сдвинуло C: голоса людей, а не рост
  //    каталога (новые записи приходят без голосов и знаменатель не меняют).
  // 🔄 2026-09-25: **985** — ещё один фильм получил голоса живых людей (разбор в тесте опоры выше).
  assert.equal([...placesIn(hubs.get('movie') ?? []).values()][0].of, 985);
});
...
test('боевой каталог: числа замера 2026-08-14 держатся', { skip: NO_BUILD }, () => {
   ...
   */
  /*
   * 🔄 Перемерено 2026-09-08. Сдвинулись ПЯТЬ величин, и каждая объяснена:
   * · каталог **5125 → 5144** (+19) — владелец продолжает одобрять кандидатов мастерской;
   * · фильмы **2787 → 2804** (+17), видеоигры **1222 → 1223** (+1), книги **64 → 65** (+1) —
   *   вместе ровно **+19**, то есть ВЕСЬ прирост каталога разошёлся по существующим хабам.
   *   Сверка сделана арифметикой, а не на глаз: 17 + 1 + 1 = 19 = прирост каталога;
   * · оценённых фильмов **983 → 984** (+1) — другое событие: голоса живых людей, а не рост.
   * НЕ сдвинулись и потому проверены отдельно: хвост 25 · страниц у фильмов 47 (семнадцать
   * карточек в разбивку новой страницей не влезли) · страниц всего 89 · сериалы 370 · романы 231
   * · практики 228 · музыканты 198. И `unknown` в парном тесте `dim-kind` равен НУЛЮ — словарь
   * видов не тронут, покрытие выросло на те же +19.
   * 🔑 Числа правятся только после этой сверки. Правка «чтобы позеленело» превратила бы растяжку
   * в украшение, а она заведена ровно затем, чтобы рост каталога не проходил молча.
   */
  /*
   * 🔄 Перемерено 2026-09-25 (каталог боя скачан заново в работе `plans/106`). Сдвинулись ПЯТЬ величин:
   * · каталог **5144 → 5153** (+9) — владелец продолжает одобрять кандидатов мастерской;
   * · фильмы **2804 → 2807** (+3), видеоигры **1223 → 1227** (+4), сериалы **370 → 372** (+2) — вместе
   *   **3 + 4 + 2 = 9** = прирост каталога: весь прирост разошёлся по существующим хабам;
   * · оценённых фильмов **984 → 985** (+1) — голоса живых людей, а не рост.
   * НЕ сдвинулись: хвост 25 · страниц у фильмов 47 · страниц всего 89 · романы 231 · практики 228 ·
   * музыканты 198 · книги 65.
   */
  assert.equal(catalog.length, 5153);
  assert.equal(tail.length, 25);
  assert.equal(byKey.movie.count, 2807);
  assert.equal(byKey.movie.rated, 985);
  assert.equal(byKey.movie.pages, 47);
  assert.equal(byKey['video-game'].count, 1227);
  assert.equal(byKey['tv-series'].count, 372);
  assert.equal(byKey.novel.count, 231);
  assert.equal(byKey.practice.count, 228);
  assert.equal(byKey['music-artist'].count, 198);
  assert.equal(byKey.book.count, 65);
