...
test('🔴 владелец: кривая быстро растёт от нуля, потом замедляется, потом плато на единице', () => {
  ...
  // ДО насыщения растёт строго и нигде не превышает единицу.
  let prev = -1;
  for (const v of [0, 1, 2, 3, 5, 10, VOTE_SATURATION - 1]) {
    const w = voteWeight(v);
    assert.ok(w > prev, `вес не растёт на ${v}`);
    assert.ok(w < 1, `вес достиг плато раньше насыщения — на ${v}`);
    prev = w;
  }
  // ПЛАТО: на точке насыщения и за ней ровно единица, а не «почти», и дальше не растёт вовсе.
  assert.equal(voteWeight(VOTE_SATURATION), 1);
  assert.equal(voteWeight(VOTE_SATURATION + 5), 1);
  assert.equal(voteWeight(200), 1);
  assert.equal(voteWeight(100000), 1);
});

test('🔴 владелец: «между 5 и 10 голосов — большая разница», «между 200 и 205 — почти неразличима»', () => {
  // Обе фразы владельца верны ОДНОВРЕМЕННО при насыщении на 20: разница 200→205 равна нулю
  // именно потому, что плато начинается задолго до двухсот (интервью №036, В2 = B).
