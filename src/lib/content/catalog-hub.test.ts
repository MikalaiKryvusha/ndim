 ...
 * `sd` = 2 — цена полного насыщения в баллах (решение владельца 2026-08-16, см. `voteWeight`).
 */
const PRIOR = { m: 4, c: 8, sd: 2 };
...
test('опора считается ИЗ ДАННЫХ: m — 90-й процентиль голосов, C — средняя по оценённым', () => {
  ...
  // СКО тоже считается ИЗ ДАННЫХ и никогда не задаётся константой: это цена полного насыщения
  // в баллах шкалы (решение владельца 2026-08-16).
  const c = (6 * 8 + 9 + 10) / 10;
  const sd = Math.sqrt(([...Array(8).fill(6), 9, 10].reduce((s, r) => s + (r - c) ** 2, 0)) / 10);
  assert.ok(Math.abs(prior.sd - sd) < 1e-12, 'СКО опоры посчитано неверно');
  // Пустой каталог не роняет расчёт и не делит на ноль.
  assert.deepEqual(catalogPrior([]), { m: 1, c: 0, sd: 0 });
});
...
test('🔴 владелец: кривая быстро растёт от нуля, потом замедляется, потом плато на единице', () => {
  assert.equal(voteWeight(0), 0, 'без голосов веса нет');
  // Строго возрастает и нигде не превышает единицу.
  let prev = -1;
  for (const v of [0, 1, 2, 3, 5, 10, 20, 50, 100, 199]) {
    const w = voteWeight(v);
    assert.ok(w > prev, `вес не растёт на ${v}`);
    assert.ok(w <= 1, `вес превысил единицу на ${v}`);
    prev = w;
  }
  // Плато: на точке насыщения и за ней ровно единица, а не «почти».
  assert.equal(voteWeight(VOTE_SATURATION), 1);
  assert.equal(voteWeight(VOTE_SATURATION + 5), 1);
  assert.equal(voteWeight(100000), 1);
});
...
test('🔴 владелец: «между 5 и 10 голосов — большая разница», «между 200 и 205 — почти неразличима»', () => {
  ...
  assert.ok(near > 0.1, `скачок 5→10 слишком мал: ${near}`);
  assert.ok(far < 0.001, `разница 200→205 слишком велика: ${far}`);
  // Именно ЗАМЕДЛЕНИЕ: тот же шаг в пять голосов у нуля весит несравнимо больше, чем у плато.
  assert.ok(near > far * 100, 'кривая не замедляется — это не логарифм');
  ...
  const linear = (v: number) => v / VOTE_SATURATION;
...
test('🔴 БОЕВОЙ СЛУЧАЙ, из которого выросло решение: 14 голосов перевешивают одинокую десятку', () => {
  ...
  const prior = { m: 4, c: 8.2405, sd: 1.6161 };
