...
test('прочерк в поле года — это «года нет», а не год (ideas/26)', () => {
  // Шаг 0 замером боевого каталога 2026-08-03: с ПУСТЫМ годом — ноль измерений, с годом «-» —
  // 170. Прежнее условие проверяло пустоту, а прочерк непуст и потому исправно печатался.
  for (const dash of ['-', '–', '—', ' - ', '--']) {
    assert.equal(dimCardTitle('Бокс', dash).year, null, `сломалось на «${dash}»`);
  }
  // Контроль прибора: настоящий год обязан выживать, иначе проверка выше зелена на коде,
  // который выкинул год вообще.
  assert.equal(dimCardTitle('Бокс', '1976').year, '1976');
  assert.equal(dimCardTitle('Бокс', 1976).year, '1976', 'число тоже терпим — так пишет стенд');
});
