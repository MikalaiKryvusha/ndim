 ...
 * (`sync-server/`, который эти документы ПИШЕТ), и в браузере (который их ЧИТАЕТ и строит
...
export interface SyncServerDoc {
  ...
  /**
   * Номер сборки — число коммитов, тронувших `sync-server/` (вычисляет `npm run sync:image`
   ...
   * синхронизации — неверно. Должен говорить о времени, когда сервер синхронизации был
   * поднят». Это ОТДЕЛЬНОЕ поле, а не переиспользование `lastRunAt`: из сердцебиения
   * выводится состояние «Работает», и заморозить его значило бы показать живой сервер
   * умершим (`syncServerState` ниже).
   *
   * `undefined` — сервер старой сборки, поля ещё не пишет; экран честно скажет «неизвестно».
   */
  readonly startedUpAt?: Millis;
  /**
