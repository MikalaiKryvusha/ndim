...
const stranger: Viewer = { isFriend: false, groups: [] };
const friend: Viewer = { isFriend: true, groups: [] };
const colleague: Viewer = { isFriend: false, groups: ['c_work'] };
...
describe('Валидация аудитории', () => {
  ...
  test(`своих кругов не больше ${MAX_CUSTOM_GROUPS}; «Друзья» сверх лимита`, () => {
    const atLimit = Array.from({ length: MAX_CUSTOM_GROUPS }, (_, i) => `c_${i}`);
...
describe('Инвариант приватности: скрытое не видно никому, кроме владельца', () => {
  ...
  test('свойство «никому» не видит ни один зритель', () => {
    for (const viewer of [stranger, friend, colleague, { isFriend: true, groups: ['c_work'] }]) {
...
describe('Дублирование по кругам — свойство видно из любого отмеченного круга', () => {
  ...
  test('виден зрителю из любого одного круга', () => {
    for (const viewer of [
      { isFriend: true, groups: [] },
      { isFriend: false, groups: ['c_work'] },
      { isFriend: false, groups: ['c_gym'] },
    ] satisfies Viewer[]) {
...
describe('Документ-подсказка audience/{viewerUid}', () => {
  test('содержит только непрозрачные id бакетов, без публичного', () => {
    assert.deepEqual(extraBucketsFor({ isFriend: true, groups: ['c_work'] }), [FRIENDS, 'c_work']);
  });
  ...
  test('дефолтный круг не дублируется, если попал и в groups', () => {
    assert.deepEqual(extraBucketsFor({ isFriend: true, groups: [FRIENDS] }), [FRIENDS]);
  });

  test('приватный бакет не попадает в читаемые никогда', () => {
    const viewer: Viewer = { isFriend: true, groups: ['c_work'] };
