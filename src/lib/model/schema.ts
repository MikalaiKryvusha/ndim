...
export interface DimDoc {
   ...
   *
   * Значения объявлены в одном месте (`tools/tag-dims-approval.mjs` → `TECH_TAG`): тег одобрения
   * `migrated` · `needs-rewrite` · `unchecked`. Облако, а не поле-статус: тегов может стать
   * больше (слово владельца: «*тегов можно сколько хочешь вешать*»).
   */
