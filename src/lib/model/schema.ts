 ...
 *
 * Модуль чистый: ни Firestore, ни Node. Исполняется и в браузере, и в сервере синхронизации.
...
/**
 * `users/{uid}` — корень пользователя. Пишет **только владелец**; читает владелец и
 * админ-клейм (замысел — «Админ может всё», интервью №041 В1, под панель `ideas/13`).
...
/** `points/{uid}` — служебное о точке человека. Читает владелец и сервер синхронизации. */
export interface PointDoc {
  /** Оси изменились — связи надо пересчитать. Сервер синхронизации выбирает по индексу `where('dirty','==',true)`. */
 ...
 *
 * `updated` — когда этот человек последний раз менял свой NDim ID. Последний блок канона 1.x в
 * раскрытой связи (`bugs/46`, кадр app-16). Кладёт его сервер синхронизации: `points/{uid}` чужого
 * человека зрителю по правилам недоступен, а у сервера синхронизации число уже загружено.
 *
 * Поле НЕОБЯЗАТЕЛЬНОЕ намеренно: в боевых `relations`, записанных прежними сборками сервера, его
 * нет. Экран в этом случае просто не показывает строку — «неизвестно» на месте даты было бы
 * выдумкой (тот же приём, что `startedUpAt` в `bugs/86`).
 */
export type RelationEntry = Relation & { readonly guestUid: Uid; readonly updated?: Millis };

/**
 * Сколько похожих людей храним в топе. Паритет с 1.x.
 *
 * Живёт здесь, а не в сервере синхронизации, потому что число видит ЧЕЛОВЕК: профиль пишет
 * «250 (максимум)», когда топ упёрся в потолок (канон 1.x, bugs/43). Две копии константы
 * разъехались бы молча, и надпись «максимум» начала бы врать.
 */
export const RELATIONS_TOP_LIMIT = 250;

/**
 * СРОК ЖИЗНИ ГОСТЯ — 7 дней ОТ РОЖДЕНИЯ АККАУНТА (`plans/63`).
 *
 * 🔵 Семантику назначил владелец, интервью №010 Р3, дословно: «*От даты создания анонимного
 * аккаунта. Создал — отсчёт пошёл*». Это обещание человеку, а не настройка: агент число не
 * пересматривает, ручки окружения нет.
 *
 * Живёт здесь, а не в сервере синхронизации, потому что срок видят ДВОЕ: сервер по нему
 * УБИРАЕТ (`sync-server/index.mjs` → `cleanupStaleGuests`), а клиент по нему РАСПОЗНАЁТ
 * «гостевая сессия истекла» (`plans/63` шаг 5). Две копии числа разъехались бы молча — и
 * экран объявлял бы живого гостя умершим или наоборот.
 */
export const GUEST_TTL_DAYS = 7;

/**
 * Истёк ли срок жизни гостя, рождённого в `bornAt`. Чистая функция — её граница (ровно
 * 7 дней — ещё жив) закреплена юнитами; и сервер, и клиент обязаны судить одним правилом.
 */
export function isGuestExpired(bornAt: Millis, now: Millis): boolean {
  return now - bornAt > GUEST_TTL_DAYS * 24 * 60 * 60 * 1000;
}

/** `relations/{uid}` — топ похожих. Читают владелец и админ-клейм (№041 В1), пишет **только сервер синхронизации**. */
...
/**
 * 🔴 ЗНАЧЕНИЯ ТЕХНИЧЕСКИХ ТЕГОВ — ОДНО МЕСТО НА ВЕСЬ ПРОЕКТ (`plans/58`).
 *
 * Владелец предупредил прямо: «*только различаем теги — по чем люди ищут фильмы, и наши
 * технические теги… смотри, не перепутай эти термины и переменные по ним*». Живут они здесь,
 * потому что это СЛОВАРЬ ДОКУМЕНТА, а форму документа в этом проекте знает TypeScript, а не
 * правила (см. шапку модуля). Отсюда их берут оба потребителя: Менеджер измерений при рождении
 * записи и разовый прибор разметки (`tools/tag-dims-approval.mjs` импортирует этот объект).
 *
 * ⚠️ Единственная неизбежная копия литерала — `firestore.rules`: правила не умеют импортировать.
 * Там она названа копией и снабжена ссылкой сюда.
 */
export const TECH_TAG = {
  ...
  UNCHECKED: 'unchecked',
  /** Одобрено владельцем: запись прошла через его руки в Менеджере измерений (интервью №044, В4 = A). */
...
export interface DimDoc {
  ...
  /**
   * 🔴 ТЕХНИЧЕСКИЕ ТЕГИ — НИКОГДА НЕ ПОКАЗЫВАЮТСЯ ЛЮДЯМ (`plans/58` → ТЕРМИНОЛОГИЯ).
   *
   * Отдельное поле, а не значения в `tags`, и это принципиально: `tags` публичное и уезжает на
   * страницу каталога с 43 011 пользовательскими значениями. Разделение держится ПОЛЕМ, а не
   * уговором о префиксах — уговор однажды не удержался бы, и «требует правки» прочитали бы все.
   *
   * Значения объявлены в одном месте — `TECH_TAG` ниже в этом же модуле. Облако, а не
   * поле-статус: тегов может стать больше (слово владельца: «*тегов можно сколько хочешь
   * вешать*»), и они лежат на ДВУХ независимых осях — что машина сказала о тексте по своду
   * (`migrated` · `needs-rewrite` · `unchecked`) и через чьи руки прошла запись
   * (`owner-approved`). Тег одной оси не отменяет тег другой.
   */
