 ...
 *
 * Менеджер измерений пишет измерение ПОЛНОЙ заменой документа, а тег одобрения ставит разметка каталога —
 ...
 * документ, но молчат о том, посылает ли его Менеджер измерений. Сними штамп — правила не заметят ничего,
...
describe('Новорождённое измерение', () => {
  ...
  test('🔒 тег при рождении РОВНО один — новая запись не объявляет себя принятой миграцией', () => {
    // Дверь правил открыта на один литерал; если Менеджер измерений начнёт слать больше, отказ придёт от
