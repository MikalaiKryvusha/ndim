...
/**
 * Ожидание расширено 2026-08-22: в наборе появился `dev: 5173` (`researches/45`, `plans/69` шаг 2).
 *
 * ПОЧЕМУ ТЕСТ ИЗМЕНЁН, А НЕ ПОДОГНАН. Он стережёт НЕ «в наборе ровно четыре порта», а «базы
 * названы в ОДНОМ месте и совпадают с замком доски». Тестовый парк делает `vite dev` слотовым
 * портом наравне с остальными, то есть его база обязана жить там же; без неё модуль слота
 * завёл бы вторую таблицу — ровно ту пару «истина ↔ зеркало», которую этот файл и существует
 * не допускать. Сверка ПОЛНЫМ РАВЕНСТВОМ сохранена намеренно: она и есть то, что заставляет
 * прийти сюда и объясниться, вместо того чтобы дописать порт молча.
 */
