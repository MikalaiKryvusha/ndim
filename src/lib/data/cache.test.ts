...
describe('кэш данных экранов', () => {
  ...
  test('контракт свежести: мои данные живут всю сессию, серверные — минуту', () => {
    // Значения контракта — часть договора с `ideas/14`, а не случайные числа: если кто-то
    // поменяет их «на глазок», этот тест скажет об этом вслух.
    assert.equal(FRESH.mine, Number.POSITIVE_INFINITY);
    assert.equal(FRESH.server, 60_000);
  });
});
