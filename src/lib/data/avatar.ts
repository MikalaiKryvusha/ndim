...
/**
 * Забыть все лица (выход из аккаунта, `ideas/18`).
 *
 * Здесь лежат ЧУЖИЕ люди — те, кто попал в мой топ связей. Пережить выход они не имеют права.
 * Заодно отпускаем blob-URL'ы: без `revokeObjectURL` браузер держит скачанные файлы в памяти
 * вкладки до её закрытия — ровно то разрастание, которого опасался владелец.
 */
