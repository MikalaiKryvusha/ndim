 ...
 *
 * Теперь комментарий и код говорят одно и то же: список белый, всё прочее молчит — и стенд, и
 * стейдж, и канал предпросмотра, и любой адрес, которого мы не знаем.
 */
export function analyticsHostAllowed(hostname: string): boolean {
  return (PROD_HOSTS as readonly string[]).includes(hostname);
}
