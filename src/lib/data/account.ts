 ...
 * удаляет заброшенные гостевые точки через `GUEST_TTL_DAYS` дней (`schema.ts`; сегодня это
 * СЕМЬ — здесь стояло «30 дней» и «через месяц» с эпохи, когда константа равнялась 30, тот же
 * класс, что `bugs/201`: копия числа пережила его правку). Если после апгрейда флаг не снять,
 * человек с настоящим аккаунтом останется невидимым и потеряет данные в срок гостя.
