 ...
 *
 * ⚠️ КАНОН ЭКОНОМИИ ЗАПРОСОВ соблюдается по построению: список Менеджера измерений — это ОДНО чтение
...
/** Строка списка Менеджера измерений. Ровно то, что есть в индексе, — большего для списка не нужно. */
...
/** Каталог глазами Менеджера измерений. */
 ...
 * Менеджера измерений читается ОДНИМ запросом к индексу `dims_list` (указание владельца об экономии
...
/**
 * Список каталога для Менеджера измерений — **РОВНО ОДНО ЧТЕНИЕ** документа `dims/dims_list`.
 *
 ...
 * Замерено прибором `tools/measure-admin-dims-reads.mjs`: открытие Менеджера измерений стоит **1 чтение**.
 *
 ...
 * открытии Менеджера измерений ради случая, который случается раз в день. Владелец эту цену не принял, и он
...
export async function createDim(draft: DimDraft): Promise<string> {
  ...
  // Новорождённое измерение несёт тег «одобрено владельцем»: обе его двери — руки владельца.
