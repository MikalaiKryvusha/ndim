 ...
 * страницы собраны с `csr = false` и НА ПОКАЗЕ клиентского JS не грузят — ради числа, которое
 * и так отдаёт Search Console. `door_click` этой цены не платит: в момент касания двери
 * Firebase грузится всё равно.
 * ⚠️ «На показе» здесь не оговорка ради осторожности: дверь оценки — отдельная сборка, и она
 * своё клиентское добро несёт. Прежняя редакция говорила «клиентского JS не несут вовсе», и
 * ровно эту формулу опровергла сборка в вердикте QA №14 (`plans/78` → «Границы»).
 *
...
// Только тип: стирается при сборке, поэтому круга импортов `funnel ↔ analytics` в рантайме нет.
...
/**
 * МЕСТА ВХОДА — закрытый союз, а не свободная строка (дефект Д4 вердикта QA №14; разбор, почему
 * закрытый, — в шапке свойств `analytics.ts`, который реэкспортирует этот союз).
 *
 * 🆕 2026-09-25 (`plans/105` Б2): место входа едет у `guest_start` — через какую дверь человек стал
 * гостем. Повод — неделя ролика 14–20.09: 9 гостей, и ни про одного неизвестно, с какой двери он
 * пришёл, потому что все двери ведут на один адрес `/profile?guest=1`. Слово каждой двери:
 *   · `root`         — большая кнопка главной `ndimspace.app` (туда ведёт ссылка из ролика);
 *   · `landing`      — мост из демо языкового лендинга `/ru` · `/en`;
 *   · `catalog_card` — дверь карточки каталога (10 222 страницы);
 *   · `signin`       — «гостем» на экране входа;
 *   · `restart`      — «начать заново» после истёкшей гостевой сессии (человек ВЕРНУЛСЯ);
 *   · `test`         — страница теста: гость рождается первой оценкой, двери нет вовсе;
 *   · `direct`       — адрес без слова двери: старая ссылка, закладка, набранный руками адрес.
 *
 * 🔑 ПОЧЕМУ СОЮЗ ЖИВЁТ ЗДЕСЬ, а не в `analytics.ts`. Профиль разбирает слово двери сразу при входе,
 * а `analytics.ts` грузится отдельным чанком только в момент отправки (`EXP-0028`). Статический
 * импорт оттуда затащил бы чанк аналитики в профиль; `analytics.ts` же и так импортирует из этого
 * файла, так что союз остаётся одним фактом в одном месте.
 * ⛔ Новое место входа добавляется СЮДА, а не приходит строкой из вызова.
 */
export const ANALYTICS_ENTRIES = ['landing', 'catalog_card', 'direct', 'root', 'signin', 'restart', 'test'] as const;

export type AnalyticsEntry = (typeof ANALYTICS_ENTRIES)[number];

/**
 * Слово двери из адреса `?guest=<слово>` → место входа. Чистая функция: её судит юнит.
 *
 * Слово вне союза (`1` старых ссылок, мусор, чужая подстановка) читается как `direct`: человек
 * пришёл по адресу, которого не ставила ни одна наша дверь. Молчанием его не теряем — иначе у
 * части гостей места входа не было бы вовсе, и в ряду появилась бы пустая корзина без имени.
 * [TESTED: 2026-09-25 · ручной прогон сборки под именем стейджа ЕВ-01…ЕВ-07 0 провалов, `?guest=1` и чужое слово → direct;
 *  отчёт qa/reports/2026-09-25_guest-entry.md]
 */
export function entryOf(word: string | null): AnalyticsEntry {
  return (ANALYTICS_ENTRIES as readonly (string | null)[]).includes(word) ? (word as AnalyticsEntry) : 'direct';
}
...
/**
 * Отпускает шаг, занятый этим визитом: следующий `claimStep` того же шага снова скажет `true`.
 *
 * Один законный повод — визит кончился, а вкладка осталась (`bugs/NEW_restart_guest_start_swallowed_in_same_tab.md`):
 * «Начать заново» после истёкшей гостевой сессии отпускает мёртвого гостя и рождает нового той же вкладкой. Вкладка,
...
export function releaseStep(step: FunnelStep): void {
  try {
    sessionStorage.removeItem(`${SESSION_PREFIX}${step}`);
  } catch {
    // Хранилища нет — занимать было нечем, отпускать тоже.
  }
}
...
export async function track(step: FunnelStep, props: AnalyticsProps = {}, options: TrackOptions = {}): Promise<void> {
  ...
  /*
   * 🆕 ВТОРОЙ ПРИБОР, 2026-08-29 (`plans/78` Ш2; решения владельца №049 В2 = В и №054 В1 = А).
   * Тот же шаг уезжает в PostHog — и это НЕ замена, а сверка: своя воронка остаётся эталоном,
   * потому что её не режут блокировщики. Расхождение рядов не поломка, а материал (у топов
   * источников измерения всегда двое и больше — разведка событийных моделей, вывод 2).
   *
   * Импорт динамический по двум причинам сразу: чанк аналитики не должен ехать в главный
   * бандл (`EXP-0028`), и `analytics.ts` импортирует из этого файла — статический импорт
   * замкнул бы круг. Ошибку глушим здесь же: чужой сервис не имеет права утащить за собой
   * запись СВОЕГО счётчика, которая идёт следующей строкой.
   */
