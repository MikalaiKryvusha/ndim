 ...
 *
 * ⚠️ ОТДЕЛЬНЫЙ ТИП, а не строка сообщения, и это принципиально: для только что заведённого гостя
 * отсутствие документа — НОРМА (он ещё ничего не оценил, документ появится с первой оценкой), а для
 * вошедшего НЕ-гостя — недостроенный профиль, который экран достраивает на месте
 * (`loadScreenHealing` в `profile/+page.svelte`; до 2026-09-19 здесь стояло «настоящая поломка», и
 * новичок, вошедший через Google, навсегда видел «Не удалось загрузить» —
 * `bugs/NEW_google_signin_newcomer_lands_on_load_error.md`). Различать эти случаи по тексту ошибки
...
export async function saveRating(uid: Uid, dimId: string, value: number): Promise<void> {
  ...
  /*
   * Шаг пути «человек начал делать дело» (`plans/78` Ш4). Место выбрано ЗДЕСЬ, а не на экране:
   * сюда приходят обе дороги — лента «Измерения» и дверь карточки каталога через
   * `saveTestRating`, — и один вызов покрывает обе. Инструментовка на экране покрыла бы одну и
   * молча потеряла вторую.
   *
   * ⛔ `dimId` НЕ уезжает и уехать не может: свойства события — закрытый список
   * (`analytics.ts` → `ANALYTICS_PROPERTIES`), предмета оценки в нём нет (интервью №002 В4).
   * Событие говорит «оценка сохранена», а не «оценено вот это».
   */
  void import('./analytics.ts')
    .then(({ capture }) => capture('rating_saved'))
    .catch(() => {});
}
...
export async function saveProfile(
  ...
  /*
   * Вторая половина обещания продукта: «меня можно найти». Человек, оценивший измерения, но не
   * рассказавший о себе ничего, для чужого топа остаётся пустой карточкой (`plans/78` Ш4).
   * ⛔ Ни одно значение профиля наружу не уезжает — только сам факт правки.
   */
  void import('./analytics.ts')
    .then(({ capture }) => capture('profile_filled'))
    .catch(() => {});
}
