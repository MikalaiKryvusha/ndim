 ...
 * измерений (`allow update, delete: if isAdmin()`), и СВОИ ПОЛОВИНЫ В ПАРАХ ТЕСТА
 * (`testPairs`: `allow list: if false` — клиент своих пар не найдёт; `bugs/154`, решение
 * владельца №041 В2 = A). Их вычищает суточный проход сервера синхронизации — решение
 * владельца В11 = А, 2026-08-01; половины пар при живом втором участнике он ОБЕЗЛИЧИВАЕТ
 * (`anonymizePairHalvesOf`). Человеку об этом говорится честно:
