<!--
  ...
  уже была, владелец поймал сразу). Кнопку темы до оживления и без JS ловит делегированный
  слушатель инлайн-скрипта `app.html` (атрибут `data-theme-toggle`), значок переключают СТИЛИ по
  `data-theme` на `<html>`, выпадашка языка — родной `<details>`. Режим `static` ставит кнопке
  `id="theme-toggle"` — адрес для приборов. Сама пара — общая на все шапки (`HeadControls.svelte`).
-->
<script lang="ts">
