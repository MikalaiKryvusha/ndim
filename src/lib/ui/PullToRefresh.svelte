<script lang="ts">
  ...
  onMount(() => {
    /*
     * ДВЕРЬ СТЕНДА: исполняемый вход в механизм обновления (`bugs/78`).
     *
     * Зачем она нужна. После решения владельца В1=Б строка «Обновить данные» убрана из «Меню»,
     * и `refreshNow` стал достижим ТОЛЬКО жестом. А жест стражами намеренно не эмулируется:
     * синтетический `touchmove` не воспроизводит физику прокрутки браузера и доказывал бы
     * работу прибора, а не продукта (EXP-0078/0080). Судья назвал следствие прямо: инвалидация
     * осталась без единой ЗАПУСКАЕМОЙ проверки, и сломайся она — узнал бы владелец пальцем.
     *
     * Дверь читается ТОЛЬКО на стенде (localhost/127.0.0.1) — тот же приём, что у `?as=guest`
     * и `?db=`; в бою `window.__ndimRefresh` не появляется вовсе (код в бандле есть — артефакт
     * один на все окружения, — но дверь не открывается). Она дёргает РОВНО тот путь, что и
     * отпущенный палец (`refreshNow(onRefresh)`), поэтому проверяет продукт, а не свою копию
     * логики. Юнит-часть (какие ключи гаснут) живёт в `cache.test.ts`.
     *
     * Хост сверяем строкой, а не готовым `isStand()` из `lib/firebase.ts`: тот модуль тянет за
     * собой Firebase SDK, а в оболочке экранов SDK не место (канон лёгкости, EXP-0028). Ровно
     * так же поступает лендинг (`routes/+page.svelte`).
     */
    if (['localhost', '127.0.0.1'].includes(location.hostname)) {
      (window as unknown as Record<string, unknown>).__ndimRefresh = () => refreshNow(onRefresh);
    }
...
{#if enabled}
  <!-- Индикатор — вне потока, поверх экрана и ПОД шапкой: `--bar-h` публикует её высоту
       (`barheight.svelte.ts`), а до публикации работает `--bar-h-fallback` — запасная высота по
       брейкпойнту (`+layout.svelte`, bugs/67). Прежние зашитые 56px не соответствовали шапке
       ни на одной ширине: живой замер даёт 61px на узких и 55px от 1024px. -->
  <div
    class="ptr"
    class:show={visible}
