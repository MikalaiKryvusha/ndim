 ...
 *   · **стенд** (localhost) — эмуляторы Firestore и Auth проекта `demo-*`. Такой идентификатор
 *     SDK считает заведомо локальным и физически не может дотянуться до боевых данных. Здесь
 *     живут `npm run stand` и e2e. 🆕 Порты — СЛОТОВЫЕ (`plans/69`): у каждой роли команды свои,
 *     умолчания равны прежним 8181/9099/9199, подстановка — ниже у самих констант.
 ...
 *
 * Значения подставляет `vite.config.ts`: он выводит слот из имени рабочего места и ставит
 * `VITE_STAND_*` сам (фаза 2 парка). 🔑 Именно поэтому здесь можно не думать о том, кто запустил
 * сборку: Vite подставляет эти переменные НА СБОРКЕ, а слот знает конфиг — значит артефакт несёт
 * СВОЙ слот при любом входе, включая `npm run build`, поднятый Playwright. Раньше их раздавала
 * обёртка запуска, и сборка мимо обёртки молча запекала слот 0, то есть эмулятор соседа.
 *
 ...
 */
/*
 * 🔴 ВЫКЛЮЧЕНО 2026-08-22 02:45 — App Check в бою отдавал 403 и шумел в консоли у КАЖДОГО.
 *
 * Что случилось. Первый боевой выкат с этой врезкой (ночь 22-го) прошёл дверь до смоука под
 * сессией, и смоук покраснел: `content-firebaseappcheck.googleapis.com/…/exchangeRecaptchaV3Token`
 * отвечает **403**, а `google.com/recaptcha/api2/clr` не доезжает вовсе. Адрес назван зондом —
 * прибор смоука печатает голое «403» без адреса, и по такому сообщению чинить нечего.
 *
 * Чем это НЕ является: поломкой продукта. Принуждение не включено (решение №009 — наблюдение
 * сначала), поэтому отказ обмена ничего не блокирует: вход, все пять экранов и содержимое
 * человека прошли смоук зелёными. Сломано ровно то, ради чего врезка ставилась, — НАБЛЮДЕНИЕ:
 * токенов нет, доли «с токеном / без» копить не из чего.
 *
 * Почему ключ снят, а не оставлен шуметь: пустая константа — это штатный предохранитель этого
 * же модуля (`maybeInitAppCheck` выходит первой строкой), и он возвращает бой ровно в то
