 ...
 * `black-bird-jdi17u8i` (2026-08-22, ряд 16, цитата-название: её `plans/70` не тронет никогда,
 * поэтому контроль перевешен ЗАРАНЕЕ, а не после падения).
 */
import {
 ...
 */
/** Короче этого кусок в кавычках цитатой не считается — так отсеиваются названия в кавычках. */
const MIN_QUOTE_CHARS = 20;

/** Кавычки, у которых открывающая и закрывающая РАЗЛИЧИМЫ, — их парность регулярка не путает. */
const PAIRED_QUOTE_RE = [
  new RegExp('«[^»]{' + MIN_QUOTE_CHARS + ',}»', 'g'),
  new RegExp('“[^”]{' + MIN_QUOTE_CHARS + ',}”', 'g'),
];

/**
 * КУСКИ ТЕКСТА ВНУТРИ КАВЫЧЕК. Здесь важнее длины — ПАРНОСТЬ.
 *
 * 🔴 Прежде это была одна жадная регулярка с порогом {20,} внутри. Для парных « » и “ ” она
 * верна: открывающая и закрывающая — разные знаки, перепутать пару нечем. Но у ПРЯМОЙ кавычки
 * открывающая и закрывающая — ОДИН И ТОТ ЖЕ знак, и порог длины ломает разбивку на пары:
 * короткое название в кавычках регулярке не подходит (не набралось знаков), она его пропускает
 * и составляет пару из ЗАКРЫВАЮЩЕЙ кавычки названия и ОТКРЫВАЮЩЕЙ следующего. Проза между
 * двумя названиями объявляется цитатой.
 *
 * 🔑 Цена измерена, а не оценена: по заданию plans/70 (533 английские копии, один и тот же
 * words() с обеих сторон) старый признак извинял 23 записи, из них 19 ЛОЖНО — живые
 * заимствования ходили с индульгенцией; и одну настоящую цитату (wicked-part-i) он не видел
 * вовсе. Ворота правок (gate-rewrites) считают запись вылеченной, если остаток лежит внутри
 * кавычек, — то есть ложная пара красила бы невылеченную правку зелёным.
 *
 * Поэтому прямые кавычки парятся ПО ПОРЯДКУ (первая со второй, третья с четвёртой), а порог
 * длины применяется ПОСЛЕ разбивки и в неё не вмешивается.
 */
export function quotedSpans(ourText) {
  const text = String(ourText);
  const spans = [];
  for (const re of PAIRED_QUOTE_RE) spans.push(...(text.match(re) ?? []));
  const parts = text.split('"');
  /*
   * Нечётные куски лежат МЕЖДУ парой кавычек. Последний нечётный при непарном числе кавычек —
   * хвост без закрывающей; цитатой он не считается, иначе одна забытая кавычка извиняла бы
   * весь остаток описания.
   */
  for (let i = 1; i < parts.length - 1; i += 2) {
    if (parts[i].length >= MIN_QUOTE_CHARS) spans.push(parts[i]);
  }
  return spans;
}
...
export function isInsideQuotes(runText, ourText) {
  ...
  if (!run) return false;
  return quotedSpans(ourText).some((q) => words(q).join(' ').includes(run));
}
...
const LIVE_CONTROLS = {
  ...
  /*
   * 🔄 ПЕРЕВЕШЕН 2026-08-22 (dev-1, партия 1 `plans/70`). ПОЧЕМУ ПРЕЖНИЙ УШЁЛ: контролем стоял
   * `corrective-measures-akp4cezf` (ряд 50) — и он же оказался ПЕРВОЙ записью партии 1, то есть
   * тем самым текстом, который партия лечит. Починив его, исполнитель убил бы измеритель:
   * следующий английский свод не начался бы вовсе. Контроль перевешен ДО правки, а не после
   * падения, — это тот же класс, что и два прежних перевеса, но пойманный заранее.
   *
   * ПОЧЕМУ ИМЕННО ЭТА ЗАПИСЬ, а не следующая по длине: ряд `black-bird` — это название книги
   * «In with the Devil: A Fallen Hero, a Serial Killer, and a Dangerous Bargain for Redemption»,
   * приведённое в кавычках. `plans/70` запрещает трогать записи, где ряд лежит внутри кавычек,
   * НАВСЕГДА — значит эта копия переживёт весь план, и контроль не умрёт на следующем же
   * вылеченном тексте. Соседи по длине (`edge-of-tomorrow`, `poyekhavshaya`) лежат в той же
   * партии 1 и не годятся по той же причине, по какой ушёл прежний.
   *
   * Ряд снят живым адресным замером 2026-08-22: 16 слов, вердикт `copied`. Порог взят ниже
   * измеренного с запасом — контроль судит ДОРОГУ (поиск статьи → кэш → разбор), а не длину
   * конкретного ряда; двенадцать слов подряд по случайности не совпадают.
   */
  en: { slug: 'black-bird-jdi17u8i', minRun: 12 },
};
