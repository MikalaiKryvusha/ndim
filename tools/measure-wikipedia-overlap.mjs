 ...
 * работа на верхнем уровне, обязан спрашивать, запустили его или подключили». Защищена была
 * только СЕТЕВАЯ часть, а самотест остался на голом `argv.includes('--selftest')` — и первый же
 * сосед, импортировавший ядро (`check-candidate-descriptions.mjs`) и принявший тот же флаг,
 * получил ЧУЖОЙ самотест вместо своего, с `process.exit(0)` в конце. Свой при этом не исполнился
 * ни разу и выглядел пройденным.
 */
