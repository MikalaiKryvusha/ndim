...
for (const doc of found.docs) {
   ...
   */
  /*
   * Подпись пустоты зависит от СТАТУСА (`bugs/173`, пункт 4). У возврата пустого замечания не
   * бывает — кнопка его не пропускает, поэтому пусто означает поломку и так и подписано. У
   * одобрения пусто — норма: одобрить можно и молча. Одна подпись на оба случая вводила бы
   * владельца и агента в заблуждение ровно в половине случаев.
   */
  const emptyNote = STATUS === 'returned'
    ? '(пусто — но кнопка возврата пустого не пропускает)'
    : '(пусто — это законно: решение без слов)';
  console.log(`   🔴 ЕГО ЗАМЕЧАНИЕ: ${c.ownerNote ?? emptyNote}`);
  /*
   * 🔑 СЛОВО ПРИ ОДОБРЕНИИ — ВТОРАЯ ПОЛОВИНА ТОГО ЖЕ КОНТУРА (`bugs/173`). Печатается ОТДЕЛЬНОЙ
   * строкой и рядом с замечанием возврата, потому что это разные события: «поправь вот это» и
   * «так и надо, делай дальше так же». Владелец потерял именно похвалу и заметил потерю сам —
   * значит она ему важна не меньше правки. Печатать её только при статусе `approved` было бы
   * ошибкой: карточка, одобренная и позже возвращённая, несёт обе строки, и обе нужны.
   */
  if (c.ownerApprovalNote) console.log(`   🟢 ЕГО СЛОВО ПРИ ОДОБРЕНИИ: ${c.ownerApprovalNote}`);
