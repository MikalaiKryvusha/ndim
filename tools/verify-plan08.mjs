 ...
 *
 * ⚠️ ЧЕСТНО О ПРОВЕРКЕ №2 (иначе она читалась бы сильнее, чем есть). Сегодня её держат ТРИ
 * независимых причины, и мутация это показала: одиночные мутанты «пишем снимок в
 * `sessionStorage`» и «восстанавливаем даже холодный экран» оставили страж ПОЛНОСТЬЮ зелёным.
 * Покраснела она только на мутанте из трёх частей разом — снимок в `sessionStorage` + `warm:
 * true` + восстановление при `nav.from === null` («до F5 900, после F5 900»), то есть на честно
 * реализованном «варианте Б» владельца. Значит проверка охраняет ровно то, что должна, но
 * повторить её падение можно лишь целым вариантом Б, а не одной строкой (родня EXP-0087:
 * удобство продукта — источник вакуумных проверок).
 *
