...
export function claimsGate(gateMarker) {
  ...
  /*
   * ⚠️ РАЗРЕЗАТЕЛЬ ЗДЕСЬ ПРОСТОЙ, И ЭТО ЗАМЕР, А НЕ НЕБРЕЖНОСТЬ. Первая редакция этой починки
   * резала ещё и по `,.;:()«»` — «чтобы пунктуация не прилипала к слову». Мутационная проба
   * показала, что классы эти НЕ НЕСУЩИЕ: снимаешь их — самотест остаётся зелёным, потому что
   * опознаётся КОМАНДА, а «нет,» не команда ни с пунктуацией, ни без неё. Проверка, которую
   * нельзя уронить, доказывает только себя, а мёртвая сложность в признаке ещё и врёт о том,
   * что она что-то держит. Убрано мутацией, а не вкусом.
   */
