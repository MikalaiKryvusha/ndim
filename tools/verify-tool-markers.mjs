 ...
 *
 * ═══ ЗДЕСЬ ОПОЗНАЁТСЯ ЗАЯВКА, А НЕ ОТКАЗ. ЭТО И ЕСТЬ ЛЕЧЕНИЕ ═══
 *
 * Две первые редакции пытались опознать ОТКАЗ — «строка начинается со слова „нет"» — и обе
 * прохудились, потому что отказ пишется прозой, а проза бесконечна:
 *
 *   ред. 1  `/^нет\b/i`      → не совпадала НИКОГДА: `\b` в JS определена через ASCII, между
 *                              кириллической «т» и пробелом границы не бывает (`bugs/193`);
 *   ред. 2  первое слово     → `нет,` `нет.` `нет;` `нет:` — четыре формы, которые человек
 *           против 'нет'       читает как отказ, машина читала как ЗАЯВКУ (нашёл dev-1).
 *
 * 🔴 Соблазн третьей редакции — «снять хвостовую пунктуацию» — УЖЕ БЫЛ ОПРОВЕРГНУТ ЗАМЕРОМ,
 * прежде чем я его написал. Замер на слитом дереве (`window-shift10` 64fc0ac, 14 живых строк
 * «Ворота:»): форм с хвостовой пунктуацией — НОЛЬ, а живой промах ровно один и другого рода:
 *
 *     verify-import-safety.mjs → «Ворота: пока НЕТ — по правилу…»
 *
 * Первое слово здесь «пока», и снятие пунктуации его не касается. То есть однострочное лечение
 * УЖЕ той дыры: оно чинит три формы, которых в дереве нет, и не чинит ту одну, которая есть.
 *
 * ═══ ПОЧЕМУ ОПОЗНАВАТЬ ЗАЯВКУ ДЕШЕВЛЕ И ПРОЧНЕЕ ═══
 *
 * Словарь отказов ОТКРЫТ: «нет», «нет,», «пока нет», «не стоит», «вне ворот», «— нет». Каждый
 * новый автор допишет туда форму, о которой я не подумал, и признак будет чиниться вечно.
 * Словарь ЗАЯВОК закрыт и проверяем: ворота зовут КОМАНДОЙ, а команда в этом дереве начинается
 * с `npm` или `node`. Поэтому заявкой считается только то, что НАЗЫВАЕТ КОМАНДУ; всё прочее —
 * ответ «не в воротах», каким бы словом он ни был написан.
 *
 * 🔑 И направление промаха переворачивается — это главное. Раньше неузнанный ОТКАЗ давал ЛОЖНЫЙ
 * КРАСНЫЙ и учил писать под прибор (dev-1 заменил запятую на тире, чтобы страж замолчал, — вот
 * ровно та цена). Теперь неузнанная ЗАЯВКА даёт «не в воротах», и если файл в воротах СТОИТ,
 * его ловит соседняя проверка — «маркер врёт о своём же месте», с верным текстом отказа.
 * Дыра не исчезла, она переехала туда, где её ловит другой сторож.
 *
 * ⚠️ Граница честная: появятся ворота, которые зовут не `npm` и не `node`, — их маркер прочтётся
 * отказом. Молча это не пройдёт: файл, стоящий в составе `guards.mjs`, тут же покраснеет
 * проверкой «стоит в воротах, а маркер говорит иначе». Направление отказа безопасное.
 */
const КОМАНДЫ = new Set(['npm', 'node']);

export function claimsGate(gateMarker) {
  if (gateMarker === null) return false;
  /*
   * ⚠️ РАЗРЕЗАТЕЛЬ ЗДЕСЬ — ОДИН ПРОБЕЛ, И ЭТО ВЫВОД МУТАЦИЙ, А НЕ НЕБРЕЖНОСТЬ.
   *
   * Первая редакция починки резала ещё и по `,.;:()«»`, вторая — по тире. Мутационная проба
   * уронила оба класса как НЕ НЕСУЩИЕ: снимаешь их — самотест остаётся зелёным. Причина одна
   * и она устройственная: опознаётся КОМАНДА, а «нет,» и «нет —» не команда ни с пунктуацией,
   * ни без неё. Проверка, которую нельзя уронить, доказывает только себя; мёртвая сложность в
   * признаке сверх того ВРЁТ, будто что-то держит, — и следующий побоится её тронуть.
   *
   * Осталось ровно два свойства, и каждое сторожится своим случаем самотеста: снятие ведущих
   * кавычек (`«`npm run guards`»` — живая форма у пяти приборов) и приведение регистра
   * (заявка заглавными).
   */
  const first = String(gateMarker)
    .trim()
    .replace(/^[`'"«*\s]+/u, '')
    .split(/\s+/u)[0]
    .toLowerCase();
  return КОМАНДЫ.has(first);
}

/**
 * 🔴 МАРКЕР ОПИСЫВАЕТ СОСТОЯНИЕ, А НЕ НАМЕРЕНИЕ — класс, усиленный QA в вердикте №26.
 *
 * ═══ ЧТО СЛУЧИЛОСЬ, И ПОЧЕМУ ИСПРАВНОГО ЧИТАТЕЛЯ МАЛО ═══
 *
 * Починка `claimsGate` научила прибор читать «пока НЕТ» как ОТКАЗ — верно, и на этом дефект
 * читателя закрыт. Но судья, разбирая свою же приёмку, назвала вторую половину класса, и она
 * шире: **«пока НЕТ» перестало быть правдой в ту секунду, когда строка встала в ворота, и
 * никто не был обязан это заметить.** Такой маркер устаревает молча ДАЖЕ ПРИ ИСПРАВНОМ
 * ПРИБОРЕ — потому что он говорит не о сегодняшнем дне, а о намерении.
 *
 * Отсюда правило, которое механизирует эта функция: **в поле, которое машина читает как ФАКТ,
 * стоит сегодняшнее состояние. План живёт в реестре `ОТСРОЧКА` (у него есть срок годности) или
 * в документе плана.** «Пока нет, вписывается после мержа» — это план.
 *
 * ═══ ПОЧЕМУ ПРИЗНАК СМОТРИТ ПЕРВОЕ СЛОВО И ТОЛЬКО У НЕ-ЗАЯВКИ ═══
 *
 * Словарь намерений открыт ровно так же, как был открыт словарь отказов, и повторять ту ошибку
 * нельзя. Замер живого корпуса (15 маркеров «Ворота:», дерево `main` 3af3595) показал, где
 * проходит настоящая граница:
 *
 *   ЗАКОННЫЕ, УПОМИНАЮЩИЕ МЕРЖ (признак обязан молчать) — их ТРИ:
 *     verify-bug-doc-state     «`npm run guards` — с окна мержей смены 10…»
 *     verify-dormant-declarations  «`npm run guards` — с окна мержей смены 10…»
 *     verify-import-safety     «`npm run guards` — условие … выполнилось при мерже»
 *   ЗАКОННЫЕ ОТКАЗЫ (признак обязан молчать) — их ТРИ, все начинаются словом «нет».
 *   ОПАСНЫЙ, ЖИВШИЙ В ДЕРЕВЕ ДО ПОЧИНКИ:
 *     verify-import-safety     «пока НЕТ — по правилу „в ворота, когда ноль красных"»
 *
 * 🔑 Наивный поиск слова «мерж» покраснел бы на ТРЁХ живых законных маркерах. Различает их не
 * словарь, а МЕСТО слова: у законных впереди стоит СОСТОЯНИЕ (команда), а мерж поминается в
 * причине, задним числом; у опасного намерение стоит ПЕРВЫМ СЛОВОМ и подменяет собой ответ.
 *
 * Поэтому признак закрыт с двух сторон и повторяет устройство `claimsGate`:
 *   1) заявка (команда) — состояние по построению, и её враньё ловит соседняя проверка;
 *   2) у не-заявки смотрится ровно ПЕРВОЕ слово, и словарь здесь мал и проверяем.
 *
 * ⚠️ Граница честная: намерение, спрятанное в середину отказа («нет — встанет после мержа»),
 * признак не возьмёт. Направление промаха то же, что у `claimsGate`, и оно безопасное: такой
 * маркер остаётся отказом, то есть сегодняшним состоянием, а в тот день, когда файл встанет в
 * ворота, соседняя проверка покраснеет словами «маркер врёт о своём же месте».
 */
const НАМЕРЕНИЯ = new Set([
  'пока', 'будет', 'встанет', 'вписывается', 'вписываю', 'планируется', 'после', 'когда', 'временно', 'скоро',
]);

export function describesIntent(gateMarker) {
  if (gateMarker === null || gateMarker === undefined) return false;
  if (claimsGate(gateMarker)) return false; // заявка — это состояние, её судит другая проверка
  const first = String(gateMarker)
    .trim()
    .replace(/^[`'"«*\s]+/u, '')
    .split(/\s+/u)[0]
 ...
 *
 * ═══ ЧТО ПИСАТЬ В ПРИЧИНУ — ЯКОРЬ-КОММИТ, А НЕ ИМЯ ВЕТКИ ═══
 *
 * Сюда приезжает всё, что маркер сказать не вправе (см. `describesIntent` ниже: в маркере —
 * сегодняшний факт, план — здесь). И у плана есть своя форма, оплаченная дважды за одни сутки
 * (правило Менеджера, `main` 3af3595): **условие снятия называет КОММИТ, а не имя ветки.**
 * «Снимается при мерже ветки dev-3» разъезжается молча — работа переезжает между ветками и
 * мастерскими, а имя остаётся тем же; «снимается, когда 79881a7 окажется в main» проверяется
 * одной командой и не зависит от того, чьей веткой она приехала.
 */
...
export function judgeFile(file, gateFiles, deferred = ОТСРОЧКА) {
  ...
  // 🔴 Третья сторона, и она про ВРЕМЯ, а не про место: маркер обязан говорить о сегодня.
  if (describesIntent(file.gate)) {
    faults.push(`${name}: маркер «${file.gate}» описывает НАМЕРЕНИЕ, а не СОСТОЯНИЕ — такая строка устаревает молча в день, когда намерение исполнится. План живёт в ОТСРОЧКА (у неё есть срок годности) или в документе плана; здесь — сегодняшний факт`);
  }
...
/**
 * 🔴 ОХВАТ ПЕРЕПИСИ, ОБЪЯВЛЕННЫЙ ВСЛУХ — замечание З1 вердикта №20.
 *
 * ═══ ПОВОД: ДВА ВЕРНЫХ ЧИСЛА, КОТОРЫЕ ЧИТАЮТСЯ КАК СПОР ═══
 *
 * После окна мержей в одних воротах стоят два прибора, и оба печатают «размер корпуса»:
 * перепись считает по `tools/`, страж импорта (`verify-import-safety`) — по `tools` и
 * `sync-server`. **Оба числа верны, каждое для своего охвата, и именно поэтому опасны:**
 * следующая сессия возьмёт любое как «сколько у нас приборов» и ошибётся уверенно. Это
 * `EXP-0226` («число из другого контура») в форме охвата.
 *
 * 🔑 Лечение — не механизм, а СТРОКА: число без знаменателя это слух. Но чтобы строка не
 * разъехалась с обходом, охват объявлен ОДНИМ местом, из которого выводятся обе вещи — и то,
 * что перепись обходит, и то, что она о себе печатает. Пара «объявление ↔ поведение»
 * проверяется `scopeFaults`: сосчитанный файл вне объявленного охвата — красный.
 */
export const ОХВАТ = {
  корни: ['tools'],
  расширение: '.mjs',
  словами: 'tools/**/*.mjs рекурсивно; sync-server/ и src/ НЕ входят',
};

/**
 * Строка сводки. Вынесена чистой функцией ровно затем, чтобы охват в ней можно было СТЕРЕЧЬ:
 * пропажа знаменателя — это возврат к слуху, и её обязан ловить случай набора, а не глаз.
 */
export function summaryLine(счёт, охват = ОХВАТ) {
  return (
    `Перепись по охвату «${охват.словами}»: файлов ${счёт.файлов} · приборов ${счёт.приборов} · ` +
    `юнитов ${счёт.юнитов} · библиотек ${счёт.библиотек} · ` +
    `объявили ворота ${счёт.воротных} (в составе guards ${счёт.состав}) · непомеченных ${счёт.непомеченных}`
  );
}

/**
 * Вердикт по охвату: сосчитанное обязано лежать внутри объявленного.
 *
 * ⚠️ Проверка сторожит ОДНУ сторону — расползание обхода за объявление (перепись молча начала
 * считать `sync-server`, а строка продолжает обещать `tools/`). Обратную сторону — сужение
 * обхода — сторожит соседний контроль «не найдено ни одного .mjs»: пустая перепись
 * бессодержательна. Обещать здесь обе стороны значило бы обещать больше, чем делается.
 */
export function scopeFaults(paths, охват = ОХВАТ) {
  const faults = [];
  for (const p of paths) {
    const posix = String(p).split(sep).join('/');
    const внутри = охват.корни.some((к) => posix === к || posix.startsWith(`${к}/`));
    if (!внутри) {
      faults.push(`ОХВАТ РАЗОШЁЛСЯ С ОБХОДОМ: сосчитан «${posix}» вне объявленных корней (${охват.корни.join(', ')}) — строка вывода обещает «${охват.словами}»`);
    } else if (!posix.endsWith(охват.расширение)) {
      faults.push(`ОХВАТ РАЗОШЁЛСЯ С ОБХОДОМ: сосчитан «${posix}» — расширение вне объявленного «${охват.расширение}»`);
    }
  }
  return faults;
}
...
const SELFTEST_CASES = [
  ...
  /*
   * ── ОТКАЗ ПИШЕТСЯ ПРОЗОЙ, А ПРОЗА БЕСКОНЕЧНА (находка dev-1 + замер на слитом дереве) ──
   * Четыре пунктуационные формы принёс dev-1; пятую — «пока НЕТ» — нашёл замер живых строк,
   * и она доказывает, что «снять хвостовую пунктуацию» было бы уже дыры. Поэтому опознаётся
   * ЗАЯВКА (команда), а не отказ, и каждый случай ниже сторожит одну сторону этого решения.
   */
  {
    name: '🔴 dev-1: «нет,» «нет.» «нет;» «нет:» — ОТКАЗ, а не заявка (было наоборот)',
    run: () =>
      ['нет, руками', 'нет. руками', 'нет; руками', 'нет: руками'].every((v) => claimsGate(v) === false),
  },
  {
    name: '🔴 ЖИВОЙ ПРОМАХ ДЕРЕВА: «пока НЕТ — по правилу…» — отказ (снятие пунктуации его НЕ берёт)',
    run: () => claimsGate('пока НЕТ — по правилу «в ворота, когда ноль красных»') === false,
  },
  {
    name: 'отказ любой прозой — «не стоит», «вне ворот» — тоже отказ: словарь отказов не ведётся',
    run: () => claimsGate('не стоит — тяжёлый') === false && claimsGate('вне ворот: ходит в сеть') === false,
  },
  {
    name: '🔑 КОНТРОЛЬ: отказ, УПОМИНАЮЩИЙ команду ворот, остаётся отказом',
    run: () => claimsGate('нет — не для `npm run guards`') === false,
  },
  {
    name: 'КОНТРОЛЬ: заявка в обратных кавычках читается заявкой',
    run: () => claimsGate('`npm run guards` (страж дешёвый)') === true,
  },
  {
    name: 'КОНТРОЛЬ: заявка командой `node` — тоже заявка (ворота зовут не только npm)',
    run: () => claimsGate('node tools/x.mjs --gate') === true,
  },
  {
    name: 'КОНТРОЛЬ: регистр не спасает отказ от опознания',
    run: () => claimsGate('НЕТ, руками') === false,
  },
  {
    /*
     * 🔑 Случай существует потому, что МУТАЦИЯ ПОКАЗАЛА: приведение к нижнему регистру ничего не
     * держало. Отказ не команда при любом регистре, и случай выше оставался зелёным без
     * `toLowerCase`. Свойство несёт ровно эта строка — заявка, написанная заглавными.
     */
    name: '🔑 заявка ЗАГЛАВНЫМИ («NPM run guards») — тоже заявка',
    run: () => claimsGate('NPM run guards') === true && claimsGate('NODE tools/x.mjs') === true,
  },
  {
  ...
  /*
   * ── МАРКЕР ОПИСЫВАЕТ СОСТОЯНИЕ, А НЕ НАМЕРЕНИЕ (класс вердикта №26) ─────────────────
   * Формулировки живые: три законных, поминающих мерж, взяты из дерева ДОСЛОВНО, а опасная —
   * та самая, что стояла у `verify-import-safety` до починки. Синтетика здесь не годится:
   * признак и объявлен по этому корпусу, и различает эти строки МЕСТОМ слова, а не словарём.
   */
  {
    name: '🔴 ЖИВОЙ ПРОМАХ: «пока НЕТ — по правилу…» — это НАМЕРЕНИЕ, а не состояние',
    run: () => describesIntent('пока НЕТ — по правилу «в ворота, когда ноль красных»') === true,
  },
  {
    name: '🔴 «пока нет — вписывается после мержа» — намерение (форма из таблицы вердикта №26)',
    run: () => describesIntent('пока нет — вписывается после мержа') === true,
  },
  {
    name: '🔑 КОНТРОЛЬ ЖИВЫМ КОРПУСОМ: заявка, поминающая мерж ЗАДНИМ ЧИСЛОМ, — состояние',
    run: () =>
      describesIntent('`npm run guards` — с окна мержей смены 10. Условие «ноль красных» выполнилось ровно') === false &&
      describesIntent('`npm run guards` — условие «ноль красных на слитом дереве» выполнилось при мерже') === false,
  },
  {
    name: '🔑 КОНТРОЛЬ: честный отказ — состояние, сколько бы прозы за ним ни шло',
    run: () =>
      describesIntent('нет — это ПРИБОР, а не страж: он отвечает на вопрос, а не судит дерево.') === false &&
      describesIntent('нет — и это ЗАПРЕТ, а не пропуск. Прибор ходит по ЖИВОМУ контуру') === false,
  },
  {
    name: 'КОНТРОЛЬ: маркера нет вовсе — намерения тоже нет (молчание судит другая проверка)',
    run: () => describesIntent(null) === false && describesIntent(undefined) === false,
  },
  {
    name: '🔑 КОНТРОЛЬ: намерение В СЕРЕДИНЕ отказа НЕ берётся — граница объявлена честно',
    run: () => describesIntent('нет — встанет после мержа') === false,
  },
  {
    name: '🔴 намерение доезжает до ВЕРДИКТА файла, а не остаётся признаком',
    run: () => {
      const f = { rel: 'verify-c.mjs', kind: 'корневой', run: 'node tools/verify-c.mjs', gate: 'пока нет — после мержа' };
      return judgeFile(f, GATES).some((s) => s.includes('НАМЕРЕНИЕ, а не СОСТОЯНИЕ') && s.includes('ОТСРОЧКА'));
    },
  },

  /*
   * ── ОХВАТ ОБЪЯВЛЕН И СВЕРЯЕТСЯ С ОБХОДОМ (замечание З1 вердикта №20) ────────────────
   */
  {
    name: '🔴 СТРОКА ВЫВОДА НАЗЫВАЕТ ОХВАТ — без знаменателя число это слух (EXP-0226)',
    run: () => {
      const с = summaryLine({ файлов: 273, приборов: 219, юнитов: 20, библиотек: 26, воротных: 11, состав: 11, непомеченных: 0 });
      return с.includes(ОХВАТ.словами) && с.includes('tools/') && с.includes('sync-server') && с.includes('273');
    },
  },
  {
    name: '🔑 КОНТРОЛЬ: файл ВНУТРИ объявленного охвата нареканий не даёт',
    run: () => scopeFaults(['tools/verify-a.mjs', join('tools', 'lib', 'x.mjs')]).length === 0,
  },
  {
    name: '🔴 ОБХОД УШЁЛ ЗА ОБЪЯВЛЕНИЕ: сосчитан sync-server — красный (это и есть спор двух чисел)',
    run: () => scopeFaults(['sync-server/index.mjs']).some((s) => s.includes('ОХВАТ РАЗОШЁЛСЯ')),
  },
  {
    name: 'КОНТРОЛЬ: чужое расширение внутри корня — тоже расхождение охвата',
    run: () => scopeFaults(['tools/x.ts']).some((s) => s.includes('расширение вне объявленного')),
  },
  {
    name: '🔑 КОНТРОЛЬ: «toolsomething» не считается за корень «tools» (граница по разделителю)',
    run: () => scopeFaults(['toolsmith/x.mjs']).some((s) => s.includes('вне объявленных корней')),
  },
];
...
function run(argv) {
  ...
  for (const path of walkTools(TOOLS).sort()) {
    ...
    обойдено.push(relative(ROOT, path));
  ...
  // Объявленный охват сверяется с тем, что реально обошли, — иначе строка вывода обещает своё.
  faults.push(...scopeFaults(обойдено));
  ...
  console.log(
    summaryLine({
      файлов: rows.length,
      приборов: приборы.length,
      юнитов: юниты.length,
      библиотек: библиотеки.length,
      воротных: воротные.length,
      состав: gateFiles.size,
      непомеченных: непомеченные.length,
    }),
  );
