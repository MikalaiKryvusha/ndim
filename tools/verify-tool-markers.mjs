 ...
 *
 * ✅ ПУСТА С 2026-08-29, ОКНО МЕРЖЕЙ, ШАГ 1. Единственная её строка — `verify-role-env.mjs` —
 * умерла ровно так, как обещала её собственная шапка: прибор снят из состава ворот при мерже
 * порции А, и первый же прогон ворот ПОКРАСНЕЛ на пережившей повод отсрочке. Строка снята тем
 * же движением, что и строка ворот.
 *
 * 🔑 Это была БЕСПЛАТНАЯ ПРОВЕРКА ЧЕСТНОСТИ САМОЙ ОТСРОЧКИ, и она её прошла: механизм, который
 * умеет только разрешать, от механизма, который умеет ещё и умирать, отличается ровно этим
 * красным. Пустая карта здесь — не мусор, а доказанное место: следующая отсрочка ляжет в форму,
 * уже показавшую свою смертность.
 */
