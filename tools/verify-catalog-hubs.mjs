...
console.log('\n— сниппет страницы говорит о ней самой, а не о виде целиком —');
{
  ...
  for (const lang of LANGS) {
    ...
    for (const kind of kinds) {
      ...
      for (const file of files) {
        ...
        if (rated > 0) {
          withRated += 1;
          // Обратная сторона: страница С оценёнными обязана называть себя топом. Без этого
          // «лечением» сошло бы снятие слова «топ» отовсюду — то есть обеднение хаба.
          if (!claimsTop) {
            mutedTitle += 1;
            if (!firstLie) firstLie = `${file}: оценённых ${rated} из ${cards}, а «топ» из заголовка ушёл`;
          }
        } else {
          withoutRated += 1;
          if (claimsTop) {
  ...
  check('🔴 та же правда в строке смысла — её видит человек', lyingLede === 0,
    `лгут ${lyingLede}`);
  check('страница С оценёнными по-прежнему называет себя топом (не обеднили)', mutedTitle === 0,
    `страниц с оценёнными ${withRated}, замолчали ${mutedTitle}`);
