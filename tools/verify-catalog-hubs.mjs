...
import { buildFreshness, buildProvenance } from './lib/build-freshness.mjs';
...
/*
 * 🔴 СВЕЖЕСТЬ АРТЕФАКТА СПРАШИВАЕТСЯ ПЕРВОЙ И ОТВЕЧАЕТ ПРО АРТЕФАКТ, А НЕ ПРО ПРОДУКТ.
 *
 * Страж судит `build/` и отвечает на вопрос о ПРОДУКТЕ. Если артефакт несвежий, он отвечает на
 * тот же вопрос про ВЧЕРАШНИЙ продукт — и отвечает убедительно, поимённо, с адресами файлов.
 *
 * **Оплачено 2026-08-30, мержем порции хабов.** Прогон на стволе дал 97/4, все четыре провала
 * ложные: `build/` лежал позавчерашний (18:57) против мержа в 14:40. Строки выглядели так:
 * «`build\ru\catalog\book.html`: оценённых 9 из 60, а заголовок зовёт топом» — то есть как
 * НАХОДКА ПРО СВЕЖУЮ ПОРЦИЮ, а не как поломка прибора. Пересборка начисто → 101/0.
 * 🔑 Красное на несвежем артефакте неотличимо от красного на дефекте, если не посмотреть на
 * время файла. Поэтому время файла теперь смотрит прибор, а не человек.
 *
 * ⛔ Страж НЕ отказывается работать: он краснеет ОТДЕЛЬНОЙ, названной проверкой и печатает её
 * ПЕРВОЙ. Отказ прятал бы настоящие находки, если сборка устарела на минуту; названное красное
 * даёт читателю то, чего ему не хватало, — повод не верить остальным строкам этого прогона.
 */
const свежесть = buildFreshness({ build: BUILD });
console.log(`\n═══ АРТЕФАКТ ═══\n`);
check(
  '🔴 сборка свежее исходников — иначе весь прогон судит ВЧЕРАШНИЙ продукт',
  свежесть.fresh,
  свежесть.why,
);
if (!свежесть.fresh) {
  console.log('     ⚠️ Всё красное ниже читай как СОМНИТЕЛЬНОЕ: оно снято с устаревшего артефакта.');
  console.log('     Лечение одной командой: npm run build');
}

/*
 * ВТОРОЙ ВОПРОС ОБ АРТЕФАКТЕ: из ЭТОГО ли он дерева. Время ловит забытую пересборку; метка ловит
 * переключённую ветку, откат и `amend` — случаи, где время как раз в порядке.
 * ⛔ «Метки нет» и «git молчит» НЕ красные: сборка могла приехать из контура без git, и обвинять
 * её было бы выдумкой. Красное — только доказанное расхождение и грязное дерево.
 */
const происхождение = buildProvenance({ build: BUILD });
check(
  'сборка из ЭТОГО дерева — иначе прогон судит чужой артефакт',
  происхождение.статус === 'своя',
  происхождение.why,
);
if (происхождение.статус === 'нет метки' || происхождение.статус === 'git молчит') {
  console.log('     ⚠️ Это НЕ обвинение: происхождение сборки просто нечем установить.');
}
