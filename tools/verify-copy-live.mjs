 ...
 * 🔴 [NOT-TESTED] СЕТЕВОЙ ПУТЬ `--live` — `readLive()` ни разу не исполнен: боевой запрос отбит
 * предохранителем сессии Интегратора (тот же класс, что запрет `git merge`, — вопрос владельца,
 * не обход). Приговор о расхождении разобран самотестом на подставных числах, само ЧТЕНИЕ боя —
 * нет. Первый живой прогон обязан быть у роли, которой бой открыт, и его результат — условие
 * входа прибора в ворота. Заявлять «прибор проверен» до этого нельзя: это был бы ложный
 * `[TESTED]`, худший из наших классов.
 *
