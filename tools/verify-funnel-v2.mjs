...
/* ── Предполётная проверка: прибор ОТВЕЧАЕТ, а не падает ─────────────────────────
 *
 * 🔴 Класс `bugs/196`, найден судом QA на этом приборе: без свежего `build/` прогон падал
 * СТЕК-ТРЕЙСОМ на `readFileSync('build/sitemap.xml')`. Читающий вывод видит трассировку
 * узла вместо вердикта и не понимает, что виноват он сам, а не продукт, — а прибор,
 * который вместо ответа падает, читается как «сломался продукт».
 *
 * Отказ обязан быть ВЕРДИКТОМ и обязан называть ЛЕЧЕНИЕ одной командой.
 */
for (const [path, cure] of [
  ['build', 'npm run build'],
  ['build/sitemap.xml', 'npm run build'],
]) {
  if (!existsSync(path)) {
    console.error(`\n❌ ПРИБОР НЕ МОЖЕТ СУДИТЬ: нет «${path}» — судить нечего.`);
    console.error(`   Прибор смотрит на СОБРАННЫЙ сайт, а не на исходники. Лечение: ${cure}`);
    process.exit(1);
  }
}
