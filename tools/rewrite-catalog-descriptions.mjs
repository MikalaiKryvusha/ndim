...
/** Слов в тексте — мера, отличающая дробление от прироста содержания (`bugs/190`). */
export const словВТексте = (text) => String(text ?? '').trim().split(/\s+/).filter(Boolean).length;
...
export function journalText(edits, { applied, skipped }) {
  ...
  for (const e of edits) {
    ...
    if (п) {
      /*
       * 🔴 ЯРЛЫК ВЫВОДИТСЯ ИЗ ШТУК И ДВИЖЕНИЯ ЗНАМЕНАТЕЛЯ ВМЕСТЕ (`bugs/190`, суд QA партии 1).
       * Прежний ярлык смотрел на ДОЛЮ: «1 → 1 предложений о создании, всего 14 → 18, доля
       * 7 % → 6 %» печатался как «✅ лучше», хотя ни одного производственного предложения не
       * убыло — доля упала оттого, что одну мысль раздробили на две. Владелец читает ярлыки,
       * не тексты; ярлык «лучше» без убыли производства — неверный ответ на единственный
       * вопрос журнала, и он же лазейка: долю можно опустить, ставя больше точек.
       * Единственное честное «лучше» — мета УПАЛА. Раздутый знаменатель отличается от
       * настоящего прироста содержания числом СЛОВ (дробление их не прибавляет), поэтому
       * слова печатаются рядом с предложениями. Дверь при этом не трогается: дробление
       * законно и заказано владельцем (класс 2 «проще, короче, понятнее»).
       */
      const словДо = словВТексте(e.before);
      const словПосле = словВТексте(e.after);
      const знак = п.метаВыросла ? '🔴 ХУЖЕ'
        : п.после.мета < п.до.мета ? '✅ лучше'
        : п.доляВыросла ? '⚠️ знаменатель сжался'
        : п.после.всего > п.до.всего
          ? `= ровно (знаменатель расширился: ${словПосле > словДо ? 'содержания прибавилось' : 'содержание раздроблено, не добавлено'})`
          : '= ровно';
      lines.push(
