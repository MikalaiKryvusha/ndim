...
test('🔴 реестр протухает четырьмя способами, и каждый называет себя', () => {
  // Исключение, пережившее свой повод, молча разрешает ровно тот дефект, ради которого страж
  // заведён, — и разрешает навсегда. Поэтому у реестра свой суд, и вот его четыре повода.
  ...
  assert.match(суд({ файл: 'tools/нет.mjs', вид: 'исключение', причина: 'была' })
    .протухшие[0].почему, /нет в дереве/);
  assert.match(суд({ файл: 'tools/c.mjs', вид: 'исключение', причина: 'была' })
  ...
  assert.match(суд({ файл: 'tools/a.mjs', вид: 'отсрочка', причина: 'потом' })
...
test('исправная строка покрывает красное и не считается протухшей', () => {
  const r = судитьРеестр({
    реестр: [{ файл: 'tools/a.mjs', вид: 'исключение', причина: 'несущая работа' }],
