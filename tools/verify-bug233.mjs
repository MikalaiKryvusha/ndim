...
try {
  ...
  console.log(`К1 · ссылка открыта в ЧИСТОМ втором браузере (почта ${EMAIL}):`);
  {
    ...
    check('продукт НЕ объявил живую ссылку мёртвой', !dead,
      dead ? `на экране «${DEAD_LINK}»` : '');
    /*
     * ⏳ ЗДЕСЬ ДВА РАЗНЫХ ТРЕБОВАНИЯ, И ОНИ РАЗВЕДЕНЫ НАМЕРЕННО.
     *
     * «Человек не превращён в гостя» — это то, что лечится кодом и обязано быть зелёным уже
     * сегодня: подмена личности тому, кто шёл ВХОДИТЬ, — вред сама по себе.
     * «Вход состоялся» требует спросить у человека почту, то есть НОВОГО экрана двери входа.
     * Это UI на лице продукта — оно идёт владельцу четырьмя макетами, а не сочиняется здесь.
     * Пока его слова нет, эта строка печатается ОЖИДАНИЕМ, а не провалом: провал означал бы,
     * что мы считаем работу проваленной там, где она сознательно не начата.
     */
    check('человек НЕ превращён в анонимного гостя', me.anonymous !== true,
  ...
  console.log('\nК2 · ссылка открыта в браузере, где ТОТ ЖЕ человек уже вошёл:');
  {
    ...
    /*
     * ПОДГОТОВКА ИДЁТ ДВЕРЬЮ ПРОДУКТА, А НЕ В ОБХОД ЕЁ.
     *
     * Первая редакция входила сюда голым `signInWithEmailLink` — и рождала аккаунт БЕЗ
     * документа `users/{uid}`, которого у настоящего человека не бывает: его заводит
     * `ensureSpaceExists` на гостевом пути. Итог — `ProfileMissingError` в консоли, то есть
     * прибор мерил состояние, которого продукт не производит, и красил красным СЕБЯ.
     * Настоящий человек становится аккаунтом так: сначала гость (документ заводится), потом
     * привязка почты по ссылке (`linkWithCredential`, UID и труд сохраняются). Так и здесь.
     *
     * ⚠️ Отдельно записано: «новый человек входит ссылкой, НИ РАЗУ не побыв гостем» — это
     * другой путь, и в нём документа действительно нет. Он заведён отдельным документом
     * бага и здесь СОЗНАТЕЛЬНО не проверяется: один прибор — один предмет.
     */
    await page.goto(`${BASE}/profile?guest=1`);
    await page.waitForTimeout(2500);
    await page.evaluate((e) => {
      localStorage.setItem('ndim-pending-email', e);
      localStorage.setItem('ndim-pending-intent', 'upgrade');
    }, EMAIL);
