...
// Читаем с НОРМАЛИЗАЦИЕЙ переводов строк. Проект живёт на Windows с `core.autocrlf`, и git
// отдаёт файлы с CRLF; проверки же написаны на `\n` (например, фронтматтер скиллов ищется
// как /^---\n…/). Без нормализации гейт краснел на 15 скиллах, у которых фронтматтер был на
// месте, — то есть падал на переводах строк, а не на содержании, и блокировал push
// (2026-07-28). Гейт обязан стеречь СОДЕРЖИМОЕ.
...
const SECRET_PATHS = [
  ...
  /\.(keystore|jks|pem)$/,
  /\.bundle$/,
  // `.env` и любые его варианты (`.env.local`, `.env.production`) — секреты по определению.
  // ЕДИНСТВЕННОЕ исключение — `.env.example`: образец без значений, он обязан быть в git, иначе
  // никто не узнает, какие переменные нужны. Исключение узкое (ровно это имя) и НЕ бесплатное:
  // содержимое образца проверяется ниже, чтобы «образец» не стал способом протащить ключ.
  /(^|\/)\.env($|\.(?!example$))/,
...
try {
  ...
  if (tracked.status === 0) {
    ...
    // ЦЕНА ИСКЛЮЧЕНИЯ ДЛЯ `.env.example`: образец обязан оставаться ОБРАЗЦОМ. Любое присваивание
    // с непустым значением здесь — это секрет, который вот-вот уедет в публичный репозиторий под
    // видом документации. Комментарии (`# NDIM_SA_B64=…`) законны: они объясняют, а не задают.
    const examples = tracked.stdout.split('\n').filter((f) => /(^|\/)\.env\.example$/.test(f));
    for (const file of examples) {
      const filled = readFileSync(file, 'utf8')
        .split(/\r?\n/)
        .filter((line) => /^\s*[A-Za-z_][A-Za-z0-9_]*\s*=\s*\S/.test(line));
      if (filled.length) {
        fail(`${file} содержит ЗАПОЛНЕННЫЕ значения (${filled.length}) — образец обязан быть пустым`);
      } else {
        pass(`${file} — образец без значений`);
      }
    }
  }
