...
for (const cfg of CONFIGS) {
  ...
  /*
   * 🔑 ГОСТЕВАЯ ДВЕРЬ ИЩЕТСЯ ПО КРЮЧКУ, А НЕ ПО ТЕКСТУ (правка 2026-09-04, `plans/80`).
   *
   * Здесь стояло `getByRole('button', { name: /Осмотреться гостем|Look around as a guest/i })` —
   * то есть последние ворота двери выката держались на КОПИРАЙТЕ. В тот час, когда владелец
   * переименовал кнопку («*в кнопку входа анонимом добавь "гостем без регистрации"*»), выкат в
   * стейдж покраснел ЦЕЛИКОМ: смоук не нашёл кнопки, не вошёл, и все следующие шаги посыпались.
   * Красный был честный, но диагноз ложный — продукт был исправен, устарел прибор.
   *
   * Теперь кнопка помечена `data-door="guest"` в `SigninScreen.svelte`, и прибор жмёт крючок.
   * Текст при этом НЕ перестал охраняться: его сверяет побайтово `verify-signin-screen.mjs` (К4).
   * Разделение намеренное: владелец волен переписывать надпись когда угодно, и это не должно
   * ронять выкат — но и подмена надписи не должна проходить молча.
   *
   * Текстовый поиск оставлен ЗАПАСНЫМ: на поверхностях, где крючка ещё нет (старые сборки,
   * лендинговая дверь), прибор по-прежнему найдёт кнопку по слову.
   */
  const guestByHook = page.locator('[data-door="guest"]');
  const guestBtn = (await guestByHook.count()) > 0
    ? guestByHook
    : page.getByRole('button', { name: /гостем без регистрации|as a guest/i });
