...
const CASES = [
  ...
  /*
   * ТРЕТЬЕ СОСТОЯНИЕ ВЕЛИЧИНЫ — «все поставили ноль» (`plans/56` шаг 1). Случай добавлен
   * потому, что именно ради него дверь научилась третьему тексту, а глазами он не смотрелся
   * ни разу: в запасном срезе таких записей нет вовсе, в полном каталоге их два из 5121.
   * Случайная выборка сюда не попадает, поэтому случай назван условием, а не слагом.
   */
  pick((d) => d.rates > 0 && d.rating === 0, 'все поставили ноль'),
