...
/**
 * Длительность перехода `slide` — МЕХАНИЗМ: столько едут ряд смайликов и строка сохранения.
 * Названа константой, чтобы длина окна сравнивалась с ИМЕНЕМ, а не с голым числом в двух местах.
 */
const SLIDE_МС = 240;

/**
 * ⚠️ ЗАМЕР, А НЕ МЕХАНИЗМ — и поэтому названо замером, а не порогом (условие У4 суда №33).
 *
 * 2026-08-29, слот 1: в окне 215 мс ряд смайликов дошёл до нулевой высоты на +182 мс от нуля
 * отсчёта. **Вывести это число из `SLIDE_МС` НЕЛЬЗЯ:** переход начался РАНЬШЕ нуля отсчёта, и
 * насколько раньше — свойство того прогона (вёрстка, машина, загрузка), а не кода. На другой
 * территории миг сдвинется.
 *
 * Прежняя редакция ставила голый литерал `182` в роли ПОРОГА — то есть утверждала про ЧУЖОЙ
 * прогон то, чего в нём не наблюдали. Это ровно тот класс, который сам этот прибор и лечил,
 * когда снимал константу «доказаны ДВЕ проверки»: число, верное для своего замера, становится
 * тихой неправдой на первом же прогоне с другим окном.
 * Поэтому число осталось, а роль сменилась: строка отчёта НАЗЫВАЕТ его источник вслух, и
 * читающий видит, на чём стоит вывод, вместо того чтобы верить порогу.
 */
const РЯД_УЕХАЛ_ЗАМЕР = { мс: 182, окноМс: 215, дата: '2026-08-29' };
  ...
  if (живые.length === 0) {
    ...
    say(
      `окно после нуля отсчёта: ${послеНуля.length} кадров (${хвостМс} мс) до смены объекта. ` +
        (хвостМс < SLIDE_МС
          ? `⚠️ Это КОРОЧЕ перехода \`slide\` (${SLIDE_МС} мс): проверки «ряд смайликов» и «кнопка» ` +
            'на этой территории покраснеть не успевают — их зелёный доказывает лишь отсутствие ' +
            'мгновенной обрезки. Мутацией доказаны ' +
            (хвостМс >= РЯД_УЕХАЛ_ЗАМЕР.мс
              ? 'ТРИ проверки из четырёх (звёзды, выделение и ряд смайликов). ' +
                `⚠️ Третья — по ЗАМЕРУ, а не по механизму: ${РЯД_УЕХАЛ_ЗАМЕР.дата}, в окне ` +
                `${РЯД_УЕХАЛ_ЗАМЕР.окноМс} мс ряд уехал на +${РЯД_УЕХАЛ_ЗАМЕР.мс} мс от нуля отсчёта. ` +
                'Это ОДНО наблюдение: на другой машине миг сдвинется, и тогда третья проверка ' +
                'снова окажется недоказанной. Перемерь, если число решает.'
