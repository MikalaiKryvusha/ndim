 ...
 *
 * ДОКАЗАН МУТАЦИЕЙ 2026-08-22 — снятием самой починки (`git stash` правки страницы), адресаты
 * названы ДО прогона. Покраснели ДВЕ проверки из четырёх, в обеих проверенных конфигурациях:
 * звёзды 8 → 0 и выделение 1 → 0 на +0 мс от нуля отсчёта (10 и 8 кадров соответственно).
...
try {
  ...
  if (живые.length === 0) {
     ...
     * а не прибора. Мутация (снятие починки) покраснила ДВЕ из четырёх: звёзды и выделение гаснут
     * мгновенно, а ряд смайликов и строка сохранения уезжают переходом `slide` за 240 мс — и
     * объект успевает СМЕНИТЬСЯ раньше, чем они исчезнут. То есть на сломанном коде две последние
     * проверки не краснеют не потому, что там всё цело, а потому, что окно кончается первым.
     *
      ...
      `окно после нуля отсчёта: ${послеНуля.length} кадров (${хвостМс} мс) до смены объекта. ` +
        (хвостМс < 240
          ? '⚠️ Это КОРОЧЕ перехода `slide` (240 мс): проверки «ряд смайликов» и «кнопка» ' +
            ...
            'мгновенной обрезки. Мутацией доказаны ДВЕ проверки из четырёх (звёзды и выделение).'
