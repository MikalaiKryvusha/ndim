 ...
 * четвёртому.
 *
 * 🔴 КАПКАН ПОРТА 9100 — ЗАНЯТОСТЬ СУДИТСЯ ПО `OwningProcess` КОНКРЕТНОГО ПОРТА, НЕ ПО ДИАПАЗОНУ.
 *
 * Порт `9100` держит `lghub_updater` (Logitech, `C:\Program Files\LGHUB\lghub_updater.exe`) —
 * посторонний процесс, к стенду отношения не имеющий. Наш auth стоит рядом и мимо: слот 0 —
 * `9099`, слот 1 — `9109`. То есть 9100 выглядит круглым портом следующего слота и не является
 * им ни на одном слоте.
 *
 * Отсюда правило суда, а не список исключений: **«занято ли» спрашивают у ПОРТА, а не у
 * диапазона.** Проба (PowerShell):
 *
 *     Get-NetTCPConnection -LocalPort <порт> -State Listen |
 *       ForEach-Object { Get-Process -Id $_.OwningProcess }
 *
 * Кто судит занятость круглым числом или диапазоном `91xx`, получает ложное «занято» на чужом
 * процессе — и идёт искать несуществующий сиротский эмулятор либо, что дороже, гасить чужое
 * (гашение только своих и адресно, `EXP-0131`: на машине владельца живут чужие агентские
 * процессы).
 *
 * ⛔ Чинить здесь нечего, и это сказано прямо, чтобы никто не «починил» работающее: сетка капкан
 * УЖЕ обходит — `MACHINE_BUSY_PORTS` ниже несёт 9100, а `SLOT_STEP` разводит слоты через десять.
 * Дефекта поведения нет; недоставало ровно этого предупреждения, и стоит оно здесь потому, что
 * решение «занято ли» принимают в ЭТОМ файле, а досье окружения читают не в тот момент.
 *
 * Родня — строка про порт 8080 и `llama-swap` в досье окружения (`AGENT_GUIDE.md` → «Досье
 * окружения», причуды, оплаченные инцидентами): один класс, разные держатели.
 * Найдено Интегратором 2026-08-29 при опознании связок стенда; проба повторена 2026-08-29
 * 20:26 +03:00 — PID 6612, `lghub_updater`.
 *
...
/**
 * СЛОТ ПО ЗАПРОСУ — флаг, переменная окружения или рабочий каталог, в этом порядке.
 *
 * 🔴 ЗАЧЕМ ЭТО ЕСТЬ (эскалация судьи, вердикт №41). Стендовые приборы выводили слот ТОЛЬКО из
 * имени рабочего каталога. А судья по чек-листу обязан гонять чужой код ВРЕМЕННЫМ деревом — и
 * любое такое имя (`_court_qa_47`, клон, копия) даёт `slotOf` → **слот 0, слот главной копии,
 * где стоит дверь выката**. Итог: **роль, чья работа — перепрогонять чужие проверки,
 * СТРУКТУРНО не могла перепрогнать зависящие от стенда.** Стенд она не поднимала, и половина
 * доказательств принималась заявкой, а не проверкой.
 *
 * 🔑 Это тот же класс, ради которого заведён страж формы адреса, — «прибор целится в чужой
 * слот», — только этажом выше: там мимо целился прибор, здесь мимо целится СУДЬЯ.
 * У `stand-launch.mjs` флаг `--slot` уже был; у приборов его не было, и это половина решения,
 * прожившая как целое.
 *
 * ⚠️ ПОРЯДОК И ЕГО ИСТОЧНИК ВОЗВРАЩАЮТСЯ НАРУЖУ НАМЕРЕННО. Прибор обязан ПЕЧАТАТЬ, откуда взял
 * слот: молчаливый выбор — ровно то, из-за чего класс и живёт. «Слот 5 · из каталога» и
 * «слот 2 · по флагу --slot» читаются по-разному, а без подписи одинаково.
 *
 * @param {{argv?: string[], env?: Record<string,string|undefined>, dirName: string}} запрос
 * @returns {{slot:number, role:string|null, note:string|null, источник:'флаг'|'переменная'|'каталог'}}
 */
export function slotFromRequest({ argv = [], env = {}, dirName }) {
  const i = argv.indexOf('--slot');
  if (i >= 0) {
    const n = Number(argv[i + 1]);
    if (!SLOTS.includes(n)) {
      throw new Error(`--slot ${argv[i + 1]}: слота нет, разведены ${SLOTS.join(', ')}`);
    }
    return { slot: n, role: ROLES[n] ?? null, note: null, источник: 'флаг' };
  }
  if (env.STAND_SLOT !== undefined && env.STAND_SLOT !== '') {
    const n = Number(env.STAND_SLOT);
    if (!SLOTS.includes(n)) {
      throw new Error(`STAND_SLOT=${env.STAND_SLOT}: слота нет, разведены ${SLOTS.join(', ')}`);
    }
    return { slot: n, role: ROLES[n] ?? null, note: null, источник: 'переменная' };
  }
  return { ...slotOf(dirName), источник: 'каталог' };
}
