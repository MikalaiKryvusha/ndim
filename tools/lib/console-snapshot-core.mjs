...
/**
 * ГОДНОСТЬ СРЕЗА — сверка его суммы с эталоном (`bugs/232`).
 *
 * 🔴 ЗАЧЕМ ЭТО ЕСТЬ, И ПОЧЕМУ БЕЗ НЕГО ПРИБОР ВРЁТ ПРАВДОПОДОБНО. Search Console отдаёт РАЗНЫЕ
 * итоги на разных размерностях, и это законно с его стороны: при мелком группировании строки
 * ниже порога конфиденциальности отбрасываются ВМЕСТЕ С ИХ КЛИКАМИ. Замер 2026-09-12 по одному
 * окну (01.07–11.09) семью разрезами:
 *
 *   без размерностей · date · device · date×device → 9 514 показов, 40 кликов  (эталон)
 *   page                                           → 11 370 показов, 41 клик   (годен)
 *   query                                          →  5 342 показов,  5 кликов (🔴 12 %)
 *   device×page                                    →  6 810 показов,  5 кликов (🔴 12 %)
 *
 * Показы по `page` ВЫШЕ эталона — так и должно: постраничная агрегация считает показ на каждую
 * показанную страницу, эталон — один показ на появление сайта. Дефект не в этом, а в потере
 * кликов на перекрёстных разрезах.
 *
 * ⛔ Цена молчания названа: раздел «полосы по устройствам» напечатал «полоса 1–10 — 762 показа,
 * 0 кликов», и это правдоподобное ложное число за час дошло до владельца и до `MASTER_PLAN.md`.
 *
 * Возвращает долю удержанного и вердикт годности. Порог кликов — 0,9: ниже него срез не отвечает
 * ни на один вопрос о кликах и CTR.
 */
export function sliceIntegrity(rows, reference) {
	const own = totals(rows);
	const refClicks = reference?.clicks ?? 0;
	const refImpressions = reference?.impressions ?? 0;
	const clicksKept = refClicks > 0 ? own.clicks / refClicks : null;
	const impressionsKept = refImpressions > 0 ? own.impressions / refImpressions : null;
	// Показы выше эталона законны (постраничная агрегация); клики выше эталона — тоже (тот же счёт).
	const fitForClicks = clicksKept === null ? false : clicksKept >= 0.9;
	return {
		clicks: own.clicks,
		...
		fitForClicks,
		note: fitForClicks
			? 'срез удерживает клики — годен для CTR и выводов о кликах'
			: `срез держит ${own.clicks} кликов из ${refClicks} — ⛔ НЕ годен для CTR и выводов о кликах`,
	};
}
 ...
 *
 * 🔑 ЗАЧЕМ ЭТО БЫЛО ЗАДУМАНО. Снимок 2026-08-28 показал разрыв CTR десктоп/мобильный в 19 раз
 * (0,19 % против 3,62 %). Но у этих же устройств разные средние позиции — 61,6 против 29,3, —
 * и потому разрыв объясняется ДВУМЯ несовместимыми способами: либо десктопные показы просто
 * глубже (виновата позиция), либо страница хуже работает на широком экране (вопрос к вёрстке).
 * Различить их можно только сравнением устройств ВНУТРИ одной полосы позиций.
 *
 * 🔴🔴 ЗАМЫСЕЛ ВЕРЕН, НО ЭТИМИ ДАННЫМИ ОН НЕИСПОЛНИМ — замерено 2026-09-12 (`bugs/232`).
 * Как только `page` соединяется с `device`, Search Console теряет 87 % кликов, и ЛЕЧЕНИЯ НЕТ:
 * фильтр вместо группировки не спасает (проверено — десктоп 1 клик вместо 15, мобилка 4 вместо 25).
 * Значит **гипотеза Г-2 этим разрезом непроверяема в принципе**, и функция ниже годится ТОЛЬКО
 * для показов: «на каком устройстве какие страницы вообще показывались и на какой глубине».
 *
 * ⛔ НЕ читать отсюда клики и CTR. Кто позовёт — получит рядом вердикт `sliceIntegrity`.
 * 📌 Чем Г-2 закрывать вместо этого — открытый вопрос; `date×device` удерживает все клики и даёт
 * устройство + среднюю позицию по дням, но не CTR внутри полосы.
 *
