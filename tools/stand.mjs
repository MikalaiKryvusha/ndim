 ...
 *
 * 🅿 ПОРТ И `--strictPort` ЗДЕСЬ БОЛЬШЕ НЕ НАЗЫВАЮТСЯ — их даёт `vite.config.ts` (фаза 2
 * тестового парка). Требования не отменились, они переехали: порт слота разводит роли, а
 * `--strictPort` роняет запуск при занятом порту вместо тихого переезда на соседний
 * (`EXP-0091`: роль смотрит ЧУЖОЙ сайт и идёт искать дефект в исправном продукте).
 *
 * 🔑 Почему переехали. Пока порт называла эта строка, знание слота лежало в ДВУХ местах: здесь
 * и в конфиге — а второе место расходится с первым в день рождения (`AGENT_GUIDE` → «Реестр
 * пар»). И главное: `vite preview` эта строка не касалась вовсе, поэтому собранный сайт роли
 * всё равно вставал на общий порт. Конфиг знает слот один раз и для всех входов сразу — в том
 * числе для тех, которые поднимает Playwright.
 */
