...
async function main() {
  ...
  for (let round = 1; round <= ROUNDS; round += 1) {
    // Время от начала замера печатаем не для красоты: минутную свежесть сохранил ПУЛЬС сервера
    // (`FRESH.pulse`), и без часов невозможно отличить «кэш не сработал» от «пульс честно
    // перечитан». Цифры (топ, статистика) с решения владельца живут всю сессию — интервью №005, В4.
  ...
  console.log(
    `  ⚠️ Что ещё может честно читать базу на тёплых кругах (интервью №005, В4):\n` +
      `     · ПУЛЬС сервера синхронизации — 1 документ раз в минуту, «Пространство»/«Меню».\n` +
      `       Он единственный остался минутным: из него выводится лампочка «Работает», и\n` +
      `       заморозь её на сессию — живой сервер показался бы умершим.\n` +
      `     · ПЕРВЫЙ НАСТОЯЩИЙ заход на стартовый экран прогона (/profile): клик по вкладке,\n` +
      `       на которой браузер уже стоит, навигацией не является (EXP-0072).\n` +
      `     Цифры — мой топ и статистика Пространства — внутри сеанса не перечитываются вовсе.\n`,
  );
