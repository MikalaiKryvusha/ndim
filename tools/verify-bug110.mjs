...
try {
	...
	head('0. мера 1: «терпение бесконечно» — умолчание без часов');
	{
		...
		check(!/до \d+ мин/.test(out0), 'МЕРА 1: срока владельцу не назначается ни словом');
		try { child0.kill(); } catch {}
		await sleep(300);
	}

	// ── 0а. БЛИЗНЕЦ: у пачки СВОИ часы, и они тоже обязаны быть сняты ──────────
	// Первая редакция фикса правила только одиночную страницу: у пачки другое сообщение в логе,
	// и правка по тексту его не нашла. С умолчанием 0 это стало хуже, чем было: `setTimeout(…, 0)`
	// срабатывает немедленно, и страница пачки закрылась бы сразу после открытия.
	head('0а. близнец: страница ПАЧКИ живёт с тем же умолчанием');
	{
		const child1 = spawn(
			process.execPath,
			[join(ROOT, 'tools', 'review.mjs'), 'batch', '--no-open', '--no-signal', '--port', String(PORT + 2)],
			{ cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'] },
		);
		child1.stdout.setEncoding('utf8');
		let out1 = '';
		child1.stdout.on('data', (c) => (out1 += c));
		await sleep(4000);
		const batchRaised = /Пачка поднята|накопилось|очередь пуста/i.test(out1);
		check(batchRaised, 'пачка вообще поднялась (иначе проверка ниже ничего не значит)');
		check(!batchRaised || alive(child1) || /очередь пуста/i.test(out1),
			'БЛИЗНЕЦ: страница пачки не закрылась сама через мгновение');
