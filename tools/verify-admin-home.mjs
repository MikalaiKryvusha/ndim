...
/*
 * 🔄 ПОДПИСЬ ДВЕРИ СМЕНИЛАСЬ ПО СЛОВУ ВЛАДЕЛЬЦА (интервью №035, 2026-08-16): раздел называется
 * «Панель администратора», а «Менеджер измерений» — один из инструментов ВНУТРИ него. Страж
 * приведён к новой правде, а не ослаблен: он по-прежнему требует ТОЧНУЮ подпись двери, просто
 * подпись теперь другая (класс `EXP-0056` — страж гниёт не только от переименований, но и от
 * смены канона; лечение — привести к правде со ссылкой на решение).
 */
const DOOR_LABEL = 'Панель администратора';
/** Инструмент внутри панели — он обязан быть НАЗВАН на экране, иначе иерархия потеряна. */
const TOOL_LABEL = 'Менеджер измерений';
...
try {
  ...
  for (const [theme, width] of [['light', 390], ['dark', 1440]]) {
    ...
    // 🆕 Иерархия, заданная владельцем: раздел ОБЩИЙ, инструмент — внутри. Проверяем ОБА имени:
    // страж, знающий только заголовок, был бы зелёным и на прежней плоской редакции.
    check(text.includes(TOOL_LABEL), `оболочка (${tag}): инструмент «${TOOL_LABEL}» назван внутри панели`);
