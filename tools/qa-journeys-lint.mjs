 ...
 * Ворота: npm run guards (страж дешёвый — ни сети, ни стенда, ни сборки; в состав вписан
 *         окном мержей смены 10 — до того файл лежал в main, а ворота его не звали, EXP-0194).
...
/**
 * Потолок Smoke — ГРУБЫЙ ПРЕДОХРАНИТЕЛЬ, а не мера.
 *
 * Настоящая мера — минуты прогона (окно 10 минут, критерий К3 эпика), и её меряют прогоном, а не
 * счётом строк. Число здесь стоит только затем, чтобы набор не разрастался незаметно между
 * замерами. Владелец продиктовал скоуп Smoke сам («самая базовая база»), и он вышел шире первой
 * редакции карты — потолок поднят с 15 до 20 вслед за его словом, а не наоборот.
 */
const SMOKE_CAP = 20;
