 ...
 *        оплачен дважды.
 *
 * 🔲 КАНДИДАТ-СЛУЧАЙ, НАЗВАННЫЙ ЧЕСТНО И ПОКА НЕ ВЗЯТЫЙ: П1 судит ОДНО направление —
 *    «шапка объявляет вылеченным, а имени `_DONE_` нет». ОБРАТНОЕ — имя `_DONE_`, а шапка
 *    говорит «🔴 ОТКРЫТ» — мимо признака, и это тоже самопротиворечие: документ спорит сам с
 *    собой ровно так же, только другой стороной. Пойман на `bugs/78` при вычистке хвоста
 *    (смена 10). Замер ширины сразу же, чтобы кандидат не был догадкой: таких документов в
 *    корпусе СЕМЬ — 75, 76, 77, 79, 82, 86, 87, и все из одной волны `ideas/21` (июль),
 *    то есть закрывали их переименованием пачкой, а шапки не догнали ни у одного.
 *    Расширение не срочно (слово Менеджера, смена 10): направление безопаснее прежнего —
 *    закрытый документ никого никуда не ведёт, — но лечится оно семью правками, а не строкой
 *    кода, и потому идёт отдельной порцией, а не прицепом к страже.
 *
