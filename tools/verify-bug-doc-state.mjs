 ...
 *
 * Ворота:   `npm run guards` — с окна мержей смены 10. Условие «ноль красных» выполнилось ровно
 *           на слитом дереве: `ndim_integrator` в одиночку давал 4 красных (П2), `main` — 14
 *           (П1), а их мерж — ноль. Ветки лечили друг друга; поодиночке красны были обе.
 * Код возврата: 0 — рассогласований нет · 1 — есть П1/П2/П3.
 *
...
function main(argv) {
	...
	if (p2.length) {
		/*
		 * Замечание судьи (вердикт №23): в шапке прибора это сказано, а в ВЫВОДЕ не было — и
		 * первый читатель на своей ветке заводил бы несуществующие долги. Класс `EXP-0226`:
		 * число без контура — слух. Строка стоит рядом с числом, а не в документации.
		 */
		console.log(
			'   ⚠️ Число П2 ЗАВИСИТ ОТ ДЕРЕВА: усыновляющая строка может жить в `STATUS.md` другой\n' +
				'      ветки. Прежде чем заводить долг — сверь с `main`:\n' +
				'      node tools/verify-bug-doc-state.mjs --tree main',
		);
	}
