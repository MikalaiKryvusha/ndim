...
test('🔴 ПЕРЕДАЧА флага чужому прибору носителем НЕ делает', () => {
  // Оплачено ложным красным на `tools/lib/selftest-pair.mjs` (помощник dev-1): он лишь передаёт
  // флаг в spawnSync чужому прибору, своего самотеста не имеет. Признак «есть подстрока»
  // потребовал бы пару к файлу, которому она не нужна.
  assert.ok(!несётСамотест("spawnSync(process.execPath, [путь, '--selftest', ...argv]);"));
  assert.ok(несётСамотест("if (process.argv.includes('--selftest')) selftest();"), 'а разбор — делает');
  assert.ok(несётСамотест("if (flag('--selftest')) selftest();"), 'и разбор через помощник тоже');
});
