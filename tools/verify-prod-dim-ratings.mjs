...
try {
  ...
  // До починки разрыв был 31 и рос бы вечно. Остаток объясняется только сиротами; порог взят
  // с запасом, чтобы страж краснел на возврате дефекта, а не на каждой новой оценке-сироте.
  check('расхождение объяснимо сиротами, а не сломанным механизмом', gap <= 60,
    `${gap} (до починки было 31 и росло)`);
} finally {
