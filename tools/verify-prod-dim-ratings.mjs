 ...
 * некуда. Такие оценки — ЕДИНСТВЕННЫЙ законный источник расхождения, и сервер синхронизации объявляет
...
try {
   ...
   * проход ещё не прошёл — до суток. Если прибор покраснел, СНАЧАЛА посмотри в лог сервера синхронизации
  ...
  check('🎯 расхождение РОВНО НОЛЬ', gap === 0,
    gap === 0 ? 'счётчики сошлись точно' : `${gap} — проверь строку «сироты:» в логе сервера синхронизации`);
} finally {
