/**
 * Сверка контура после выката В4 (интервью №005) — живым браузером, гостем.
 *
 * Что проверяет (гостю доступно ровно это):
 *  · страницы РИСУЮТСЯ пикселями, а не «отдают 200» — проверяется высотой контента;
 *  · выкаченный чанк несёт новый контракт свежести {mine:1/0,server:1/0,pulse:6e4};
 ...
 *
 * Запуск: `node tools/verify-prod-b4.mjs [--base https://ndim-stage.web.app]`
 * По умолчанию — бой. Стейдж (`plans/53` фаза 4) проверяется ТЕМ ЖЕ прибором: два набора проверок
 * разъехались бы, и стейдж перестал бы проверять то, что уезжает в бой.
 */
...
// Кадры разведены ПО КОНТУРАМ: общая папка означала бы, что прогон стейджа затирает кадры боя и
// пары «до/после» не существует — уже оплаченный урок (`measure-bug117`, флаг `--label`).
const OUT = `test-results/prod-b4${CONTOUR.name === 'prod' ? '' : '-' + CONTOUR.name}`;
const SCREENS = [
  { path: '/', name: 'landing' },
  { path: '/menu', name: 'menu' },
