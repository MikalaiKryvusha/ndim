...
try {
  for (const [theme, width] of [['light', 390], ['dark', 1440]]) {
    ...
    // врезку подтверждения, выходит ВТОРОЙ. Этот страж поймал изменение сам — «признак сессии
    // снят» покраснел ровно на том, что сессия и не должна была сниматься с одного тапа.
    // Проверяем обе ступени: врезка обязана появиться, и только потом выход.
    //
    // Ищем врезку ПО КОНСТРУКЦИИ (`.awarn` + её кнопки), а не по тексту: текст врезки — живое
    // слово владельца, он переписал его 2026-09-05 («Выйти? Вы пользуетесь временным гостевым
    // аккаунтом…»), и страж, привязанный к формулировке, покраснел бы на копирайте вместо
    // дефекта — тот самый класс, которым уже падал смоук двери выката. Побайтово текст сверяет
    // другой прибор; здесь предмет — что подтверждение ЕСТЬ и что до него человек не вышел.
    await page.locator('button.arow', { hasText: /Выйти/ }).first().click();
    await page.waitForTimeout(600);
    check(await page.locator('.awarn .awarn-cta').first().isVisible().catch(() => false),
