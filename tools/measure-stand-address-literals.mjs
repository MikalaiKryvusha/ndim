#!/usr/bin/env node
/**
 * ПРИБОР ЗАМЕРА — литералы адреса стенда в приборах (`bugs/231`). НЕ страж.
 *
 ...
 * поимённо (критерии приёмки — `bugs/231`).
 *
...
console.log('═══ ЛИТЕРАЛЫ АДРЕСА СТЕНДА В ПРИБОРАХ (bugs/231) ═══\n');
...
console.log(
  `\n⚠️ «Пишет» — эвристика по именам методов. Прежде чем звать это дефектом, прочитай файл:` +
    ` запись может идти не в стенд. Разбор и критерии приёмки — bugs/231.`,
);
