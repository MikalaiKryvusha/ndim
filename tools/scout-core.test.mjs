...
import {
  ...
  отказВладельца, ОТКАЗ_ВЛАДЕЛЬЦА, корзинаОбъекта,
...
/**
 * ── ПРИМЕНЕНИЕ ПРАВИЛА, А НЕ САМО ПРАВИЛО (условие вердикта №39, дефект Д1) ────────────────
 *
 * Судья показал дыру точно: правило `отказВладельца` было закрыто шестью юнитами и вшитой
 * мутацией, а ВОСЕМЬ СТРОК, применявших его в приборе разведки, не покрывал ни один тест.
 * Убери `continue` — отклонённый владельцем объект поедет в партию, и зелёными останутся все
 * шесть случаев, `test:tools` и `guards`. Цена настоящая: партия уходит на вычитку ВЛАДЕЛЬЦУ.
 *
 * 🔑 Блок стережёт не ветку, а ПОРЯДОК: отказ сильнее отсутствия имени и сильнее каталога.
 * Порядок раньше жил только в последовательности строк прибора — то есть нигде.
 */

test('🔴🔴 ПРИМЕНЕНИЕ: отклонённый владельцем объект не попадает НИ В ОДНУ рабочую корзину', () => {
  for (const метка of [null, { ru: 'Моя борьба', en: 'Mein Kampf' }]) {
    for (const вКаталоге of [() => true, () => false]) {
      const р = корзинаОбъекта({ qid: 'Q48244', метка, вКаталоге });
      assert.equal(р.корзина, 'отклонён', 'отказ владельца сильнее и метки, и каталога');
      assert.match(р.причина, /продвижени/u, 'причина едет вместе с решением, а не теряется');
    }
  }
});

test('🔴 ПОРЯДОК: для отклонённого сверка с каталогом НЕ ЗОВЁТСЯ ВОВСЕ', () => {
  // Счётчик вызовов, а не чтение строк: порядок доказывается наблюдением, а не глазом.
  let звали = 0;
  const вКаталоге = () => { звали += 1; return true; };
  корзинаОбъекта({ qid: 'Q41675', метка: { ru: 'Книга рекордов', en: 'Guinness' }, вКаталоге });
  assert.equal(звали, 0, 'отказ адресный: он решается одним QID и каталога не касается');
});

test('корзины остальных трёх случаев названы верно', () => {
  const нет = () => false;
  const да = () => true;
  assert.equal(корзинаОбъекта({ qid: 'Q480', метка: null, вКаталоге: да }).корзина, 'безымянный',
    'имя не добыто — сверять нечем, и объект НЕ объявляется отсутствующим');
  assert.equal(корзинаОбъекта({ qid: 'Q480', метка: { ru: 'Дон Кихот' }, вКаталоге: да }).корзина, 'наш');
  assert.equal(корзинаОбъекта({ qid: 'Q480', метка: { ru: 'Дон Кихот' }, вКаталоге: нет }).корзина, 'новый');
});

test('🔒 сверка получает ТОЛЬКО непустые имена — пустая метка не ищется в каталоге', () => {
  // Пустое имя, отданное в каталог, совпало бы с чем угодно нормализацией — тот же класс,
  // что «пустая метка в первой редакции прибора».
  let полученные = null;
  корзинаОбъекта({
    qid: 'Q480',
    метка: { ru: 'Дон Кихот', en: null },
    вКаталоге: (имена) => { полученные = имена; return false; },
  });
  assert.deepEqual(полученные, ['Дон Кихот']);
});
