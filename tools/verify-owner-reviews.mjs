...
import { textHash, decisionPath, ARCHIVE_DIR, selftest, mdToHtml } from './lib/review-core.mjs';
...
writeFileSync(
	...
	[
		...
		// Машинная метка в фикстуре стоит НАМЕРЕННО: до `bugs/112` она приезжала владельцу на
		// страницу видимым мусором «&lt;!-- owner-review: … --&gt;», потому что разбор клал её в
		// текст ответа, а `mdToHtml` экранирует HTML. Проверки ниже стерегут, чтобы не вернулось.
...
// ── `bugs/112`, корень дефекта: HTML-комментарий обязан быть НЕВИДИМ ──────────────────────────
// Проверяется на самой функции, а не через страницу: мест утечки было два (под ответом и под
// общим комментарием документа), и оба лечит одна строка в `mdToHtml`. Страж через страницу
// накрыл бы только первое.
{
	const mark = '<!-- owner-review: by="Николай Кривуша" at="2026-08-02T17:51:49.003Z" -->';
	check('HTML-комментарий не показывается человеку', !mdToHtml(`ответ\n${mark}`).includes('owner-review:'));
	check('комментарий В СЕРЕДИНЕ строки тоже вырезается', !mdToHtml(`до ${mark} после`).includes('owner-review:'));
	check('текст вокруг комментария уцелел', /до\s+после/.test(mdToHtml(`до ${mark} после`)));
	check(
		'внутри ограждённого кода комментарий ОСТАЁТСЯ (там он содержание)',
		mdToHtml('```html\n<!-- пример -->\n```').includes('&lt;!-- пример --&gt;'),
	);
}
...
for (const cfg of CONFIGS) {
	...
	// ── `bugs/112`: время ответа — человеку, машинная метка — машинам ───────────────────────
	const pageText = await page.locator('body').innerText();
	// Ищем СИНТАКСИС метки (`owner-review:` с двоеточием), а не слово: путь страницы содержит
	// каталог `test-results/owner-reviews/`, и проверка на голое слово краснела на пустом месте.
	const leakAt = pageText.indexOf('owner-review:');
	check(
		`${tag} машинная метка НЕ протекает на страницу`,
		leakAt < 0,
		leakAt < 0 ? '' : `…${pageText.slice(Math.max(0, leakAt - 70), leakAt + 40).replace(/\s+/g, ' ')}…`,
	);
	const whenText = (await page.locator('[data-q="В2"] .when').count())
		? (await page.locator('[data-q="В2"] .when').innerText()).trim()
		: '';
	check(`${tag} у отвеченного вопроса показано, КОГДА он сохранён`, whenText.length > 0, whenText);
	check(
		`${tag} время показано человеку, а не ISO-строкой`,
		/\d{1,2}:\d{2}/.test(whenText) && !/T\d{2}:\d{2}:\d{2}/.test(whenText),
		whenText,
	);
...
// `bugs/112`: провенанс существовал всегда, но ТОЛЬКО машинный. Владелец 2026-08-03: «хочу про
// время, чтобы ответы имели таймстемп» — потому что видел не время, а комментарий с ISO-строкой.
// 🔴 Проверяем ПРИВЯЗКУ отметки к своему ответу, а не её наличие где-нибудь в файле: первая
// редакция этого стража смотрела на весь документ и мутацию «отметку не пишем» НЕ ловила —
// её закрывала отметка соседнего вопроса (`TESTING_FRAMEWORK`: зелёный без мутации не считается).
check(
	'МЕСТО 1 — ВИДИМАЯ отметка стоит СРАЗУ ПОД своим ответом',
	/\*\*Ответ:\*\*[^\n]*\n\*🕒 Сохранено [^\n]* · Николай Кривуша\*/.test(md),
);
check(
	'МЕСТО 1 — отметку получил КАЖДЫЙ сохранённый ответ',
	(md.match(/\*🕒 Сохранено /g) ?? []).length >= 2,
	`нашлось ${(md.match(/\*🕒 Сохранено /g) ?? []).length}`,
);
check(
	'ВИДИМАЯ отметка — в МЕСТНОМ времени, а не UTC',
	!/\*🕒 Сохранено \d{4}-\d{2}-\d{2}T/.test(md) && /\*🕒 Сохранено .*\d{1,2}:\d{2}/.test(md),
	(md.match(/\*🕒 Сохранено [^*]+\*/) ?? [''])[0],
);
