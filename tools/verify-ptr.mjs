...
async function run() {
  ...
  for (const theme of ['light', 'dark']) {
    for (const width of [390, 1440]) {
      ...
      // ── Строка «Обновить данные» в «Меню» ───────────────────────────────────────────────
      await navigate(page, '/menu');
      const row = page.locator('button.row', { hasText: ROW_TEXT });
      await row.waitFor({ state: 'visible', timeout: 15000 });
      check(await row.isVisible(), `«${ROW_TEXT}» — строка на месте (${label})`);

      // Отметка времени: до первого обновления — «только что», а не выдуманное число
      // и не пустота (данные-то уже прочитаны при заходе в приложение).
      const ago = (await row.locator('.val').innerText()).trim();
      check(
        ago === 'только что' || /назад$/.test(ago),
        `отметка времени честна: «${ago}» (${label})`,
      );

      // Парная проверка (EXP-0070): сначала докажем, что БЕЗ обновления заход бесплатен, —
      // иначе «после тапа читает» ничего не значило бы, читай оно всегда.
      reads.reset();
      await navigate(page, '/relations');
      const warmReads = reads.get();
      check(
        warmReads === 0,
        `повторный заход на «Связи» бесплатен до обновления (${label})`,
      ...
      reads.reset();
      await row.click();
      // ⚠️ Окном, а не «дождаться, что кольцо перестало крутиться»: селектор `:not(.spinning)`
      // истинен уже В МОМЕНТ клика — обработчик асинхронный, и разметка ещё не обновилась.
      // Такое ожидание возвращалось мгновенно и мерило ноль чтений на исправном продукте.
      // Чтения накапливаются, поэтому фиксированное окно здесь честнее любого признака.
      await page.waitForTimeout(2500);
      const afterTap = reads.get();
      check(
        afterTap > 0,
        `тап по «${ROW_TEXT}» реально перечитывает данные (${label})`,
        `прочитано документов: ${afterTap}`,
      );
      if (afterTap > 0) console.log(`      прочитано: ${reads.paths().join(' · ')}`);
