 ...
 *   · порог: D_LIMIT=155
 * Ворота: у САМОГО МЕРИТЕЛЯ — нет: он печатает числа о живом каталоге и никого не судит.
 ...
 *   🔴 А вот у `--selftest` ворота ЕСТЬ: `npm run test:tools` через `measure-hub-copy.test.mjs`.
 *   Освобождение мерителя на него НЕ распространяется — причина освобождения («каталог растёт»)
 *   принадлежит мерителю, а вердикт самотеста от данных не зависит вовсе: он герметичен, строит
 *   две свои песочницы и детерминирован (вердикт №37, Д2).
 * Код возврата: 🔴 ДВА ПУТИ, И ОНИ РАЗНЫЕ — не одно «0 всегда».
 *   · меритель (обычный запуск и `--fixture`): 0 всегда — ему нечего проваливать;
 *   · `--selftest`: 0 — контроли сошлись, 1 — хотя бы один разошёлся.
 *   ⚠️ Строка «0 всегда» стояла здесь и ПОСЛЕ того, как появился путь `--selftest`, дающий 1, —
 *   пережила собственное снятие в пределах одного файла и одной порции (вердикт №37, Д1).
 *   Маркеры этой шапки несущие: их читает перепись приборов, и она прямо объявляет, что ПРАВДУ
 *   маркера не судит. Меняешь поведение — правь шапку тем же движением.
 *
...
if (process.argv.includes('--selftest')) {
  ...
  // Корень сюда НЕ передаётся: самотест герметичен и настоящего дерева не касается (вердикт №37).
