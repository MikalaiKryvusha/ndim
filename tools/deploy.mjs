...
/**
 * Доказательство стоп-правила контура — 8 случаев, ничего не выкатывая.
 *
 * 🔴 ЖИВЁТ ПОД ПРЕДОХРАНИТЕЛЕМ, КАК И ВСЁ ОСТАЛЬНОЕ (`ideas/43`, первый полевой случай).
 * Самотест, стоящий СНАРУЖИ предохранителя, — это ровно тот дефект, ради которого страж
 * импорт-безопасности и написан: у `rewrite-catalog-descriptions.mjs` он печатал зелёный,
 * ни разу не отработав. Здесь он возвращает код, а не зовёт `process.exit`, — чтобы у двери
 * была ОДНА точка выхода.
 */
function selftest() {
	const cases = [
 ...
 */
async function liveHash(site, { expect = null, tries = 20, pauseMs = 3000 } = {}) {
	let why = 'не пробовали';
	let last = null;
	for (let attempt = 1; attempt <= tries; attempt += 1) {
		try {
			const response = await fetch(`${site}/ru`, { redirect: 'follow', cache: 'no-store' });
			const html = await response.text();
			const hash = html.match(/__sveltekit_([a-z0-9]+)/)?.[1] ?? null;
			if (hash) {
				last = hash;
				/*
				 * 🔴 ЖДЁМ НЕ «ХОТЬ КАКОЙ-ТО ХЕШ», А ОЖИДАЕМЫЙ — и это вторая половина `bugs/137`,
				 * которую первая починка пропустила, потому что я обобщил один симптом на весь класс.
				 *
				 * Первый отказ выглядел как «не прочитан», и повторы были сделаны РОВНО под него.
				 * Второй отказ пришёл в другом обличье того же класса: контур отдавал СТАРЫЙ хеш
				 * (`1g0erkt` вместо `1m3gy35`) — то есть ответ читался с первой попытки, функция
				 * сразу возвращала его, и повторы не работали вовсе. Замер спустя две минуты
				 * показал правильную сборку: выкат доходил оба раза, спешил замер.
				 *
				 * 🔑 Настоящий класс — «замер распределённой раскатки слишком нетерпелив», и у него
				 * ДВА обличья: хеша нет и хеш старый. Лечение обязано накрывать оба, иначе оно лечит
				 * симптом, который случилось увидеть первым.
				 */
				if (!expect || hash === expect) {
					// ПОВТОР ОСТАВЛЯЕТ УЛИКУ: без неё прогон, прошедший сразу, и прогон, спасённый
					// десятой попыткой, выглядят одинаково — и починка снова станет недоказуемой.
					if (attempt > 1) {
						console.log(
							`     (${site} отдал ожидаемое с попытки ${attempt} из ${tries} — раскатка шла ${((attempt - 1) * pauseMs) / 1000} с)`,
						);
					}
					return { hash, why: null, attempt };
				}
				why = `отдаётся ${hash}, ждём ${expect}`;
			} else {
		...
		// Пауза здесь — не «на всякий случай», а единственная разница между исправным выкатом и
		// ложным красным: Firebase Hosting раскатывает релиз не мгновенно.
		if (attempt < tries) await new Promise((resolve) => setTimeout(resolve, pauseMs));
	}
	return { hash: expect ? null : last, why, attempt: tries, last };
}
...
/**
 * ── ВСЯ РАБОТА ДВЕРИ ЖИВЁТ ЗДЕСЬ (`plans/79`) ────────────────────────────────────────────
 *
 * 🔴 ДО 2026-08-29 ЭТИ 32 ОПЕРАТОРА СТОЯЛИ НА ВЕРХНЕМ УРОВНЕ МОДУЛЯ. Это значит буквально
 * следующее: `import('tools/deploy.mjs')` из любого файла — из юнита, из чужого прибора, из
 * пробы в консоли — ЗАПУСКАЛ ВЫКАТ В БОЙ. Не «мог бы», а запускал: среди операторов были
 * `await liveHash(...)` (сеть) и `await smokeGate(...)` (сборка, порты, дочерние процессы).
 * Замер dev-1 отнёс дверь к разряду 🟠 «СЕТЬ на верхнем уровне», а суд №18 назвал её
 * единственным непочиненным носителем класса, чья верхнеуровневая работа — это выкат.
 *
 * 🔑 ЦЕНА КЛАССА ЗДЕСЬ НЕ ТОЛЬКО В СЛУЧАЙНОМ ВЫКАТЕ. Вторая половина дороже и тише: прибор
 * без предохранителя ОТМЕНЯЕТ собственную проверяемость — импорт зовёт `process.exit()`,
 * тестовый файл умирает до первого утверждения, и `node --test` засчитывает его пройденным.
 * То есть любой юнит этой двери был бы зелёным независимо от того, что в нём написано.
 *
 * Порядок шагов и каждая их строка сохранены дословно — переехал только отступ.
 *
 * @returns {Promise<number>} код возврата двери (0 — выкат прошёл и проверен).
 */
async function main() {
	...
	/*
	 * 🔑 ПРЕДУПРЕЖДЕНИЕ О ГРЯЗНОМ ДЕРЕВЕ — НА СТАРТЕ, А НЕ В КОНЦЕ (оплачено 2026-08-17).
	 *
	 * Расписка стейджа пишется ПОСЛЕДНЕЙ строкой, и до 2026-08-17 про негодность расписки дверь
	 * сообщала там же — то есть через двадцать минут сборки и загрузки 20 645 файлов. Сессия узнавала
	 * «дверь в бой её не примет» уже потратив весь прогон, и платила вторым таким же.
	 *
	 * Случай был самый обычный: агент правил свои же приборы, пока дверь работала. Правка приборов
	 * выкат не меняет, но замок судит ДЕРЕВО — и правильно судит, потому что «правка мимо сборки» это
	 * суждение, а замок обязан быть механическим.
	 *
	 * ⛔ Дверь здесь НЕ отказывает: выкат грязного кода на стейдж законен, для проб он и нужен. Она
	 * лишь называет цену заранее — и это разница между «предупредил» и «поймал на выходе».
	 */
	if (CONTOUR.name === 'stage') {
		const atStart = repoState();
		if (atStart.dirty) {
			console.log(
				'\n⚠️  ДЕРЕВО ГРЯЗНОЕ НА СТАРТЕ — расписка стейджа выйдет НЕГОДНОЙ для боя.' +
					'\n   Прогон законен (стейдж для проб и существует), но выкат в бой потребует ПОВТОРА' +
					'\n   после коммита. Хотите годную расписку — закоммитьте сейчас и запустите заново.' +
					'\n   И не правьте дерево, пока дверь работает: расписка снимается в КОНЦЕ прогона.',
			);
		}
	}
	...
	/*
	 * 🔑 Целевой контур спрашивается С ОЖИДАНИЕМ (`expect`): замер ждёт ИМЕННО нашу сборку, а не
	 * «хоть какой-то хеш». Соседний — БЕЗ ожидания: у него мы спрашиваем текущее значение, каким бы
	 * оно ни было, и ждать от него нечего.
	 */
	const target = await liveHash(CONTOUR.site, { expect: builtHash });
	const alienAfter = (await liveHash(alien.site, { tries: 3 })).hash;
	console.log(`  цель  ${CONTOUR.title}: сборка ${builtHash} · сайт отдаёт ${target.hash ?? target.last ?? 'не прочитан'}`);
	console.log(`  сосед ${alien.title}: было ${alienBefore ?? 'не прочитан'} · стало ${alienAfter ?? 'не прочитан'}`);
	if (target.hash === null) {
		/*
		 * Два обличья одного отказа, и они РАЗВЕДЕНЫ намеренно: «ничего не прочитал» и «читаю старое»
		 * лечатся по-разному, а слитые в одно сообщение отправляют сессию искать не ту поломку.
		 * В обоих случаях названо, сколько ждали, — без этого нельзя отличить «не доехало» от
		 * «не дождались».
		 */
		const waited = Math.round((target.attempt * 3000) / 1000);
		console.error(
			target.last
				? `\n🔴 ${CONTOUR.title} за ${waited} с так и не отдал нашу сборку: ${target.why}.` +
						`\n   Раскатка либо не дошла, либо идёт дольше окна замера. Проверить руками:` +
						`\n   ${CONTOUR.site}/ru — если там уже ${builtHash}, значит замер нетерпелив (окно в liveHash).`
				: `\n🔴 ХЕШ ${CONTOUR.title} НЕ ПРОЧИТАН за ${waited} с — причина: ${target.why}.` +
...
/**
 * 🔴 ПРЕДОХРАНИТЕЛЬ «ЗАПУЩЕН ИЛИ ПОДКЛЮЧЁН» — форма из `ideas/43`, критерий К2.
 *
 * Признак опознаётся стражем ПО СМЫСЛУ (сравнение `import.meta` с `process.argv[1]`), а форма
 * сверена на трёх способах вызова: абсолютный путь с обратными слэшами, относительный и со
 * строчной буквой диска. Разъедься они, дверь отключилась бы у всех и молча.
 *
 * ⚠️ `--gate-only` и `--selftest` ходят ТЕМ ЖЕ путём, что боевой выкат: одна точка входа, одна
 * точка выхода. Отдельная дверь для проверок разъехалась бы с рабочей — тот самый довод, по
 * которому в проекте запрещена вторая дверь выката (`plans/53`).
 */
