...
async function liveHash(site, { tries = 6, pauseMs = 2500 } = {}) {
	...
	for (let attempt = 1; attempt <= tries; attempt += 1) {
		try {
			...
			if (hash) {
				/*
				 * 🔑 ПОВТОР ОБЯЗАН ОСТАВЛЯТЬ УЛИКУ, иначе починка `bugs/137` недоказуема: прогон,
				 * прошедший с первой попытки, и прогон, спасённый третьей, выглядели бы одинаково —
				 * и следующая сессия поверила бы зелёному, не зная, работает ли повтор вообще.
				 */
				if (attempt > 1) {
					console.log(`     (хеш ${site} прочитан с попытки ${attempt} из ${tries} — раскатка запаздывала)`);
				}
