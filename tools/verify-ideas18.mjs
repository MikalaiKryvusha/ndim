 ...
 *   2. **Повторный заход внутри окна свежести не стоит НИ ОДНОГО обращения к базе.** Это деньги
...
async function run(browser, { theme, width }) {
   ...
   *
   * ⚠️ Так сделано НЕ для красоты. Круг из пяти экранов длится десятки секунд, а данные сервера
   * синхронизации свежи 60 секунд — и «0 чтений» на втором круге зависело бы от того, успел ли
   * прогон уложиться в окно. Страж, зелёный или красный по секундомеру, — это лотерея, а не
   * страж. В тройке между заходами проходят секунды, и проверка меряет КЭШ, а не часы.
   *
...
async function main() {
  ...
  // тема и каждая ширина были покрыты, а прогон оставался в пределах окна свежести.
  const combos = QUICK
