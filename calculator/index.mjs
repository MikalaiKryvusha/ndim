...
//         CALC_FULL_SYNC_AT_HOUR — час суток ночного полного прохода, местное время (деф. 0)
...
/**
 * ЧАС СУТОК ночного полного прохода (местное время серверного ПК). Канон 1.x —
 * `daily_sync в 00:00` (`researches/13` §4).
 *
...
export function lastScheduledFullPass(now) {
  ...
  moment.setHours(FULL_SYNC_AT_HOUR, 0, 0, 0);
  if (moment.getTime() > now) moment.setDate(moment.getDate() - 1);
  ...
  if (once) {
    ...
    log(
      `режим службы: цикл каждые ${intervalSeconds} с, тихий период ${QUIET_MS / 1000} с, ` +
        `ночной полный проход в ${String(FULL_SYNC_AT_HOUR).padStart(2, '0')}:00 по местному времени` +
