 ...
 * запуске. Поэтому все инструменты, импортирующие вычислитель, лежат рядом с ним — как его тесты. Отвечает на три вопроса, на которые
 ...
 *
 * И попутно четвёртый (`bugs/109`): находит ли вычислитель НОВЫЕ измерения, если они
...
/** Метка, по которой узнаём пересборку индекса: её печатает сам вычислитель. */
const REBUILD_MARK = 'индекс каталога пересобран';

/**
 * Считает пересборки индекса за один цикл вычислителя.
 *
 ...
 * вычислителя, и она же видна владельцу в контейнере.
 */
...
// ── ОПЫТ 4. bugs/109: видит ли вычислитель новые измерения в БОЕВОЙ форме ─────
