...
// правилам: `points/{uid}` читают только владелец и вычислитель (`firestore.rules`). Значит
// единственный законный способ показать эту строку — положить число в саму запись топа. Здесь и
// проверяется, что вычислитель это делает и не врёт значением.
//
...
// вычислителя, см. economy.test.mjs). Первая редакция этого файла канон пропустила и уронила
