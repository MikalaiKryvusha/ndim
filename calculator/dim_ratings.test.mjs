...
describe('сводка оценок каталога', () => {
	...
	test('оценка на измерении ВНЕ каталога не роняет цикл и объявляется в логе', async () => {
		...
		// Докладываются ОБЕ величины: измерений-сирот и оценок на них. Путать их дорого —
		// именно оценки объясняют расхождение с `space/stats.ratings`, а не измерения.
		assert.match(log, /сироты: измерений вне каталога 1, оценок на них 1/);
		assert.match(log, /dim-deleted/, 'сирота назван поимённо — иначе его не найти');
