...
> ⚙️ **Сначала — актуальная механическая команда:** `npm run kaif:version`
> (`node .kaif/kaif-core.mjs version`). Если проза этого файла расходится с машинерией, верь машинерии
> и релизным заметкам origin: принятая локальная копия скилла жизненного цикла молча устаревает от
> релиза к релизу.
...
- Замеченные дефекты самого фреймворка заводятся скиллом `/report-bug` (шаблоны A/B — с аттестацией
  дедупликации); развёртывание с привязкой к origin перед заведением проверяет и открытые тикеты
  истока на тот же класс. Полевые отчёты об обновлении/установке живут в `reports/KAIF_UPDATES/` —
  докладывая версию, упомяни неотправленные, если такие есть.
