...
11. **Короткий отчёт в чат** (1–3 строки): что сделал, что проверил, что дальше — открывается
    строкой доставки `DELIVERY: <метрика владельца> X → Y; moved by: … | blocker: …` (метрика —
    из `MASTER_PLAN.md`; нулевая дельта законна только с названным блокером — охота судьи
    «строка доставки»). → следующая задача.
