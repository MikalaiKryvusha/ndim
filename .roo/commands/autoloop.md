...
> 📥 **Человек написал по ходу цикла — сначала классифицируй.** Мимоходом брошенная идея или баг НЕ
> про текущую задачу уходит в `ideas/`/`bugs/` (с пометкой источника) и подтверждается одной строкой,
> а серия ПРОДОЛЖАЕТСЯ; прерывают её только прямая интерактивная просьба или явное «стоп/переключись».
> Сообщение, пришедшее ПОСРЕДИ хода, — то же слово: автора подписывает система (`AGENT_GUIDE.md` →
> «Слово владельца посреди хода»): «стоп» завершает цикл в этом же ходе; «переключись» — сначала
> строка `PARKED:` в `STATUS.md`.
...
1. **Возьми** следующую автономную задачу из пула (приоритет из STATUS). Убедись, что её можно
   проверить БЕЗ человека и особых ресурсов. Если нет — отложи, возьми следующую. Действует правило
   свежести контекста (`AGENT_GUIDE.md` → «Свежесть контекста»): прошло больше 60 минут с последнего
   освежения или следующая задача ТЯЖЁЛАЯ — перечитай ядро и обнови свидетель (отметка + цитата;
   `/refresh-context` делает и то и другое) ДО взятия задачи.
...
3. **Реализуй** точечно, с комментариями. Не усложняй. Исполняй по фейбл-циклу (`/fable-method`;
   ...
   сначала построй лестницу (`/plan-epic`: исследование → метаплан), затем исполняй по фазам.
...
- Владелец НАЗВАЛ время конца этого прогона, и оно настало → **начни `/end-chat-soft`**; до этого
  времени — обычный темп, без раннего финиша из страха перед сроком (`AGENT_GUIDE.md` → «Работа до
  названного часа»). В начале каждой итерации снимай время пробой (`date '+%Y-%m-%d %H:%M %z'`;
  PowerShell — `Get-Date -Format 'yyyy-MM-dd HH:mm zzz'`), никогда на глаз; перед любой церемонией
  закрытия печатай `BOUNDARY: now <эта проба> · named <час владельца> · pool <empty | N items>` —
  решают часы (origin issue #96).
...
- Серьёзная развилка по UI/UX/бренду/архитектуре, которую агент НЕ должен решать один → заведи
  `/interview` и приостановись. (Проект, где развёрнут контур `/owner-reviews`, вместо этого ставит
  интервью в очередь пакетной страницы «накопилось N» — инвариант I7 — и переходит к
  незаблокированной работе; цикл приостанавливается, только когда незаблокированного не осталось.)
