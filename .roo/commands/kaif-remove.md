...
- **Частичное** — удалить **ядро и обёртку** фреймворка, но **сохранить содержательные артефакты**:
  `bugs/`, `interviews/`, `ideas/`, `researches/`, `homeworks/`, `reports/` и любое другое знание,
  произведённое работой. Накопленное знание агента переживает удаление; уходит только машинерия KAIF.
...
1. **ОБЯЗАТЕЛЬНО — спроси владельца естественным языком, КАКОЕ удаление запускать, и дождись явного,
   ...
   - Флаг `--all` или явная фраза «полное удаление» / «выжги полностью» (на любом языке владельца)
     считается явным ответом за **полное**; «оставь артефакты» / «частично» — за **частичное**.
     Всё остальное → переспроси.
...
2. **Определи то, что принадлежит KAIF**, из `.kaif/kaif.json` и известной раскладки:
   ...
   - **Содержательные артефакты (сохраняются при частичном, удаляются при полном):** файл домашних
     правил `HOUSE_RULES.md` и портрет голоса `AUTHOR_STYLOMETRY.md` (если есть), `bugs/`,
     `interviews/`, `ideas/`, `researches/`, `homeworks/`, `plans/`, `reports/`.
