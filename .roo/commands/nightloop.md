...
1. **Время ≥ времени подъёма** (по умолчанию 09:00 по местному; задай его при запуске цикла).
   ...
   (`AGENT_GUIDE.md` → «Работа до названного часа»).
2. **Человек написал в чат — сначала классифицируй, потом переключайся.** Прямая просьба → выйти,
   ...
   одной строкой и ПРОДОЛЖАЙ ночь. Уровень видения → `/fix-vision`, затем продолжай.
...
6. **Короткий отчёт в чат** (1–3 строки): чтобы утром человек увидел прогресс; открывается строкой
   `DELIVERY: <метрика владельца> X → Y; moved by: … | blocker: …` (метрика — из `MASTER_PLAN.md`;
   нулевая дельта законна только с названным блокером — охота судьи «строка доставки»).
