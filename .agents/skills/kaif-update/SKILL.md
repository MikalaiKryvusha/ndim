...
>
> ⚠️ **Предскажи проход ДО прохода.** Готового `--dry-run` нет (`diff` требует манифеста v2 и на
> старом развёртывании отказывается работать). Дешёвый и точный способ: собрать песочную копию
> проекта (`git archive HEAD | tar -x -C <песочница>`) и прогнать обновление СНАЧАЛА в ней —
> получишь настоящий диф без риска (сделано 2026-07-28, предсказание совпало с боем байт в байт).
>
