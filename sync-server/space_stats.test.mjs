...
describe('Сервер синхронизации пишет статистику Пространства', () => {
  ...
  test('полная и частичная синхронизации отчитываются раздельно, как в 1.x (bugs/42)', async () => {
    const server = (await db.doc('space/server').get()).data();
    // Первый цикл процесса — полный проход: блок полной записан, частичной ещё не было.
    assert.ok(server.fullSync, 'блок полной синхронизации должен быть записан');
    /*
     * ⚠️ ЗДЕСЬ БЫЛО 3, СТАЛО 2, И ЭТО НАМЕРЕННАЯ СМЕНА СМЫСЛА (bugs/85, часть В).
     *
     * `checked` — то, что виджет показывает строкой «Пользователей проверено». Гостя в
     * Пространстве не видно (интервью №004, В3), и «Всего людей» его не считает. Пока
     * `checked` считал гостей, на ОДНОМ экране две строки означали РАЗНЫЕ популяции, и
     * владелец читал разницу как поломку.
     *
     * Топ гостя при этом по-прежнему считается и пишется — это его собственное дело
     * (проверка `usersSynced === 3` выше как раз про это и стережёт границу: считаем всем,
     * а в витрину Пространства пускаем только видимых).
     */
    assert.equal(server.fullSync.checked, 2, 'проверены двое НЕ-гостей: гость не в счётчиках Пространства');
    assert.equal(server.fullSync.updated, 3, 'все топы новые — все записаны');
    assert.equal(server.fullSync.relationsComputed, 2);
    assert.ok(server.fullSync.nextAt > server.fullSync.at, 'следующая полная — в будущем');
    assert.equal(server.partialSync, undefined, 'частичных циклов ещё не было — блок не выдуман');
  });
