// Сервер синхронизации связей NDim Space 2.0 (фаза 4 мастер-плана).
// В интерфейсе он называется «Сервер синхронизации» — термин 1.x (AGENT_GUIDE → словарь).
//
// Фоновая пакетная задача: находит обновлённые NDim ID (человек изменил оценки),
...
//     и в НОЧНОМ полном проходе (заданный час суток), дальше дочитываются ТОЛЬКО точки
//     с обновлённым NDim ID. Прогрев и ночной проход — РАЗНЫЕ вещи (bugs/85): прогрев неизбежен при старте,
...
//     изменившимся: в обычном цикле — обновлённым NDim ID, в полном проходе — каждому, чей
...
//     (первый расчёт ещё не случился или firstSeen моложе SYNC_NEWCOMER_MINUTES),
...
// Клиенту запись в relations запрещена правилами; сервер синхронизации ходит через Admin SDK
...
//
// Запуск: node sync-server/index.mjs --once   (один цикл — для стенда и тестов)
//         node sync-server/index.mjs          (цикл каждые SYNC_INTERVAL_SECONDS, деф. 60)
// Ручки:  SYNC_QUIET_SECONDS     — тихий период, с (деф. 120; стенд ставит 0 — там человек
//                                  не должен ждать, tools/stand.mjs)
//         SYNC_FULL_AT_HOUR — час суток ночного полного прохода, ПО UTC (деф. 0 — как 1.x)
//         SYNC_NEWCOMER_MINUTES  — окно новичка без тихого периода, мин (деф. 30)
//
...
//
// Номер сборки = ЧИСЛО КОММИТОВ, ТРОНУВШИХ `sync-server/` (`git rev-list --count HEAD --
// calculator`), вычисляется при сборке образа и приходит сюда через SYNC_BUILD. Требование
 ...
 * шаг 1).
 *
 * 🔵 Дата — слово владельца, интервью №042 В3: «*Начнём удалять с октября*» (2026-08-21).
 * До неё спит ВСЯ уборка гостей по сроку жизни, включая прежнюю 30-дневную, — по букве ответа.
 * Момент взят в локальном времени владельца (+03:00), а не в UTC: «с октября» сказано по его
 * календарю.
 *
 * Ручки окружения у даты нет намеренно (как у ESCORT_MAX_RUNS): переменная среды позволила бы
 * контуру включить удаление людей молча.
 */
const GUEST_CLEANUP_START_MS = Date.parse('2026-10-01T00:00:00+03:00');

/**
 * Отсрочка, младше которой документ пары теста НЕ рассматривается на уборку (`plans/42`).
 * Не срок жизни пары, а граница дешевизны: пару заводят двое ЖИВЫХ, поэтому свежая мусором
 * быть не может, и сегодняшний трафик читать незачем. Сутки заодно снимают гонку с
 * присоединением второго, которое идёт прямо сейчас.
 */
const PAIR_GRACE_DAYS = 1;
 ...
 *
 * ⚠️ Раньше здесь стоял ПЕРИОД (`SYNC_FULL_HOURS = 24`), и якорем суток был **момент
 ...
 */
const FULL_SYNC_AT_HOUR = Number(process.env.SYNC_FULL_AT_HOUR ?? 0);
 ...
 */
const FULL_SYNC_EVERY_CYCLE = process.env.SYNC_FULL_EVERY_CYCLE === '1';
...
// локальный по умолчанию, чтобы сервер синхронизации случайно не потянулся в боевой Firestore.
...
if (
  ...
  console.error(
    `[sync] СТОП: проект ${projectId} совмещён с эмулятором (FIRESTORE_EMULATOR_HOST=` +
 ...
 *
 * До 2026-08-15 сервер синхронизации знал только `(default)` и потому физически не мог обслуживать стейдж.
...
if (allowed && !allowed.includes(databaseId)) {
  console.error(
    `[sync] СТОП: проект ${projectId} и база ${databaseId} — разные контуры. ` +
...
/**
 * 🔴 КЛЮЧ СЕРВИСНОГО АККАУНТА — ИЗ ОКРУЖЕНИЯ, НЕ ИЗ ФАЙЛА.
 *
 * Правило владельца 2026-08-15, дословно: «*ключи нужно вынести в .env, никогда ключи в коде и
 * других файлах не храним*». До этого дня контейнер монтировал папку с ключом
 * (`-v …/secrets:/secrets:ro` + `GOOGLE_APPLICATION_CREDENTIALS=/secrets/sa.json`), и у монтирования
 * ПАПКИ обнаружилась цена: положенный туда ключ ВТОРОГО контура немедленно оказался внутри боевого
 * контейнера — то самое смешение контуров, которое запрещает `plans/53`.
 *
 * `NDIM_SA_B64` — base64 от JSON ключа. Base64, а не сырой JSON: значение проходит через `.env`,
 * `docker --env-file` и оболочки, а там фигурные скобки, запятые и кавычки разбираются каждым
 * парсером по-своему. Base64 — одна строка чистого ASCII, которую нельзя разобрать неправильно.
 *
 * ⚠️ Честная цена решения: переменные окружения контейнера видны в `docker inspect` и в списке
 * процессов хоста. Файл, смонтированный только для чтения, этого не показывал. Это осознанный
 * обмен, сделанный по прямому слову владельца, а не по недосмотру.
 *
 * ЧЕТВЁРТЫЙ страж пары: ключ обязан принадлежать ТОМУ ЖЕ проекту, что `FIREBASE_PROJECT_ID`.
 * Иначе контейнер стейджа с боевым ключом писал бы в топы живых людей — та же авария, от которой
 * бережёт страж «проект × база» выше, только заходящая с другой стороны.
 */
function credentialFromEnv() {
  const raw = process.env.NDIM_SA_B64;
  if (!raw) {
    console.error(
      '[sync] СТОП: не задан NDIM_SA_B64 — ключ сервисного аккаунта берётся из окружения ' +
        '(base64 от JSON ключа), а не из файла. Образец — .env.example в репозитории.',
    );
    process.exit(1);
  }
  let key;
  try {
    ...
    console.error(`[sync] СТОП: NDIM_SA_B64 не разбирается (ожидался base64 от JSON): ${error.message}`);
    process.exit(1);
  }
  if (key.project_id !== projectId) {
    console.error(
      `[sync] СТОП: ключ принадлежит проекту ${key.project_id}, а запуск объявлен как ${projectId}. ` +
        'Перекрёстная пара «ключ × проект» — всегда ошибка запуска, а не замысел.',
    );
    process.exit(1);
  }
  return key;
...
const log = (message) => console.log(`[sync ${new Date().toISOString()}] ${message}`);

// ── Кэш процесса ─────────────────────────────────────────────────────────────
// Сервер синхронизации — единственный писатель relations и единственный полный читатель точек,
 ...
 * связи», наследие формата 1.x. Чтобы не смешивать, внутри сервера синхронизации аноним
...
/**
 * ЧИСТКА МЁРТВЫХ ОЦЕНОК — оценок на измерениях, которых в каталоге больше нет (`bugs/111`).
 *
 * Решение владельца 2026-08-03: «*почистить юзерам профили от мёртвых измерений*».
 *
 * ═══ ЧЕМ МЕШАЛИ, ПОКА ЛЕЖАЛИ ═══
 *
 * Не только цифрой на витрине. Оценка на удалённом измерении:
 *   · **участвует в математике связей** — `computeRelation` берёт ВСЕ оценки человека, и двое,
 *     оценившие удалённое измерение, получают общую ось, которой в продукте нет. Хуже того,
 *     знаменатель Дайса `(|мои| + |его|)` растёт у носителя — то есть похожесть занижается
 *     именно ему, и молча;
 *   · **не может быть убрана человеком** — экран «Измерения» рисуется ИЗ КАТАЛОГА и сироту не
 *     показывает вовсе. Оценка есть, влияет, а удалить её владельцу нечем.
 * Замер боя 2026-08-03: 15 измерений, 25 оценок.
 *
 * ═══ ЦЕНА ЗАПРОСОВ (прямое требование владельца: «с уважением к запросам») ═══
 *
 * 🔑 **ПОИСК СИРОТ СТОИТ НОЛЬ ЧТЕНИЙ.** Оба набора уже в руках и прочитаны для другой работы:
 * оценки людей лежат в `pointsCache` (его читает математика связей), а список существующих
 * измерений — в `writtenDimRatings` (он поднят для сводки каталога). Ни одного запроса ради
 * самой чистки не делается: ни `collectionGroup`, ни обхода коллекций, ни `listDocuments`.
 * Платим только за УДАЛЕНИЯ, и только когда есть что удалять; нет сирот — проход бесплатен.
 *
 * Работает ТОЛЬКО в полном проходе. Это не осторожность ради осторожности: судить «измерения
 * нет в каталоге» можно лишь по ПОЛНОМУ знанию и о каталоге, и о точках. В обычном цикле
 * дочитываются только обновлённые NDim ID — там такого знания нет.
 *
 * ═══ ПРЕДОХРАНИТЕЛЬ ═══
 *
 * 🔴 Это удаление ДАННЫХ ЛЮДЕЙ, и ошибка здесь необратима. Оба предохранителя защищают от
 * одного и того же сценария — каталог прочитался частично или пусто, и «мёртвым» стало всё:
 *   · чистка не запускается, если каталог подозрительно мал;
 *   · чистка ОТКАЗЫВАЕТСЯ целиком, если жертв больше потолка, и говорит об этом громко.
 * Отказ лучше половинчатого удаления: массовое сиротство означает, что сломано что-то другое,
 * и такое обязан увидеть человек, а не «почистить» робот.
 */
const MIN_CATALOG_FOR_CLEANUP = 100;
const ORPHAN_CLEANUP_MAX = 200;

async function cleanupOrphanRatings(points, catalogIds) {
  if (catalogIds.size < MIN_CATALOG_FOR_CLEANUP) {
    log(`чистка мёртвых оценок пропущена: каталог подозрительно мал (${catalogIds.size})`);
    return 0;
  }

  const victims = [];
  for (const [uid, point] of points) {
    for (const dimId of Object.keys(point.ratings)) {
      if (!catalogIds.has(dimId)) victims.push([uid, dimId]);
    }
  }
  if (victims.length === 0) return 0;

  if (victims.length > ORPHAN_CLEANUP_MAX) {
    log(
      `🔴 чистка мёртвых оценок ОТКАЗАНА: жертв ${victims.length} при потолке ${ORPHAN_CLEANUP_MAX}. ` +
        'Массовое сиротство означает, что сломано что-то другое — нужен взгляд человека.',
    );
    return 0;
  }

  // Удаляем и в базе, и в кэше: дальше по циклу из кэша считаются топы, статистика и сводка
  // каталога. Разойдись они — человек увидел бы связи по оценкам, которых уже нет.
  const deletions = victims.map(([uid, dimId]) => (batch) => batch.delete(db.doc(`points/${uid}/dims/${dimId}`)));
  await commitInChunks(deletions);
  for (const [uid, dimId] of victims) delete points.get(uid)?.ratings[dimId];

  /*
   * ⚠️ `points/{uid}.updated` СОЗНАТЕЛЬНО НЕ ТРОГАЕМ. Это отметка «когда человек последний раз
   * менял свои оценки», и на ней держится «активных за 7 дней». Подняв её, сервер объявил бы
   * активными людей, которые ничего не делали, — то есть соврал бы на витрине ради удобства
   * уборки. Флаг `dirty` тоже не нужен: топы в полном проходе пересчитываются всем.
   */
  const people = new Set(victims.map(([uid]) => uid)).size;
  log(`чистка мёртвых оценок: удалено ${victims.length} у людей — ${people}`);
...
async function loadDims(now) {
   ...
   * Дефект нашёлся чтением кода и воспроизведён `sync-server/measure-dims-index.mjs`.
   *
...
async function ensureDimsIndex(dimsCount) {
   ...
   * документов оставался, условие не выполнялось НИКОГДА, и сервер синхронизации перечитывал весь
 ...
 * Анонимные гости НЕ кандидаты ни в чей топ — но сам владелец-гость получает свой топ против
 * публичных точек на общих основаниях.
 *
 * 🔴 ДОВОД ЭТОГО ПОВЕДЕНИЯ ОТМЕНЁН, А ПОВЕДЕНИЕ ЖИВЁТ (`bugs/227`, помечено 2026-08-30).
 * Здесь стояло «В3: гость невидим другим» — решение интервью №004, В3 = А. Оно **отменено
 * ответом владельца №009 В5 от 2026-08-01**, дословно: «*Можно всё как и пользователю с
 * полноценныым аккаунтом!*»; `plans/22` фиксирует отмену прямым текстом.
 * Строка кода не изменена намеренно: снятие фильтра — решение о ПОВЕДЕНИИ, и цена его названа
 * в самом вопросе №009 В5 и никем не снята (гость живёт 7 дней, и в чужих топах появились бы
 * точки, исчезающие через неделю). Вопрос подан владельцу отдельной страницей вычитки.
 * ⚠️ До его нового слова поведение остаётся прежним, но ссылаться на №004 В3 как на
 * ДЕЙСТВУЮЩЕЕ решение больше нельзя: оно отменено.
 */
function topFor(ownerUid, points) {
  ...
  for (const [otherUid, other] of points) {
    ...
    // Гость исключён из чужих топов. ⚠️ Довод отменён (№004 В3 → №009 В5, 2026-08-01) —
    // разбор в шапке `topFor` и в `bugs/227`; строка ждёт слова владельца о ПОВЕДЕНИИ.
     ...
     * владелец и сервер синхронизации). Число у нас уже есть — оно загружено для статистики
...
export async function cleanupStaleGuests(now = Date.now()) {
  ...
  for (const uid of expired) {
    try {
      ...
      /*
       * Гонка с внешней 30-дневной чисткой: учётка исчезла МЕЖДУ отбором и этим шагом.
       * Это ровно тот исход, к которому шли, — след снят, проход продолжается (замечание
       * второго судьи 2026-08-21: без catch один такой бросок обрывал бы весь суточный
       * проход). Ветка достижима только внедрением в код — тестом не покрыта, принята
       * чтением. Любая ДРУГАЯ ошибка — настоящая и обязана уронить проход, как раньше.
       */
      if (error?.code !== 'auth/user-not-found') throw error;
      log(`гость ${uid}: учётная запись уже исчезла (внешняя чистка опередила) — след снят`);
    }
  }
 ...
 *
 * Человек удаляет всё, что вправе удалить, сам и мгновенно (`src/lib/data/erase.ts`). Но часть
 * следов ему недоступна по правилам, и это не недоделка, а устройство безопасности:
 ...
 *   · его дружбы `friendships/{pair}` (`plans/63` шаг 4): удалить свою пару клиент вправе, но
 *     оборванный на середине каскад оставляет сироту — а у живого друга тогда висит
 *     несуществующий человек, которого не снять ничем. Дыра была общей для ОБОИХ уборщиков
 *     (таблица §4 метаплана эпика 22), закрыта одним куском с уборкой гостей.
 *
...
export async function cleanupDeletedPeople(now = Date.now()) {
  ...
  for (const uid of gone) {
    ...
    // Дружбы обеих сторон — осиротевшая пара висела бы у живого друга вечно; кусок общий
    // с уборкой гостей (plans/63 шаг 4).
    await deleteFriendshipsOf(uid);
...
/**
 * Убирает документы пар теста, у которых НЕ ОСТАЛОСЬ НИ ОДНОГО ЖИВОГО УЧАСТНИКА
 * (`plans/42`, такт В, снятый `PENDING:` «гигиена старых пар»).
 *
 * ═══ ОТКУДА БЕРЁТСЯ МУСОР ═══
 *
 * Пару заводят двое, и чаще всего оба — гости. Гость исчезает (сам удалил аккаунт либо его
 * убрал `cleanupStaleGuests`), а документ пары переживает обоих: в нём нет ни `dirty`, ни
 * `lastSync`, ни чего-либо ещё, за что зацепился бы существующий уборщик. Без этой функции
 * `testPairs` растёт монотонно и вечно — навсегда, вместе с трафиком поверхности ТЕСТ.
 *
 * ═══ ПРИЗНАК СМЕРТИ — ОТСУТСТВИЕ ТОЧКИ, А НЕ СРОК ═══
 *
 * Живой участник = существует `points/{uid}`. Тот же признак, по которому
 * `cleanupDeletedPeople` узнаёт ушедшего, и по той же причине: надгробий не заводим, их
 * пришлось бы хранить вечно, и они сами стали бы следом человека. Пара, где жив ХОТЬ ОДИН, —
 * не мусор: второй вправе вернуться по своей ссылке и забрать свои ответы.
 *
 * ═══ ЦЕНА ЗАПРОСОВ (канон владельца «экономить запросы к базе») ═══
 *
 * Живых узнаём через `listDocuments()` — он отдаёт ССЫЛКИ, а не данные, то есть чтений не
 * тратит вовсе. Сами пары читать приходится (uid участников лежат в полях), поэтому
 * запрос сужен возрастом: `created < now - PAIR_GRACE_DAYS`. Пара младше суток мусором быть
 * НЕ МОЖЕТ по построению — её только что завели двое живых, — и сегодняшний трафик не
 * читается ни разу. Заодно снимается гонка с присоединением второго прямо сейчас.
 *
 * ⚠️ `created` пишет клиент (`Date.now()`), и правила проверяют лишь `is int`. Значит поле —
 * не источник истины о времени, а ФИЛЬТР ДЕШЕВИЗНЫ: соврав им, человек добьётся только того,
 * что его собственная пара будет прочитана раньше или позже. Решение об удалении принимается
 * по живым точкам, которые подделать нельзя.
 *
 * Возвращает число удалённых пар.
 */
export async function cleanupStalePairs(now = Date.now()) {
  const cutoff = now - PAIR_GRACE_DAYS * 24 * 60 * 60 * 1000;
  const [pairs, pointRefs] = await Promise.all([
    db.collection('testPairs').where('created', '<', cutoff).get(),
    db.collection('points').listDocuments(),
  ]);
  ...
  for (const pair of pairs.docs) {
    const { aUid, bUid } = pair.data();
    // Участники — только непустые uid: у неприсоединённой пары `bUid` равен null.
    const members = [aUid, bUid].filter((uid) => typeof uid === 'string' && uid.length > 0);
    /*
     * Пустой список участников — не «все мертвы», а битый документ. Молча удалять его
     * значило бы прятать дефект записи; оставляем и говорим вслух, чтобы он нашёлся.
     */
    if (members.length === 0) {
      log(`пара ${pair.id}: в документе нет ни одного участника — НЕ трогаю, это дефект записи`);
      continue;
    }
    if (members.some((uid) => alive.has(uid))) continue;

    await pair.ref.delete();
    removed += 1;
    log(`пара ${pair.id} осиротела (участников живых нет) — удалена`);
  }
...
export async function runCycle() {
  ...
  if (fullPass) {
    if (nightly) {
      ...
      /*
       * Пары — ПОСЛЕ обоих: их уборка судит по живым точкам, а первые два как раз и решают,
       * какие точки живы. Запусти её раньше — пара, чей единственный участник осиротел этой
       * же ночью, пережила бы проход и ждала следующего.
       */
      await cleanupStalePairs(startedAt);
    }
  ...
  if (!fullPass && ready.length === 0) {
    /*
     * 🔴 СЛОВО ВЛАДЕЛЬЦА 2026-08-18: «*это не „грязные точки“ — вульгарное название… это NDim ID.
     * и нужно говорить, что обновлённых NDim ID нет*». Лог читает ОН, и читает часто.
     *
     * Замена не косметическая, а точная: точка человека и ЕСТЬ его NDim ID — продукт уже говорит
     * этими словами на его же экранах («Мой NDim ID», «Последнее обновление NDim ID»). Прежнее
     * слово было внутренним жаргоном, вытекшим наружу, а `AGENT_GUIDE` → «Правила текста
     * продукта» запрещает жаргон на лице продукта прямо.
     */
    log(
      deferred > 0
        ? `тихий период: человек ещё оценивает, отложено NDim ID — ${deferred}`
        : 'обновлённых NDim ID нет — пересчитывать нечего',
    );
  ...
  if (readyUids.length > 0) {
    log(`обновлённых NDim ID: ${readyUids.length} (${readyUids.join(', ')})${deferred ? `, отложено тихим периодом: ${deferred}` : ''}`);
  }
  ...
  /*
   * ЧИСТКА МЁРТВЫХ ОЦЕНОК — здесь и только здесь (`bugs/111`, слово владельца 2026-08-03).
   *
   * Место выбрано не случайно: к этой строке уже прочитаны И все точки, И весь каталог, а
   * дальше по циклу из них считаются топы, статистика и сводка каталога. Почисти мы позже —
   * и цикл посчитал бы связи по оценкам, которых уже нет; почисти раньше — судить было бы не
   * по чему. Ноль запросов ради самого поиска, см. шапку функции.
   */
  if (fullPass) await cleanupOrphanRatings(pointsCache, writtenDimRatings);
  ...
  // дёшев, а код очевиден), но ЗАПИСЬ заслуживают не все: в обычном цикле — только обновлённые
   ...
   * обновлённый NDim ID за цикл; таких единицы — канон экономии не страдает. Порядок (сначала
