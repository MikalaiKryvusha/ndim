...
export default defineConfig({
	...
	webServer: {
		...
		/*
		 * 🔴 120_000 → 300_000, и это ЗАМЕР, а не запас «на всякий случай».
		 *
		 * `command` — `npm run build && npm run preview` одной строкой, то есть в это окно
		 * обязана уложиться ВСЯ сборка: 10 222 пререндеренных страницы. Замер 2026-09-09 на
		 * машине владельца: **сборка ровно 99 с** при окне 120 — то есть окно стояло на грани,
		 * и любая занятость машины опрокидывала прогон. Так и вышло: один и тот же коммит дал
		 * и полный прогон (106 тестов), и `Timed out waiting 120000ms` дважды подряд.
		 *
		 * ⚠️ Отказ был МОЛЧАЛИВО ВРЕДНЫМ: он выглядит как красный e2e, то есть как дефект
		 * правки, хотя код к нему непричастен. Дешевле дать сборке честное окно, чем каждый раз
		 * заново выяснять, чей это красный.
		 * ⛔ Это НЕ ослабление стража: `reuseExistingServer: false` выше не тронут, и прогон
		 * по-прежнему судит СВОЮ свежую сборку, а не чужую живую.
		 */
		timeout: 300_000,
	},
	projects: [
