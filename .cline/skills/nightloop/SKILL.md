...
1. **Время ≥ времени подъёма** (по умолчанию 09:00 по местному; задай его при запуске цикла).
   ...
   на связь. Настал час подъёма — это значит **НАЧНИ `/end-chat-soft`** — никогда не обрыв впопыхах
   и никогда не РАННИЙ финиш из страха перед сроком: работай в обычном темпе до самого часа
   (`AGENT_GUIDE.md` → «Работа до названного часа»). В начале каждой итерации снимай время пробой
   (`date '+%Y-%m-%d %H:%M %z'`; PowerShell — `Get-Date -Format 'yyyy-MM-dd HH:mm zzz'`), никогда на
   глаз; перед любой церемонией закрытия печатай
   `BOUNDARY: now <эта проба> · named <час владельца> · pool <empty | N items>` — решают часы (origin issue #96).
2. **Человек написал в чат — сначала классифицируй, потом переключайся.** Прямая просьба → выйти,
   немедленно переключиться на него. **Мимоходом брошенная идея или баг не про текущую задачу** →
   оформи документом (`/propose-idea` / `/report-bug`, источник: «брошено владельцем»), подтверди
   одной строкой и ПРОДОЛЖАЙ ночь. Уровень видения → `/fix-vision`, затем продолжай. Сообщение,
   пришедшее ПОСРЕДИ хода, — то же слово: автора подписывает система (`AGENT_GUIDE.md` → «Слово
   владельца посреди хода»): «стоп» завершает цикл в этом же ходе; «переключись» — сначала строка
   `PARKED:` в `STATUS.md`.
...
3. **Сделай**: код → сборка (см. `AGENT_GUIDE.md` → «Сборка») → развернуть → тест на стенде (см.
   ...
   (`INTENT`/`AUTH`/`TWINS`/`PENDING`) действуют и внутри цикла. ТЯЖЁЛАЯ задача без готового плана →
   сначала `/plan-epic` (исследование → метаплан), затем исполняй по фазам.
...
- Стоп = час подъёма → **начни `/end-chat-soft`**: доведи текущую задачу до естественной точки
  среза в обычном темпе, затем полные неспешные церемонии (статус + эстафета, проход судьи,
  коммит И push, сводка ночи в чат). Час подъёма ограничивает РАБОТУ, а не закрытие.
- Стоп = человек написал — переключись на него; дай короткую сводку ночи.
- Стоп = критическая ошибка — доведи текущий микрошаг до собирающегося состояния, если можешь,
  **закоммить и запушь** (не оставляй сломанный или незакоммиченный `main`), опиши, что случилось
  и что пробовал; жди.
